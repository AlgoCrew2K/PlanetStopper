...
def measure(label, url, headers=None, params=None):
    ...
    try:
        ...
        print(f"[{label}] status={resp.status_code} time={dt:.2f}s "
        ...
        try:
            ...
            import json
            j = json.dumps(data)
            print(f"    json_parse={time.perf_counter()-t1:.2f}s "
            ...
            print(f"    json parse FAILED: {type(exc).__name__}: {str(exc)[:120]}",
                  flush=True)
    except Exception as exc:
        print(f"[{label}] FETCH FAILED time={time.perf_counter()-t0:.2f}s "
...
if fred_key:
    measure("FRED-DGS10", ai_advisor._FRED_OBSERVATIONS_URL, params={
        "series_id": "DGS10", "api_key": fred_key, "file_type": "json",
