...
FAKE_AUTOTUNE = {
    "run_timestamp": "2026-05-11T16:00:00", "symphony_id": "test-symphony-a",
    "oos_alpha": 1.5, "train_alpha": 3.0, "baseline_decision": "Adopted AI",
    "fallback_oos_alpha": 0.8, "default_oos_alpha": 0.2,
}
...
prev = None
c0, w0 = mem()
print(f"MODE={MODE} N={N} baseline committed={c0:.1f}MB rss={w0:.1f}MB "
      f"handles={handles()} threads={threading.active_count()}", flush=True)
...
for i in range(N):
    with patch.object(ai_advisor.database, "get_latest_autotune_run", return_value=FAKE_AUTOTUNE), \
    ...
    if i % 5 == 0 or i == N - 1:
        ...
        print(f"iter {i:3d}: committed={c:8.1f}MB rss={w:8.1f}MB "
              f"handles={handles():5d} threads={threading.active_count()} "
              f"gc_objs={len(gc.get_objects()):7d}", flush=True)
        if prev is not None:
