...
class _FakeResp:
    ...
    headers = {}
    def json(self):
        import json; raise json.JSONDecodeError("x", "", 0)
    def raise_for_status(self):
        from requests.exceptions import HTTPError; raise HTTPError("429")
...
FA = {"run_timestamp": "t", "symphony_id": "s", "oos_alpha": 1.5, "train_alpha": 3.0,
      "baseline_decision": "Adopted AI", "fallback_oos_alpha": 0.8, "default_oos_alpha": 0.2}
...
with patch.object(ai_advisor.requests, "get", _fake_get):
    ...
    for name in ("_build_suggestible_surface", "_read_current_strategy",
                 "_build_sentiment_section", "_build_macro_section",
                 "_build_fundamentals_section", "_build_technicals_section",
    ...
    for i in range(3):
        with patch.object(ai_advisor.database, "get_latest_autotune_run", return_value=FA), \
