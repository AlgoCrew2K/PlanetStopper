...
1. **A real, currently-RED, production completeness gap in `_compact_subtree`'s verbatim-copy path** (fr-test, `7ca7c0c6`): a nested if-node that does not independently qualify as a cascade rung or hedge sub-gate is copied verbatim into a cascade overlay, and that verbatim region can mix a genuine nested RSI tier with unrelated `CORE_ASSET_` placeholder content -- leaking core-strategy structure into the reported overlay. 2 test parameters (`test_detected_cascade_fire_branch_never_includes_a_core_asset_placeholder` on `real_tree_04`/`real_tree_06`) are left correctly RED pending a fr-engine/team-lead verdict; never blind-made-to-pass.
2. **A third size-cliff-constant reuse, uncalibrated for that reuse** (team-lead relay, fr-review finding): `_is_internal_hedge_subgate` still uses `_SIZE_CLIFF_MAX_RATIO`/`_SIZE_CLIFF_MAX_ABSOLUTE_FIRE_NODES` for its own narrower "is this hedge-internal machinery" question (distinct from cascade-root qualification, out of scope for the AC-6 rebuild) -- fr-review flagged this specific reuse as display-fidelity-scoped and non-blocking, but never separately calibrated against real data the way the original cascade-qualification use was. Tracked, not fixed this cycle.
3. **A "vs(rsi(XLV,50)) display-fidelity deferral"** (team-lead relay, 2026-07-16) -- relayed to fr-doc as a named residual without full supporting detail; recorded here verbatim as cited rather than expanded on, since fr-doc does not have independent context to describe it further. Follow up with the team lead or fr-engine directly before treating this as understood.
4. **A "threshold-sanity-flag candidate"** (team-lead relay, 2026-07-16) -- same status as #3: relayed without full detail, recorded as a named open item, not expanded on speculatively.

### Files changed (this cycle, `d79760d0..e90a1626`, partial -- Cluster D wiring not yet landed as of this entry)
...
- `advisors/frontrunner_detector.py` -- AC-3 `extract_fr_checks` (new), AC-6 `detect_frontrunner_cascades` rebuild onto the direction-explicit rule, two independent bug fixes (early-continue, second range filter) (`bf6f026b`)
...
- `tests/advisors/test_frontrunner_detector.py` (pre-existing, wave-1) -- 12 cycle-caused stale parametrized failures found and reproduced by fr-test (`7ca7c0c6`); 10 fixed (9-param stale-range-assertion deletion + 1-param Paragons carve-out removal), 2 left correctly RED (residual #1 above)
