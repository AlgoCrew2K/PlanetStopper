...
## DE-SLEEVES-P3-001 — Managed Sleeves P3: wiring + surfaces (2026-07-08, cycle in progress — GREEN landed, one BLOCK-level gap still open per task #31)
...
| **OPEN (not yet resolved): `reconcile_sleeve_or_pause` must compare AGGREGATE cash/positions across all sleeves sharing one Alpaca account, not one sleeve's own cash against the whole account's cash** | As currently committed (`9d8e46c`), the function passes one sleeve's own `ledger_state.cash_usd` against `alpaca_orders.get_account`'s account-level `"cash"` field — correct only by coincidence when exactly one sleeve exists; a second sleeve would cause every sleeve to false-pause every tick, since each one's fractional cash share will never match total account cash within tolerance. Found by `s3-ux`'s live multi-sleeve visual-gate testing (task #25), not by the original RED/GREEN cycle. **PM ruling recorded:** replace with a one-sided aggregate cash invariant (`Σ sleeve cash ≤ account cash + tolerance`; a float-based check is sufficient) where an aggregate breach pauses ALL sleeves; positions get aggregated per-symbol across sleeves, with an unattributed broker position (held by neither ledger) ignored rather than flagged as drift. RED assigned to `s3-test-writer` (multi-sleeve/float/external-position scenarios + fixture rewrite), GREEN to `s3-engine` (task #31, pending as of this entry — RED/GREEN not yet dispatched to completion). This row will be updated with commit SHAs and pass counts once it lands, not rewritten. |
