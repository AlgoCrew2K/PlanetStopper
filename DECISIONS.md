...
**Update to this section (2026-07-16, same day as the original write, folding in context that crossed in transit per the team lead's instruction to land it on the next DECISIONS touch rather than a dedicated commit):**

1. **RESOLVED, not a residual anymore — the `_compact_subtree` verbatim-copy completeness gap on `real_tree_04`/`real_tree_06`** (originally residual #1 in this entry's first version, fr-test's `7ca7c0c6` finding). Fixed at commit `101ad377`: `_is_internal_hedge_subgate` and `_compact_if_node`'s own fire/continuation selection were migrated from size-based to direction-explicit (matching `_qualifies_as_cascade_rung`'s existing rule), and `_compact_subtree`'s verbatim-copy path now stubs a non-qualifying nested if-node carrying a `CORE_ASSET_` placeholder when neither of its own branches reaches VIX. fr-doc verified this directly against the `101ad377` source (not the commit message alone) before updating `docs/generated/advisors_frontrunner_detector.md`. **Replaced by a narrower, newly-discovered residual (below): `real_tree_09`'s inverted-polarity gate.** Also worth recording honestly: an earlier, isolated attempt at this same direction-explicit migration (`7ca7c0c6`, a team-lead-ratified hypothesis) was tried and FALSIFIED — it introduced 3 new failures by moving the fire/continuation mislabeling rather than fixing it, because it changed the branch-selection rule without also fixing `_is_internal_hedge_subgate` or the verbatim-copy path together. The later, comprehensive fix (`101ad377`, fixing all three in the same commit) succeeded where the narrow one failed — a real illustration of "the right fix sometimes requires fixing co-dependent functions together, not incrementally."
2. **NEW residual — `real_tree_09`/n2oo inverted-polarity hedge gate** (fr-engine, `101ad377`, not yet resolved, flagged separately to the team lead): a non-qualifying if-node in this tree has genuine VIX content (VXX/UVXY) in its ELSE branch and `CORE_ASSET_` placeholders in its TRUE branch — the inverse of the pattern every other guard in this file assumes. The defect-#5 fix deliberately checks "does EITHER branch have VIX" before stubbing (to avoid erasing genuine hedge content when the polarity is inverted), so this node falls through to the ordinary verbatim-copy path — the `CORE_ASSET_` leak persists here BY DESIGN, since losing real VXX/UVXY signal would be strictly worse than the known impurity. 2 test parameters remain genuinely RED (`test_detected_cascade_fire_branch_never_includes_a_core_asset_placeholder`, both `real_tree_09`) pending a team decision on how to handle inverted-polarity gates generally.
3. **MOOT, not a residual anymore — the third size-cliff-constant reuse in `_is_internal_hedge_subgate`** (originally residual #2, team-lead relay of an fr-review finding). That reuse no longer exists as of `101ad377` — `_is_internal_hedge_subgate` was migrated off `_SIZE_CLIFF_MAX_RATIO`/`_SIZE_CLIFF_MAX_ABSOLUTE_FIRE_NODES` entirely (see residual #1's resolution above). Both size-cliff constants are now fully unused anywhere in `frontrunner_detector.py`, kept only as the AC-6 regression-test anchor.
4. **A discriminator COMPLETENESS fix, not a residual — recorded for completeness of the record** (fr-engine, `101ad377`, distinct from the Stage-1/2 rhs-fn falsification story already in this entry): `_is_ticker_comparison`'s original `bf6f026b` implementation checked `rhs-fixed-value? is False`, but real trees routinely omit that key entirely (`None`) for a genuine ticker comparison rather than setting it explicitly `False` — confirmed on a real node in `real_tree_04` (id `74083377-ec89-4844-8ec1-d80bc2aae07c`, `rhs-val="SH"`, no `rhs-fixed-value?` key at all). Per the verdict's own population sweep, `False` (385 nodes) and absent/`None` (510 nodes) both pair exceptionlessly with a ticker `rhs-val` (895/895 combined); only `True` (166/166) pairs with a numeric `rhs-val`. The `is False`-only check silently dropped the 510-node absent-key population entirely (`extract_fr_checks` returned no tuple at all — neither `fr_key` nor `rhs_ticker`, a fully vanished FRCheck). Fixed to `is not True`. **Does not affect the reported 44/550 baseline** (ticker comparisons never populate `fr_key` under either version) but does improve completeness of genuine crossover/`vs()`-form capture going forward.
5. **`vs(rsi(XLV,50)) display-fidelity deferral`** (team-lead, full context relayed 2026-07-16, folded in here per the team lead's instruction — this entry's first version cited this only as a bare, undetailed item). For genuine ticker-crossover checks, `FRCheck` captures `rhs_fn` (ratified amendment at `07b4c0cb`, from fr-test's `0d98c2bb` finding that `rhs-fn-params.window=50` is REAL RHS indicator data for the crossover case) but NOT `rhs_window` — fr-engine explicitly declined to add the field because no RED test drives it, correct TDD minimalism per their own message on record. Consequence: `advisors.frontrunner_builder.format_vs_fr_key` renders the `vs()` display form as `LQD:50:vs(XLV)` today, not the fully RHS-indicator-faithful `vs(rsi(XLV,50))`. Deferred follow-up: needs a new fr-test RED test pinning a `FRCheck.rhs_window` field plus the corresponding `format_vs_fr_key` change. Recorded as: a display-fidelity enhancement, deliberately deferred per TDD discipline, not a defect.
6. **Threshold-sanity-flag candidate** (team-lead, full context relayed 2026-07-16, folded in here per the same instruction as #5). The corrected extraction surfaces low-threshold RSI-"overbought" gates in the operator's real book (the `DGRO:10:30` / `EDC:10:26` / `TQQQ:10:30` class — a `gt` gate at an oversold-range threshold fires ~always, the same underlying disease as the `SPY:10:31` misdiagnosis this whole cycle exists to fix) that have NO exact Atlas `fr_key` match, so they classify `no_edge_data` and render with no warning of any kind. Candidate follow-up (explicitly NEXT-cycle scope, needs its own A/C): a heuristic "suspicious configuration" flag — e.g. an overbought-`gt` gate with threshold below roughly 50 — so `SPY:10:31`-class misconfigurations remain visible to the operator even without Atlas edge evidence to back them. This cycle deliberately ships evidence-only classification; the sanity-flag heuristic is a new UI surface, not a data-pipeline fix, and is out of scope here.

### Files changed (this cycle, `d79760d0..101ad377`, partial -- Cluster D wiring not yet landed as of this entry)
...
- `advisors/frontrunner_detector.py` -- AC-3 `extract_fr_checks` (new), AC-6 `detect_frontrunner_cascades` rebuild onto the direction-explicit rule, two independent bug fixes (early-continue, second range filter) (`bf6f026b`); a further comprehensive fix migrating `_is_internal_hedge_subgate`/`_compact_if_node`/`_compact_subtree` off the size-based premise entirely, plus the discriminator completeness fix (`101ad377`)
- `advisors/frontrunner_builder.py` -- AC-5 six new functions, unit-tested, NOT YET WIRED (`bf6f026b`; wiring tracked separately as Cluster D)
...
- `tests/advisors/test_frontrunner_detector.py` (pre-existing, wave-1) -- 12 cycle-caused stale parametrized failures found and reproduced by fr-test (`7ca7c0c6`); 10 fixed at `7ca7c0c6` (9-param stale-range-assertion deletion + 1-param Paragons carve-out removal), 2 more fixed at `101ad377` (the original `real_tree_04`/`real_tree_06` completeness gap); 2 remain correctly RED, but as a DIFFERENT case than originally documented (`real_tree_09`'s inverted-polarity gate, residual #2 above)
- `feature-plans/frontrunner-signals.md` -- 6 plan-doc amendment commits recording every ruling/verdict/falsification as it happened (`101df72a`, `77060593`, `d166d871`, `99960a57`, `06c6ebf6`, `93e27efb`, `07b4c0cb`) -- the plan-doc-commit-per-ruling discipline is itself part of what made this entry possible to write accurately
- `docs/generated/advisors_frontrunner_signals.md` (new), `advisors_frontrunner_detector.md`, `advisors_frontrunner_builder.md`, `INDEX.md`, `app.md` -- this cycle's doc-writer output, including two correction commits (`e90a1626` after an AC-5 overclaim was found and fixed; `c595b97b` after `101ad377` landed and this doc needed reconciling against it)

### Verification

**Not yet complete as of this writing -- this section will be updated when it is.** What is confirmed as of `c595b97b`: 110/110 across the full RED batch at `bf6f026b` (fr-engine's self-reported count, consistent with fr-test's/fr-fe's own per-file counts); fr-engine's own reported sweep at `101ad377` (all 11 real fixture trees, 1,309 detected cascades, 0 no-vix violations, 0 `CORE_ASSET_` leaks except the flagged `real_tree_09` residual); fr-doc's own independent spot-verifications throughout this cycle (documented inline above and in `docs/generated/advisors_frontrunner_detector.md`) of the Paragons retraction, the rhs-fn discriminator mechanism, the SPY:70:62/51-43 node-count citation via the real `_count_nodes`, the final 44/550 recovery number on two trees, and — for this update specifically — direct confirmation via `git show 101ad377:advisors/frontrunner_detector.py` that the size-cliff constants are genuinely unused and the fire/continuation split is genuinely direction-explicit (not accepted from the commit message alone). What is NOT yet confirmed: fr-test's/fr-review's full-battery targeted `-n0` + credential-less counts at a stable final SHA (Cluster D wiring has not landed as of this writing, so no such SHA yet exists); the PM's first-hand live E2E on the rendered Frontrunner tab with real signal data (per the project's hard "tests-green is never sufficient" gate); and CI's authoritative full-tree `-n2` run. Per the team-lead's standing instruction, this entry's numbers are cited from direct verification (fr-doc's own runs) or explicit attribution to the producing agent (fr-engine/fr-test/fr-review) -- never fr-doc's own paraphrase of unverified claims.

### Reference

DE-FR-SIGNALS-001; branch `feature/frontrunner-signals`; plan `feature-plans/frontrunner-signals.md`; verdict `.claude/fr-signals-inputs/mirror-pattern-verdict.md`; addendum in `direction-validation.md`. Commits cited: `915ba65f` (plan), `101df72a` (AC-7 persisted-render ruling), `77060593` (Stage-1 discriminator, now retracted), `d166d871` (xover-form ruling), `99960a57` (vs-form + self-mirror UNRESOLVED, later mooted), `06c6ebf6` (AC-4 fixture-table fix), `93e27efb` (the verdict -- Stage-1 retracted, discriminator corrected), `07b4c0cb` (FRCheck amendment), `212f41a5` (AC-1/AC-2 GREEN), `ae5fe22d` (AC-7 UI GREEN), `bf6f026b` (AC-3/4/5/6 GREEN), `7ca7c0c6` (stale-cluster fix, and a falsified-then-reverted fire/continuation-direction attempt), `101ad377` (the comprehensive fix that succeeded where `7ca7c0c6`'s narrow attempt failed -- `_is_internal_hedge_subgate`/`_compact_if_node`/`_compact_subtree` migrated together, plus the discriminator completeness fix), `e90a1626` (this doc-writer's own AC-5-overclaim correction), `c595b97b` (this doc-writer's reconciliation of the detector doc against `101ad377`). Supersedes `DE-FRONTRUNNER-001`'s wave-1 detector description for the cascade-recognition rule (see `docs/generated/advisors_frontrunner_detector.md`'s "Superseded" section -- the wave-1 signature is preserved as dated history, never rewritten). This entry is itself incomplete pending Cluster D wiring and the final PM-authoritative gate run -- update the Verification section, not this Reference section, when those land.
