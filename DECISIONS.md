...
**STATUS: PARTIAL — AC-14 is a TWO-MECHANISM scope, both in scope from the start; the PM issued a precision correction (2026-07-13) after this doc-writer's prior wording risked implying one mechanism was swapped for the other. It was not — both mechanisms are separately required and tracked; one has landed, one has not.**

**Mechanism 1 — route-side suppression (display honesty, including historical rows already in the DB): GREEN, landed by r1-fe, commit `9693cdc4` on `fix/advisor-remediation-r1`. 294-test targeted regression pass clean before commit.**

**Mechanism 2 — producer-side no-write-when-flag-off (`divergence_explainer.py` early-return when `SECOND_WINDOW_CVAR_ENABLED` is off, so `NOT_APPLICABLE` rows are never WRITTEN in the first place — the audit's "dead producer still emits rows" half, i.e. DB-growth hygiene, distinct from Mechanism 1's display-honesty half): STATUS: PENDING, r1-engine's queue. RED already committed at `caa878fb`; implementation not yet landed.**
