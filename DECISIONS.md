...
**STATUS: PARTIALLY GREEN — correcting this doc-writer's own earlier overstatement (2026-07-13): the prior version of this entry marked AC-14 fully GREEN; r1-fe's follow-up clarified it is two INDEPENDENT halves, only one of which has landed.**

**Half 1 — display-layer suppression + guardrail copy: GREEN, landed by r1-fe, commit `9693cdc4` on `fix/advisor-remediation-r1`. 294-test targeted regression pass clean before commit.**

**Half 2 — producer-side stop-writing (`divergence_explainer.py` early-return when `SECOND_WINDOW_CVAR_ENABLED` is off, so `NOT_APPLICABLE` rows are never WRITTEN in the first place): STATUS: PENDING, r1-engine's queue, NOT landed. Explicitly NOT closed by Half 1** — r1-fe's fix only stops the row from being SERVED (filters at the route/display layer); the row is still written to the DB. Do not read Half 1's GREEN as covering this.
...
**Half 1's mechanism diverges from the PM's original adjudicated fix direction — recorded precisely, confirmed by r1-fe:** the adjudication drafted a SOURCE-side fix ("`run_divergence_explainer` will no longer WRITE `NOT_APPLICABLE` rows"). What Half 1 actually implements is a DISPLAY-side suppression predicate — `advisors/divergence_explainer.py` was NOT touched by r1-fe; the fix lives inside `api_advisor_observations()`, the `GET /api/advisor-observations` route handler (`app.py:5191`, the filter block), which was leaking `NOT_APPLICABLE` rows verbatim on the `symphony_id`-filtered path (the no-`symphony_id` path was already safe via the existing `_ADVISOR_ROLES` exclusion). Fixed by applying the SAME suppression predicate the Overview panel already used. **r1-fe confirms this matches the PM-adjudicated root cause exactly:** `database.py:1226`'s `get_advisor_observations_for_symphony()` IS the no-role-filter accessor that leaks the row; r1-fe did NOT edit `database.py` — the fix filters its return value at the route layer. **This is display-only — it stops the row from being SERVED, not from being WRITTEN.** Half 2 (stop the write at the source, `divergence_explainer.py`) is the SEPARATE, still-pending piece — it stays in r1-engine's queue, per the plan's own division of labor, not a gap in r1-fe's work.
