...
**Open item gating sf-review's final approve: SF-R-1 (reviewer BLOCK, PM-upheld) — RED-pinned at `55610d6`, sf-eng fixing, NOT YET RESOLVED.** A crash inside `dispatch_action` immediately AFTER a successful broker submit (e.g. `database.attach_alpaca_order_id` raising post-ack, with the pre-submit RESERVED insert having already succeeded) discards the runner-local evidence (`fire_ids`/`action_results` order IDs) that finding #9's latch-compensation reads — both look empty, pacing restores, and a still-true condition submits a SECOND live order on the very next tick, potentially doubling a real position. Confirmed RED live: tick 2 called `submit_bracket_order` with a fresh `client_order_id` while the first order sat live at the broker. **Pinned invariant (test-side, mechanism left to the fix):** a crash anywhere around submit must never double an order — when the broker may have received it, compensation must fail CLOSED (accept one lost/silenced episode, the same documented trade-off as finding #9) rather than open (re-fire); only the two-tick outcome is pinned (submit called exactly once across both ticks, exactly one `sleeve_orders` row, no new fire from the crashed tick). This is the one item gating sf-review's final approval of the cycle — do not treat the cycle as complete until this lands GREEN.
