...
**Findings #1/#2 (CRITICAL, `sleeve_id` KeyError / mode-override), #4 (fill-poll-before-cancel), #5/#19 (stop-rounded-before-sizing), #6 (partial-fill delta pricing), #9–#15 remainder (MED hardening batch), AC-11 churn brake — RED landed; GREEN is IN-FLIGHT IN THE WORKING TREE, NOT YET COMMITTED (sf-eng, touching `sleeves/tick_orchestrator.py`, `sleeves/rules/actions.py`, `sleeves/alpaca_orders.py`, `sleeves/reconciliation.py`).** Do not treat these as fixed until a commit SHA lands — an uncommitted working-tree diff is not a shipped fix. `37fefbe`'s own commit message notes the capstone end-to-end test (`test_paper_arm_status_promotion`'s full-ladder test) already passes against sf-eng's in-tree (uncommitted) wiring fix, which is a strong signal finding #1/#2 are functionally resolved, but this entry will only say RESOLVED once that lands as a real commit with its own verification counts. This entry will be extended with each remaining finding's own resolved-paragraph at that point. See the audit's own recommended fix order (`VERDICT-branch.md` §7) for sequencing: #1+#2 together, then #3 (done), then #4, then #5/#19+#6, then #7/#8 (done), then the MED/LOW hardening wave.
