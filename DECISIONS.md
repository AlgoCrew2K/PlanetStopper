...
Cycle in progress. Migration 034 (`c1047c1`), the P2 RED suite (`817df3e`, revised `6872d40`/`f883059`/`ab4ea8a`), and the P2 GREEN (`7e0efe1`, 314 passed / 0 failed / 0 skipped across `tests/sleeves/`) have all landed on `feature/managed-sleeves`. Open: s2-review's verdict (pending) and the entry-requires-explicit-stop schema change below (being encoded as RED now — not yet GREEN). This entry will be finalized once those close out before cycle-complete.
...
| Entry actions require explicit exit parameters (`stop_loss_pct` or `trailing_stop_pct`; `take_profit_pct` optional) in the rule document — a `buy` with no declared stop is a schema error, not a silent engine default | Credit to s2-rules-impl, who surfaced this in the `7e0efe1` commit body rather than silently deciding (naming `_DEFAULT_BRACKET_STOP_LOSS_PCT`/`_DEFAULT_BRACKET_TAKE_PROFIT_PCT` explicitly as "NOT a tuned risk parameter, flagged as an open design point"). `sizing.py`'s `risk_pct` mode already requires a stop distance to compute qty, so an entry action's definition is incomplete without one; AC-7 makes the exit part of the entry, owned by the rule author, never a silently-applied engine constant; a hardcoded default stop would couple an untested constant into real position sizing. The placeholder bracket-default constants are being removed with no fallback, encoded as a RED test now (status as of `7e0efe1`: `schema.py` still permits a bare `buy` with no exit field, and `actions.py` still applies the 10%/20% default bracket band — not yet corrected at the time this row was written; see the Status line above for the live cycle state). |
