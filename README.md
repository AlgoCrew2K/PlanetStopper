...
| `guard_preconditions.py` | Per-symphony Kaminski & Lo (2014) stop-justification statistics — lag-1 return persistence vs. daily Sharpe ratio, classified into 5 honest verdict classes. Pure math, no I/O; advisory display only, surfaced on the Performance tab. |
...
- **Performance (`/performance`)** — returns, Sharpe/Sortino, drawdown, calmar, win-rate, and the live-vs-counterfactual ("Guard Alpha") comparison. The route surfaces an "insufficient history" banner below a minimum sample size so underpowered metrics are not shown as precise. A separate Stop-Justification Preconditions panel measures, per symphony, whether the statistical evidence (return persistence vs. Sharpe ratio) supports a trailing stop's theoretical expected-return case at all -- distinct from the realized Guard Alpha comparison above (see `guard_preconditions.py`, `DE-GUARD-ALPHA-PRECONDITIONS-001`).
