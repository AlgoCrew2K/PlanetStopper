...
- **Per-exit attribution telemetry (approved 2026-06-01)**: persist the currently-unused `also_true` co-fire field in `exit_triggers` (richer per-layer exit attribution) so future exit-layer decisions — e.g. a SIP re-decision — are data-rich, not a 23-row judgment call. Small, low-risk; standalone change or folded into Phase 3.
- **Alpaca paper key stored (2026-06-01)**: `ALPACA_KEY`/`ALPACA_SECRET` in gitignored `.env`; IEX data-API auth validated (HTTP 200). Phase 1's 250-day fetch is UNBLOCKED (data via `data.alpaca.markets` free IEX). The paper-*trading* endpoint is separate + not needed for the overhaul.
