...
- [ ] AC-3: Daily incubation tick (off-hours, off-execution-path, CC-2 lazy import, D-1 never-raises): for each `INCUBATING` candidate, fetch its forward daily return via ONE `composer_backtest_client.run_backtest` call windowed from admitted_at (existing 1 req/s client; the day's new segment extracted from the response), plus the same-window SPY series (one shared call per tick, not per candidate); append to `incubation_daily`. Bill-bounded: ≤ MAX_INCUBATING+1 Composer calls per day, batched in one tick. A failed fetch degrades that candidate's day honestly (row absent; no fabrication); N consecutive fetch failures (`INCUBATION_MAX_FETCH_FAILURES = 5` [PM-ASSUMED]) or a Composer 422 tree-invalid → status `EXPIRED` with reason.
...
- [ ] AC-5: Surfacing: the Strategy Builder tab renders survivors with an honest status badge — "Incubating — day N of 63", "Promoted — recommended", "Failed incubation (reason)", "Expired (reason)" — and ONLY `PROMOTED` candidates render in the recommended/actionable styling. The weekly scheduler's persisted advisory observations gain an additive `incubation_status` field; existing consumers parse old rows unchanged.
...
- **Engine**: new `advisors/incubation.py` — admission adapter (called from `strategy_builder_engine.propose_strategies` persistence path + the weekly scheduler), the daily tick (`run_incubation_tick()`), promotion evaluation. D-1 contract throughout; `type(exc).__name__` error surfacing.
- **Scheduling**: the daily tick invoked from the daemon's existing off-hours daily slot (same pattern as the 03:00 lens pipeline — CC-2 lazy import from app.py's scheduler; NOT a new OS timer) [PM-ASSUMED — if recon shows the daemon slot is unsuitable, a `python -m advisors.incubation` entry + systemd timer mirrors prism_scheduler; recon decides, flag to PM].
...
- Daemon down for days → tick catches up from last recorded trading_day (window is calendar-anchored to admitted_at, not tick-count-anchored).
