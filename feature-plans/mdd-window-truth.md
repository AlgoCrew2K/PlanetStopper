...
- [ ] **AC-2 (lifetime scalar kept, but never as a leg):** Composer's lifetime `max_drawdown` scalar remains available and is rendered as its OWN clearly-labelled figure (naming its `invested_since` start), never as the counterpart in the Bot-vs-Held subtraction.
