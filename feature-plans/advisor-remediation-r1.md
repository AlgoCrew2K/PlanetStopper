...
- **AC-7** `rejection_reason` (pbo_veto / below_spy_alpha / oos_inferior_to_incumbent / fdr_not_winner) is rendered per rejected candidate on all three advisor result surfaces; the blanket "did not clear the FDR correction threshold" copy (ai_advisor.html:1996/2002) is replaced; `fdr_not_winner` renders as "cleared the significance bar but was not the single promoted winner" (doc-reconciliation §1.6). **AC-7b (added mid-cycle, PM adjudication):** NEW engine-side token `oos_inferior_to_incumbent` for the KEEP_INCUMBENT-on-OOS-grounds case (fails the acceptance_gate.py:257 precondition), which previously persisted rejection_reason=None — indistinguishable from a survivor in persisted records; post-AC-17 this becomes the most common Evaluate-path rejection, so rendering it is load-bearing. Implementation contained to backtest_gate_engine.py (new branch, uses the already-available incumbent_oos_alpha batch param; acceptance_gate.py untouched); precedence stage-ordered: **pbo_veto > below_spy_alpha > fdr_not_winner > oos_inferior_to_incumbent > None** — with fdr_not_winner made an EXPLICIT `this_winner_trial_is_none` check (no longer a blind catch-all), so oos_inferior_to_incumbent is the precise residual: cleared every veto, WAS the batch's FDR winner, beat SPY, and still lost to the incumbent. (Corrects this AC's earlier precedence line, which was inconsistent with the PM's original chain message; a genuinely-FDR-winning candidate that loses to the incumbent was previously mislabeled fdr_not_winner — the explicit check fixes that too.)
...
### Panel unreachability (AC-17 — added mid-cycle on r1-engine's proven finding; PM adjudication, [PM-ASSUMED] — operator may overrule)
- **AC-17** ADOPT_CANDIDATE becomes REACHABLE for tree-structural candidates. Proven defect: all three engines construct BacktestCandidate with empty candidate_params/incumbent_params/theory_prior_params → candidate_panel_score is the CONSTANT 0.5 vs incumbent's CONSTANT 0.75 (hardcoded inc_stability=1.0, backtest_gate_engine.py:820) → `0.5 >= 0.75 + PANEL_ADOPT_MARGIN_THRESHOLD(0.0)` is false unconditionally → ADOPT_CANDIDATE mathematically unreachable regardless of performance; badge accessor (database.py:1631, `WHERE verdict='ADOPT_CANDIDATE'`) therefore returns 0 unconditionally. FIX (contained to advisors/backtest_gate_engine.py — acceptance_gate.py and autotuner.py UNTOUCHED): when candidate_params AND incumbent_params are BOTH structurally empty (no parameter-vector representation exists), the parameter panel is NOT APPLICABLE — set cand_stability = inc_stability (exact tie) so the adoption decision rests entirely on the OOS-superiority precondition (acceptance_gate.py:257) + the three hard vetoes (BHY winner, PBO, SPY baseline — which AC-4/5 are simultaneously making real). REQUIREMENTS: (a) the tie fires ONLY on both-empty — one-side-empty is a caller bug and must NOT trigger it (guard/assert); (b) partial param population is FORBIDDEN at the three construction sites (re-triggers the broken algebra: real params without a real theory-prior require stability ≥ 1.0 = zero-change candidates only); (c) panel_breakdown records the N/A honestly ("parameter panel not applicable — no parameter-vector representation") so UI/persistence never imply a panel evaluated; (d) regression: real-params candidates keep byte-identical semantics; (e) end-to-end RED: a strong-OOS empty-params candidate reaches verdict ADOPT_CANDIDATE and increments the badge accessor; an OOS-inferior candidate still KEEP_INCUMBENT. RATIONALE for in-R1: shipping honesty copy over a pipeline that structurally cannot ever produce a survivor would preserve the exact defect class this cycle exists to close; the panel's own design doc calls it a one-directional brake on top of OOS superiority, not an independent hard veto via a data-availability artifact.
