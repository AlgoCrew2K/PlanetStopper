...
- [ ] AC-5: Primary sample = the autotuner's existing 250-day if-held replay series per symphony (reuse the existing replay seam; do NOT re-derive symphony returns from raw ticker bars). Secondary/live corroboration = daily diffs of `shadow_history.current_return` (the LIVE if-held trajectory — never `shadow_return`, which is the frozen per-day exit value), restricted to the current epoch (epoch-additive semantics; never stitch across epochs).
...
- **Data**: replay sample via the autotuner's existing 250-day replay seam (recon task for the cycle: identify the exact accessor; audit anchor `autotuner.py:1346-1419` `_collect_sim_returns`); live sample via `shadow_history` reads (migration 008), current-epoch filter.
