...
## AC-1 RESULT (empirical profile — DONE 2026-06-29, cgroup-bounded on the droplet)
**VERDICT: `ALPHABOT_MAX_JOBS=1` ALONE SUFFICES — AC-3 (per-symphony memory release / chunking) is OUT OF SCOPE.**
- Full 11-symphony run at `n_jobs=1` inside a `MemoryMax=3G` cgroup: **cgroup MemoryPeak = 2.03 GiB** (process VmHWM 2.06 GiB) vs the 3.00 GiB cap → **990 MB headroom**; **zero cgroup OOM**. Memory growth ~1.1 MB/min (flat plateau — `history_125d` dominates, not trial accumulation).
- `save_autotune_run` confirmed reachable at `n_jobs=1` (single-symphony completion run: COMPLETED in 64s, 1 `autotune_runs` row written, peak 365 MB, exit 0).
- Root cause reconfirmed: `Parallel(n_jobs=-1)` at `synthetic_history.py:670` forks 2 workers on the 2-core box → parent (~2 GB) + 2 worker tick-data copies > 3 GiB → OOM (the live-service-cgroup OOM, dmesg Jun 26, pid 285168, UID 997). `n_jobs=1` → joblib sequential backend (no fork) → 2.03 GiB.
- **Fix scope locked:** AC-2 (bound `n_jobs=1` on the autotune path via a threaded param at `autotuner.py:2158`→`synthetic_history.generate_synthetic_history`, NOT a global default change) + AC-6 (droplet `.env` `ALPHABOT_MAX_JOBS=1`, defense-in-depth) + AC-7 (reproducibility-neutral) + AC-4 (live-verify). **`n_jobs=1` specifically** (on a 2-core box `n_jobs=2`≡`-1`, both fork 2 workers).

**AC-4 SHARPENING (PM, Rule-2 on the AC-1 caveat):** the 11-symphony 2.03 GiB peak was measured during the **replay phase only** (the 11-sym run was killed before the Optuna trial phase; trials were monkey-patched to 100, not the production 500). The single-symphony full-completion run did exercise the trial phase but only for 1 small-holdings symphony (365 MB). **AC-4 MUST run a FULL, real `OPTUNA_N_TRIALS_PRODUCTION` (500-trial), to-COMPLETION 11-symphony autotune under the 3 GiB cgroup** to confirm the *end-to-end* peak (replay + every symphony's full study) stays <3 GiB and `autotune_runs` populates for the real set — NOT another partial/reduced-trial run. (Per-symphony studies are released between symphonies per the recon, so the replay-phase `history_125d` is expected to remain the peak, but AC-4 must MEASURE this end-to-end, not infer it.)
