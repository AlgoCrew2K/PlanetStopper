...
- **OUT:** any exec/trade primitive (permanently); the `retirement_recommender.py` math core (byte-frozen); the recommender's gates/thresholds; **shadow_history retention** (the real reason the droplet shows 0 recs today is the ~49-day retention starving the conservative gates — a separate droplet ops/product knob, surfaced to the operator, NOT a code change in this cycle).
