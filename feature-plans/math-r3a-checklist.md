...
- [ ] **AC-9 (checklist status updated):** DECISIONS.md checklist items (a) and (b) are flipped from UNMET to MET with a pointer to the new artifacts/tests, so R3-d's gate reads true from the durable record.
...
- **Authoritative sources the RED tests read:** the Optuna search-space definition in `autotuner.py` (`~:308-309` squeeze range, plus the full space around `:253` `OPTUNA_N_TRIALS_PRODUCTION` / the space builder); `_MC_REPLAY_SIMULATION_PATHS=300`; the arm band `alpha_bot_execution.py:1373-1381`. DB sentinel: tests set `DB_PATH` via `tests/conftest.py`.
- **No new dependencies.**
