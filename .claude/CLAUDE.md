...
| `advisors/lens_options_proxy.py` | Options/vol proxy lens (feat/options-proxy): `_fetch_options_proxy() -> dict`; fetches VIXCLS (spot VIX) + VXVCLS (3-month VIX) from FRED (free, no subscription); classifies term-structure regime (contango=spot<3m, backwardation=spot>3m, flat=±2% via `_FLAT_BAND_RATIO=0.02`); derives risk read (risk-off=backwardation+VIX≥`_VIX_ELEVATED_THRESHOLD=20`, risk-on=contango+VIX<`_VIX_LOW_THRESHOLD=15`, neutral otherwise); honest-availability (`available=False`+`reason` on any failure); bounded retry (3 attempts, `MAX_OPTIONS_RETRY_WAIT_SECONDS=6.0 s`); D-1 (`type(exc).__name__` only); off-execution-path; **wired into `ai_advisor._build_derivatives_section` (lazy import, CC-2)** — the derivatives lens is a real producer. |
| `advisors/lens_breadth.py` | 500-name S&P market-breadth producer (gold-standard): `get_sp500_constituents()` (datahub CSV primary / Wikipedia fallback, 490–510 sanity band, dot-format pass-through, `[]` on failure), `compute_breadth(constituents)` (% above 50/200-day SMA over the QUALIFYING sub-universe — ≥200 bars + current bar; reports qualifying/total), `fetch_breadth()` (public entry; persists to `lens_warehouse`; honest-availability `available=False`+`reason`; never raises). Reuses `synthetic_history.fetch_bars` (Alpaca IEX, ~17 batched reqs). "Above" = `>=` (immaterial vs `>` for real prices). IEX-basis caveat. D-1. Off-execution-path; no production caller yet (intended to feed the technicals breadth number). |
| `advisors/lens_crossasset.py` | Cross-asset risk-appetite lens: `fetch_crossasset() -> {available, risk_read∈{risk-on,neutral,risk-off}, components, source, reason?}`; 6 free Alpaca ETFs — credit (HYG−LQD relative), duration (TLT), USD (UUP), gold (GLD), EM credit (EMB), commodity-growth (DBC); per-component momentum → derived aggregate (NO hardcoded signs, regime-robust); reuses `synthetic_history.fetch_bars` (1 batched req); persists to `lens_warehouse`; honest-availability (excludes missing components, `available=False` if <`_MIN_COMPONENTS`=3); constants `_RISK_ON_THRESHOLD`/`_MIN_BARS`/`_LOOKBACK_DAYS`; D-1; off-execution-path; inflation deliberately excluded (separate macro-lens concern — RINF); no production caller yet (feeds a 6th lens-section fast-follow). |
