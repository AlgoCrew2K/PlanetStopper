...
| `advisors/community_strats.py` | Community strategies loader: `load_community_strategies(*, limit, min_oos_sharpe, client, force_refresh) -> dict`; routes the `captplanet.strategies` Atlas read through `atlas_cache.cached_pull` (weekly TTL, operator bill-protection directive); validates via `symphony_schema.validate_tree`; deduplicates by tree-structural SHA-256 hash (`_strip_ids` → `json.dumps(sort_keys=True)` → sha256 — NOT `database.compute_composition_hash`, which hashes `list[str]` symphony IDs); sharpe-filters keeping no-sharpe docs; returns `{available, candidates, stats, source}`; never-raising D-1 contract; first production caller: `propose_strategies` via the `community_candidate_infos` adapter (injected at the route boundary) |
