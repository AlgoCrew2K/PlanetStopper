...
> **SUPERSEDED (this specific drafted copy only, 2026-07-13) — advisor-remediation-r1, DE-ADVISOR-R1-001.** The "Corrected text" above hardcodes "Claude Opus" (twice: "via Claude Opus" and "generated by Opus") — this predates the operator's 2026-07-13 AC-16 directive routing suggestion-producing LLM calls to Fable via an accessor (`model_config.get_advisor_suggestion_model()`), and AC-16's attribution-coherence clause forbids a hardcoded model-name literal in any attribution copy. **Do not apply this drafted string verbatim.** What actually shipped (r1-fe, commit `949ce47e`): the run-controls-note is accessor-driven (reads the live `advisor_suggestion_model` template context key, computed via `model_config.get_advisor_suggestion_model()` + a shared `_humanize_model_name()` display map), names no model literally, and otherwise preserves this section's substance (Atlas community sourcing, the single-strongest-candidate cap, FDR/Yekutieli calibration). The REQUIRED PAIRING (run-level provenance banner) is also shipped, as AC-11 — see `DECISIONS.md` `DE-ADVISOR-R1-001` §AC-1..3 and §AC-11..12 for the full record.
...
> **SUPERSEDED (2026-07-13) — advisor-remediation-r1, DE-ADVISOR-R1-001.** The wiring gap this section flags AS informational ("not a doc fix") is now CLOSED: `dated_returns=`/`spy_returns_fn=` are wired into both engines' `evaluate_candidate_batch` calls exactly as this section's suggested code fix describes (r1-engine, commit `82479560`, mirroring `strategy_builder_engine.py:807-826` as recommended). Gate strength is now uniform across all three advisor engines — no copy caveat is needed, and none was applied. See `docs/generated/advisors_backtest_gate_engine.md`'s "FDR integrity invariant" paragraph and `DECISIONS.md` `DE-ADVISOR-R1-001` §AC-4..6 for the full record.
