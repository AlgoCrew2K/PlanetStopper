...
- **`n_obs` addition (settled with `mdd-ui`/`mdd-test`, 2026-09-03):** both return dicts gain `"n_obs": int` — the count of trading days the same-window peak-to-trough was computed over (one count covers both `if_held`/`dry_run`, which are now same-window by construction). Named to match `guard_preconditions.py`'s existing `PersistenceStats.n_obs`/`N_MIN_OBS` statistical-sufficiency convention (closer semantic fit than `observation_count`/`coverage_days`). Backs `mdd-ui`'s re-scoped `mdd_insufficient` threshold (`n_obs < 30`), distinguishing "no computation happened" (`n_obs` 0/1) from "computed but below the Bailey/de-Prado stability floor." **Always present as an int whenever the function returns at all**, including when `dry_run`/`if_held` are `None`. Final return shape: `{"if_held": float | None, "dry_run": float | None, "if_held_lifetime": float | None, "n_obs": int}`.
- **Early-return path removed for `if_held`/`dry_run`/`n_obs`:** the OLD code early-returned `{"if_held": None, "dry_run": None}` when `sym_dict.get("max_drawdown") is None` (Composer scalar missing). That gate no longer applies to `if_held`/`dry_run`/`n_obs` — they are now fully independent of `sym_dict["max_drawdown"]` (sourced only from `shadow_history`). Only `if_held_lifetime` depends on the Composer scalar and can be `None` independently (when `sym_dict.get("max_drawdown") is None`), while `n_obs`/`if_held`/`dry_run` are still honestly computed (or honestly `0`/`None`) from `shadow_history` regardless.
