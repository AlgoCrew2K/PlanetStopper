...
| `guard_event_count` | int | Total exit-trigger count (VALID entries only, F-008). |
...
See `DE-GAP-001`, `DE-LIVE-DASH-001`, `DE-POSTMORTEM-INTEGRITY-001`, and `DE-EXIT-FRICTION-REALIZED-001` in `DECISIONS.md`.

**Consumed by:** `fetchGuardAlphaSummary()` in `static/index.js`. Populates `#dollar-saved-headline`, `#guard-event-count`, `#dollar-saved-basis-label` in `templates/index.html` (`data-testid="dollar-saved-panel"`) -- plus, as of exit-friction-realized-savings, the additive `#dollar-saved-realized-headline`/`#dollar-saved-realized-coverage` sibling elements (STATIC "marks basis" caption in the template, never JS-injected -- RULING A honesty requirement). Does not clobber `#guard-alpha-headline` (windowed % guard alpha from `/api/strip/<window>`).
