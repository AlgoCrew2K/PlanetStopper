...
- **AI Advisor routes** — unified single-page SPA at `GET /ai-advisor` renders all 6 tabs in one server-side render; GET sub-routes for all 5 old per-tab pages now 302-redirect to `/ai-advisor`; POST action routes (suggest, evaluate, accept, reject, chat/send, strategy-builder/run) kept their request shapes and paths, but the three operator-Evaluate routes (asset-swaps/evaluate, logic-changes/evaluate, strategy-builder/run) gained substantial response-JSON extensions in the advisor-remediation-r1 cycle — N=1 statistical-honesty caveats, a granular `rejection_reason` per candidate, a `low_power` statistical-power flag (plus caveat text), and (Strategy Builder only) run-level built-new/Atlas provenance + degraded-mode reporting. See `DE-ADVISOR-R1-001` in `DECISIONS.md` and the route sections below.
...
**Advisor-remediation-r1 response-JSON extensions (`DE-ADVISOR-R1-001`, 2026-07-13):**
- **AC-6 (N=1 honesty):** the top-level `caveats` field is built via `_n1_honest_caveats(proposal.caveats)` — strips any FDR/Yekutieli-branded text and appends the honest N=1 disclosure ("single-candidate check — no multiple-testing correction applies (N=1)"). This route always evaluates exactly one operator-named candidate, so an FDR-branded caveat here would misrepresent a no-op correction as real.
- **AC-7:** `gate_result` carries `rejection_reason` (one of `pbo_veto` / `below_spy_alpha` / `fdr_not_winner` / `oos_inferior_to_incumbent` / `None`) — the granular cause, distinct from the coarse `gate_reason` title.
- **AC-9 (low-power caveat, `a5eaa3b0`, closes an r1-review Checkpoint-3 finding):** `gate_result.low_power` is a boolean (`_low_power(validation_days)` against the `MIN_POWER_FOLD_DAYS=121` UI threshold, `app.py:3867` — a display-caveat constant, not a gate-math constant). When `True` AND the candidate is a genuine `ADOPT_CANDIDATE` survivor, `_LOW_POWER_CAVEAT` is appended to the top-level `caveats` array — the boolean alone (present since AC-9's first landing) was never a sufficient operator-visible disclosure; this commit added the actual caveat text.
- **AC-13:** `_evaluate_single_variant` (shared with Logic Changes) now returns a 4th tuple element, `baseline_returns_pct`, reused instead of issuing a second duplicate baseline `run_backtest` call — cuts one Composer round-trip per operator Evaluate click.
...
**Tests:** `tests/ai_advisor/test_ac7_route_json_rejection_reason.py` (8/8 GREEN), `tests/ai_advisor/test_r1_power_caveat.py` (12/12 GREEN, both routes' caveat-text append).
...
3. Call `propose_strategies(objective, universe, screen_config=ScreenConfig(), live_returns=[], symphony_id=..., community_candidates=...)` (`app.py:3813`). Built-new (accessor-driven generation model — currently Fable via `model_config.get_advisor_suggestion_model()`, AC-16 — C1→C2→C3) AND atlas-suggested candidates flow into ONE FDR batch (AC-21).
...
**Advisor-remediation-r1 response-JSON extensions (`DE-ADVISOR-R1-001`, 2026-07-13):** survivors additionally carry `low_power` (AC-9) and rejected candidates carry `rejection_reason` (AC-7, four values: `pbo_veto` / `below_spy_alpha` / `fdr_not_winner` / `oos_inferior_to_incumbent`); the run-level payload gains `built_new_count`/`atlas_count` provenance rollup and a `mode_notice` degraded-run indicator (AC-11), plus `screens_skipped`/`screens_skipped_reason` (AC-12, set when `live_returns=[]` causes the drawdown/Pearson screens to be skipped rather than silently applied against nothing). **Known gap, in progress (r1-review Checkpoint-3 BLOCK finding, tracked in `DECISIONS.md` `DE-ADVISOR-R1-001`):** these route-JSON fields are proven to reach the JSON response by route-level tests, but as of this writing `static/ai_advisor.js`'s `sbRunAnalysis()` — the LIVE-RUN handler wired to this route's "Run analysis" button — does not yet consume all of them (`built_new_count`/`atlas_count`/`mode_notice`/`screens_skipped`/`error_category`/`low_power`/`rejection_reason`). See [static/ai_advisor.js](static_ai_advisor_js.md) for the current (pre-fix) state of `sbRunAnalysis()`; that doc will be updated once the fix lands.
