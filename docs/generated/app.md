...
**Source:** `app.py`
**Last updated:** 2026-08-27 (Retirement Approval Lifecycle, Cycle 2c epic-completion, `DE-RETIRE-APPROVAL-001` / implementer-side `DE-RETIRE-POLISH-001`, HEAD `01521618` (`83926991` + a same-cycle safety-guard-extension commit, `01521618`, that extends the `tests/security/test_retirement_action_no_trade_boundary.py` transitive-AST/scoped-window scan to Cycle 2c's 4 new call paths -- Group E/F, mutation-tested by `ret3-review`; see the "`_dispatch_retirement_decision`" section below and `DE-RETIRE-APPROVAL-001`'s "Safety-guard extension" subsection in `DECISIONS.md`) -- closes the Retirement Recommender epic (Cycle 2a math core -> Cycle 2b operator surface -> Cycle 2c polish, no further cycle planned). Closes AC-1..AC-8 from a SECOND, later PR #139 `/code-review` pass (a distinct F1-F7 finding list from the FIRST pass documented in the "Prior" entry immediately below -- do not conflate the two). AC-1/AC-2: `_retirement_recommender_tick_worker()` and `ai_advisor_tab()`'s retirement block now resolve each symphony's DISPLAY NAME (`advisors.frontrunner_builder.resolve_incumbent_display_name`, reused verbatim, not a new lookup) for the card, the explainer prompt, and the persisted explanation -- the raw Composer hash stays available as a `title=` tooltip, never dropped, honest hash fallback when unresolvable. AC-3: the checklist block now renders `build_checklist`'s previously-discarded `candidate_name` field -- reverses the first `/code-review` pass's own F7 ACCEPT ruling ("unused by the template," see the Revise-round section below) on the SAME underlying fact. AC-4: new `_retirement_find_reusable_prior_explanation()` skips the nightly LLM explainer call and reuses the most recent prior night's explanation verbatim for a materially-unchanged (candidate_id, sibling_id) pair (9-condition match incl. both display names, fail-open on any lookup failure) -- bounded, non-recurring metered spend for a persistently-flagged pair; a genuinely new/changed pair is still explained fresh. AC-5 (`database.py`): `upsert_retirement_decision`'s `decided_at` is now preserved verbatim across every write once first set, instead of being re-stamped on every re-approve/re-reject. AC-6: the AC-4-from-Cycle-2b live-join relocates from `ai_advisor_tab()` into a new shared `_join_retirement_approval_status()` called INSIDE `_fetch_retirement_recommendations()` itself, so `GET /api/retirement-recommendations` now carries `approval_status` too, not just the rendered panel. AC-7 (`static/ai_advisor.js`): `retDispatchDecision(action, btnEl)` now derives the candidate id and ancestor card directly from the clicked button element (`.closest(...)`) instead of a page-wide `querySelectorAll` scan. AC-8 (`database.py`): the unused singular `get_retirement_decision` accessor is deleted (zero production callers, verified before deletion). `advisors/retirement_recommender.py`, `advisors/retirement_explainer.py`, and `advisors/retirement_checklist.py` are all BYTE-UNCHANGED this cycle (verified zero diff) -- every fix is app-layer/template/JS/DB-accessor only, preserving the LLM-free recommender core and the deterministic checklist/explainer modules untouched. See the updated sections below and `DE-RETIRE-APPROVAL-001`'s "Cycle 2c" section in `DECISIONS.md`.) Prior: 2026-08-26 (Retirement Approval Lifecycle, PR #139 review remediation, `DE-RETIRE-APPROVAL-001`, HEAD `5655edfd` -- a PR-level `/code-review` on the Cycle-2b diff found 4 fixable findings + a stale comment, all closed same-cycle before merge: F3, `static/ai_advisor.js`'s `retDispatchDecision` reject-confirmation color was hardcoded to success-green regardless of the action taken; F4, the same function's success handler never updated the card's own status chip, leaving a stale "Pending" chip next to a correct confirmation message; F5, `templates/ai_advisor.html` carried a dead duplicate hardcoded fallback for the checklist's holdings-unavailable note (unreachable, since `retirement_checklist.build_checklist` always sets the real constant whenever it's needed); F6, `app.py`'s live-join loop stamped an unused `_rec["_decision"] = _decision` key nothing ever read. Also this cycle: the previously-flagged `ruff format` gap in `_dispatch_retirement_decision` is fixed (`d5f74177`). See the "`window.retDispatchDecision`" section of `docs/generated/static_ai_advisor_js.md`, the F5/F6 corrections in the sections below, and `DE-RETIRE-APPROVAL-001`'s Revise-round section in `DECISIONS.md`.) Prior: 2026-08-26 (Retirement Approval Lifecycle, Phase 2 Cycle 2b, `DE-RETIRE-APPROVAL-001`, HEAD `04c483ca` -- layers an operator decision surface on top of the Cycle-2a Retirement Recommender: the 03:45 producer (`_retirement_recommender_tick_worker`) now runs each recommendation through `advisors.retirement_explainer.explain_recommendation` (fable-5, D-1) before persist, stamping `explanation` into `raw_response`; `ai_advisor_tab()` gains an AC-4 live-join reading the new mutable `retirement_decisions` table (migration 038) fresh per request, plus AC-6/AC-7 render-time checklist assembly (`advisors.retirement_checklist.build_checklist`, no LLM) for approved cards; two new status-only routes, `POST /ai-advisor/retirement/{approve,reject}`, delegate to a shared `_dispatch_retirement_decision` helper -- deliberately NOT the frontrunner `/proposal/approve` route (which reaches `composer_draft_client.save_symphony`). The LLM is strictly out of the approve/reject/checklist action path (operator ruling, Gate-2b), structurally proven by a transitive AST call-graph walk over both route handlers (`tests/security/test_retirement_action_no_trade_boundary.py`, a same-cycle fix replacing an original single-hop scan that a thin one-line route wrapper could have slipped past). The Cycle-2a panel doc's "no operator affordance" claim is corrected in place (historical pointer preserved, not deleted) -- see the "Retirement Approval Lifecycle" subsections below, the new `docs/generated/advisors_retirement_explainer.md` + `docs/generated/advisors_retirement_checklist.md`, and `DE-RETIRE-APPROVAL-001` in `DECISIONS.md`.) Prior: 2026-08-25 (`DE-CORR-DATE-ALIGN-001`, HEAD `6b164736` -- `/ai-advisor` route's Correlations-panel series assembly switched from a positional `list[float]` per symphony to a date-keyed `dict[str, float]` (`dict(zip(_dates, _live_rets))`), so `correlation_diagnostic.compute_pairwise_correlations` aligns each symphony pair by shared calendar date instead of raw list index; per-symphony trigger-day series are sparse and event-driven (unrelated calendars across symphonies), so positional alignment was pairing unrelated dates for any multi-symphony book. `correlation_diagnostic.py` carries zero diff -- its date-keyed alignment path already existed and was correct; the defect was purely this route's assembly. Known follow-up, not fixed here: the input remains exit-trigger-day returns (sparse, selection-biased), not a daily-NAV series -- a Phase-2 BASIS decision. See the updated Correlations-panel template-context row below and `DE-CORR-DATE-ALIGN-001` in `DECISIONS.md`. Prior: 2026-08-25 (`DE-CLOSED-BOUNCE-001` revise round, PR #136 `/code-review` finding #2, HEAD `faebc986` -- closed-market cumulative-return bounce, server leg. The original fix (HEAD `8b0ad603`) had the closed_frozen branch's new default-window block set `guard_alpha`/`window`/`windowed_cumulative_return`; finding #2 removed the `windowed_cumulative_return` assignment as write-only dead weight once the client-side fix's revise round (finding #1) meant no comparison row is ever sourced from a windowed-strip payload any more (see `docs/generated/static_index_js.md`) -- `guard_alpha`/`window` are unaffected. See the updated "Frozen/EOD snapshot recompute" subsection below and `DE-CLOSED-BOUNCE-001`'s revise-round addendum in `DECISIONS.md`. Prior: 2026-08-25 (`DE-CLOSED-BOUNCE-001` original cycle, HEAD `8b0ad603` -- closed-market cumulative-return bounce, server leg. The `closed_frozen`/`pre_market` branch's inline `_portfolio_strip` (built directly via account-basis helpers, never through `_compute_portfolio_strip`) never set `guard_alpha`/`window` at all -- the structural trigger that made `static/index.js`'s `renderGuardAlpha` fall back to `fetchWindowedStrip('30d')` on EVERY closed-market poll, racing a genuine `/api/state` poll for the comparison rows. Fixed with a dedicated `try`/`except` block (+33 lines, deliberately separate from the surrounding try) that mirrors the live path's own default-window strip computation (`app.py:1859-1881`).) Prior: 2026-08-25 (`DE-HELD-BASIS-001` F6 follow-up, guard_delta_vw leak closed on BOTH Tier-2-floor else branches) -- two structurally identical bugs: `_compute_portfolio_strip()`'s live-path Tier-2 today-change floor ELSE branch, and `get_state()`'s frozen/EOD-snapshot Tier-2 floor ELSE branch (`app.py:2412`). Both fire on zero paired coverage (no symphony has a same-trading-day `shadow_history` row, so `guard_delta_vw` resolves `None`) and both previously assigned their raw VW dict (`_vw_tc_floor` / `_snap_vw_tc`) verbatim to the outward-facing today-change field, leaking the internal `guard_delta_vw` key into the public `/api/state` JSON -- the same leak class F6 (below) closed on the paired-coverage IF branches, missed by F6's own regression test since it only covered those. Fixed on both by building an explicit fresh `{if_held, dry_run}` dict on the else branch, mirroring each branch's own IF-branch construction. `analytics.py` untouched -- the leak was the route's failure to strip the internal key before assignment, not a computation defect. See the updated "Tier-2 floor re-derivation" and "Held-basis convergence, frozen-branch parity" subsections below and the F6-follow-up addendum (with the reviewer's leak-surface completeness proof) to `DE-HELD-BASIS-001` in `DECISIONS.md`. Prior: 2026-08-20 (frontrunner-proposal-identity Revise 2, `DE-FR-PROPOSAL-IDENTITY-001`, PR #126 `/code-review` findings F5/F8/F9/F10) -- the `frontrunner_proposals` prefetch loop's per-row JSON-preview truncation is DRY-extracted into a new shared `_bounded_json_preview()` helper (F9); `database.load_state()` moves into its own dedicated `try`/`except` so a failure degrades identity resolution to the raw-hash fallback for every row instead of skipping the per-row loop body entirely, and is skipped outright when there are zero pending proposals (F5); `resolve_incumbent_display_name`/`OVERLAY_NOT_RECORDED_TEXT` are now imported from `advisors.frontrunner_builder` instead of duplicating the hash→name lookup and fallback-text literal locally (F8/F10); a new `overlay_not_recorded_text` template context variable threads the SAME Python constant into Jinja, closing a drift risk between two previously-independent copies of the same fallback string. See the updated "Proposal-identity render prefetch" subsection below and `DE-FR-PROPOSAL-IDENTITY-001`'s Revise-2 section in `DECISIONS.md`. Prior: 2026-08-20 (frontrunner-proposal-identity, original cycle, `DE-FR-PROPOSAL-IDENTITY-001`) -- `ai_advisor_tab()`'s `frontrunner_proposals` prefetch loop additively resolves each row's incumbent display name (hash→name via one request-scoped `database.load_state()` call, honest hash fallback), normalizes a non-`dict` `metrics_json` to `{}` (AC-6, closing a real Jinja `Undefined`-vs-`format()` crash this cycle found and fixed alongside the normalization), and builds a bounded `overlay_tree_preview` mirroring the existing `candidate_tree_preview` transform. See the new "Proposal-identity render prefetch" subsection under "`ai_advisor_tab()` — Frontrunner Builder panel prefetch" below and `DE-FR-PROPOSAL-IDENTITY-001` in `DECISIONS.md`. Prior: 2026-08-13 (held-basis-convergence Revise 1, PR #125 review findings F1-F9/F13/F14, `DE-HELD-BASIS-001`) -- the mandatory `/review` skill gate on the original cycle's PR returned 14 findings; F4 pivots marker resolution to `bot_state_entry`-primary (see `docs/generated/analytics.md`), making the original cycle's 2-site `app.py` sym_dict-marker-threading REDUNDANT and REMOVED; F6 requires 4 explicit `include_paired_guard_delta=True` opt-in sites (`_compute_portfolio_strip`'s Tier-0/1/2 + the frozen branch's `_snap_vw_tc`, a mutation-testing gap on the first two closed by a dedicated follow-up commit); F5 brings the frozen/closed-market branch to held-basis parity with its live twin (`_snap_bot_state` gains the marker key explicitly, the frozen Tier-2 floor gains the same paired re-derivation the live floor already had) -- correcting the original cycle's FALSE "structurally immune" claim about that branch (a real, narrower EOD Composer-fetch-failure gap exists, closed at the render layer only, zero diff to `alpha_bot_execution.py`, AC-7 held). See the updated `_compute_portfolio_strip()` and Frozen/EOD snapshot sections below and `DE-HELD-BASIS-001`'s "Revise 1" section in `DECISIONS.md`. Prior: 2026-08-13 (held-basis-convergence, original cycle, `DE-HELD-BASIS-001`) -- `_compute_portfolio_strip()`'s and `get_state()`'s live-poll sym_dict builders now thread BL-9's `current_return_is_reconstructed` marker through to `analytics.get_symphony_today_change` (2 sites: `app.py:~1508-1514`, `app.py:~2624-2630`), and `_compute_portfolio_strip`'s Tier-2 (no cached/last-good account totals) floor path re-derives its rendered `today_change.dry_run` from `if_held + guard_delta_vw` (the new paired-membership delta) instead of the raw dry-run-only-membership average -- fixes a Today-row guard-alpha overstatement (a triggered symphony's reconstructed `bot_state` value was standing in for its raw if-held trajectory) plus a latent phantom-delta-on-coverage-gap bug; zero diff to `alpha_bot_execution.py`/`math_engine.py`/`reporting.py`. See the updated `_compute_portfolio_strip()` section below and `DE-HELD-BASIS-001` in `DECISIONS.md`. Prior: 2026-08-05 (BL-5..8/10..12 hygiene bundle, `DE-AUDIT-BL5-12-001` -- seven independently-scoped hygiene/documentation/defensive fixes from audit #118's P3/P4 backlog (BL-9's basket-reconstruction marker ships as its own `DE-AUDIT-BL9-001` entry, `alpha_bot_execution.py` only, not this file). `GET /api/autotune-runs` additively stamps each row with `never_adopted_streak` (BL-8, `analytics.compute_never_adopted_streak`, grouped from the already-fetched rows, zero extra DB round-trip, response stays a bare JSON array); the stale `_ADVISOR_ROLES`-area comment above `api_advisor_observations()` claiming DIVERGENCE_EXPLAINER "still writes one per autotune run" is corrected (BL-7, false since AC-14; the filter below it is legacy-row defense for 22 pre-AC-14 rows, not ongoing-write suppression) -- see the updated sections below and `DE-AUDIT-BL5-12-001` in `DECISIONS.md`.) Prior: 2026-08-05 (BL-4, HEAD `b82de309`, `DE-AUDIT-BL4-001` -- `_ACCOUNT_TOTALS_HTTP_TIMEOUT_S` raised `10 -> 30` (AC-6, optional per the plan, [PM-ASSUMED] ruled in-scope; reviewer-noted tradeoff: this blocks the background scheduler thread, not the 1-minute engine dispatch path, up to 30s on a slow Composer call); `_compute_portfolio_strip()`'s own `basis`/`account_basis_stale`/`account_basis_as_of` computation is pinned byte-unchanged (AC-5) -- the render-side consumers (`static/index.js`) are new this cycle, see `docs/generated/static_index_js.md`; closes audit #118 Findings D1/D2/D4 -- see the updated `_account_totals_last_good` constants block and `_refresh_account_totals()` sections below and `DE-AUDIT-BL4-001` in `DECISIONS.md`.) Prior: 2026-08-05 (BL-3, HEAD `fdcb07e1`, `DE-AUDIT-BL3-001` -- `guard_alpha_summary()` gains two additive net-of-friction $-saved fields (`cumulative_saved_dollars_net_of_friction`/`saved_dollars_realized_net_of_friction`), friction subtracted at the percentage level per entry via a lazily-imported `autotuner.SIM_EXIT_FRICTION_PCT` (single source of truth, never re-defined locally); closes audit #118 Finding M1 -- see the updated `GET /api/guard-alpha-summary` section below and `DE-AUDIT-BL3-001` in `DECISIONS.md`.) Prior: 2026-07-29 (guard-alpha-saved-coherence, `DE-GAS-COHERENCE-001` -- `guard_alpha_summary()` gains an optional `?window=<token>` query param (the SAME token vocabulary `/api/strip/<window>` accepts, resolved via the SAME `analytics._window_cutoff_date`), making the dashboard's $-saved panel byte-comparable to History's own `get_history_summary` total at any shared token -- see the updated route section below.) Prior: 2026-07-25 (hygiene cycle, `DE-HYGIENE-R1-001` -- `GET /api/guard-alpha-preconditions` gains a structural phantom-key exclusion filter (`_is_symphony_state_entry`), closing a gap where 5 non-symphony `bot_state` metadata keys rendered as phantom `INSUFFICIENT_DATA` "symphony" rows (16 served on the droplet vs. 11 real); see the new Phantom-key exclusion subsection below. `guard_preconditions.py` carries zero diff.) Prior: 2026-07-25 (strategy-incubation-gate, `DE-INCUBATION-GATE-001` -- new read-only `GET /api/incubation` route + `_incubation_badge()` helper; `run_scheduler()` gains a 03:30 daily incubation-tick slot (`_run_incubation_tick`/`_incubation_tick_worker`); `ai_advisor_tab()` additively live-joins each Strategy Builder survivor's persisted `candidate_hash` against the incubation ledger -- see the new sections below. Prior: 2026-07-24 (exit-friction-realized-savings, `DE-EXIT-FRICTION-REALIZED-001` -- `GET /api/guard-alpha-summary` extended additively with `saved_dollars_realized`/`realized_coverage` (dual-basis $-saved); new `GET /api/exit-turnover` read-only route; see the updated/new sections below. Prior: 2026-07-24 (guard-alpha-preconditions, live-gate correction R2, `DE-GUARD-ALPHA-PRECONDITIONS-001` -- the `GET /api/guard-alpha-preconditions` route section's cache-hit-only description corrected to reflect the underlying bounded backward-scan probe (was exact-today-only); see the updated section below). Prior: 2026-07-23 (guard-alpha-preconditions, `DE-GUARD-ALPHA-PRECONDITIONS-001` -- new `GET /api/guard-alpha-preconditions` route (`guard_alpha_preconditions()`), the Kaminski & Lo 2014 stop-justification precondition read; see the new route section below and `docs/generated/guard_preconditions.md`). Prior: 2026-07-21 (fix-ops-cluster, `DE-OPS-CLUSTER-001` -- FINAL confidence-program cycle, ops/observability cluster + residuals: F-1 (`/api/state` ~157-connects/poll fixed via a `conn=` kwarg threaded through 11 `analytics.py` functions, both the live and closed_frozen/pre_market branches of `get_state()`), F-005 (new `GET /health` unauthenticated liveness probe), F-010 (compact single-line Composer read-timeout logging, replacing ~958 full tracebacks/month), F-030 (`propose_strategies` gains `invocation_source` advisory-DB write attribution), F-003 residual (panic-stop malformed-entry extraction moved inside the per-symphony try, closing the queue-abort gap `DE-PANIC-STOP-CONFIRM-001` left open), and the suggest-hash BACKLOG item (`/ai-advisor/suggest`'s `composer_symphony_id` now hash-resolves on both hash and name input). See `DE-OPS-CLUSTER-001` in `DECISIONS.md`. Prior: 2026-07-21 (fix-display-cluster, `DE-DISPLAY-TRUTH-001` F-016/F-026 -- `_compute_portfolio_strip` hist_source fix, new `_safe_analytics`/`_tc_cr_mdd_floats` sections). Prior: 2026-07-20 (fix-f023-perf-view, `DE-PERFVIEW-ID-MISMATCH`, F-023 -- `GET /api/performance/symphonies` (`api_performance_symphonies()`, app.py:3383) rebuilt to source `{id, name}` pairs from `database.load_state()` (bot_state, keyed by the Composer hash) instead of a post-mortem-history-derived list of bare display NAMES; the old shape used the same NAME as both picker label and picker value, and that name was then sent as `symphony_id` into the hash-keyed `shadow_history` query -- matching zero rows for every one of the 11 live symphonies, silently disguised as the generic "Insufficient history" empty state (root cause, not a data gap -- 7,330+ real rows existed under the correct hash). `GET /api/performance?scope=symphony` gains a new `symphony_id_recognized` boolean (AC-4, scope=symphony only, never emitted on scope=aggregate) distinguishing a genuine no-data hash from a totally unrecognized id; `static/performance.js`'s `renderBanner()` and `static/ai_advisor.js`'s `loadSymphonies()` consume the new shape -- see `docs/generated/static_performance_js.md` (new) and `docs/generated/static_ai_advisor_js.md`); prior: 2026-07-20 (fix-f003-panic-stop, `DE-PANIC-STOP-CONFIRM-001` — `perform_account_liquidation()`'s per-symphony sell confirmation restructured: a status-code gate (200/201/202=success, else a `LIQUIDATION FAILED` line) replaces the prior unconditional "Liquidated" print, each symphony's sell attempt is isolated in its own try/except so one fault no longer aborts the remaining queue, and the function now returns a structured per-symphony outcome dict — log/isolation/return-value only, zero trade-behavior change (AC-5); no dashboard consumer yet, deferred); prior: 2026-07-20 (fix-f008-data-integrity, `DE-POSTMORTEM-INTEGRITY-001` -- `guard_alpha_summary()` gains a read-time provenance validity guard (routes through `analytics.is_valid_post_mortem_entry`), excluding post-mortem trigger entries lacking a recognized `if_held_source` stamp from the $-saved aggregate; new `excluded_invalid_count` response field; `/api/history/<int:days>`'s `get_history_summary` path gains the SAME guard (AC-5b) -- see `docs/generated/analytics.md`); prior: 2026-07-17 (math-r0, `DE-MATH-R0-001` -- AC-3..8/AC-8b Performance/History/strip render-truth fixes, closing `DE-MATH-AUDIT-001` MA-6/MA-7/MAPERF-03/04/05/06/13); prior: 2026-07-14 (branch-integration merge of `feature/frontrunner-builder` onto the R2 program. Frontrunner Builder wave-2 — DE-FRONTRUNNER-002: 4 new Frontrunner Builder AI Advisor routes — `POST /ai-advisor/frontrunner-builder/run` [async 202 dispatch via a dedicated executor], `POST /ai-advisor/proposal/approve`, `POST /ai-advisor/proposal/reject`, `GET /ai-advisor/frontrunner-builder` redirect stub — plus `ai_advisor_tab()` prefetches pending `frontrunner_proposals` with `candidate_tree` bounded to a 4000-char preview. R2-3: `POST /ai-advisor/asset-swaps/evaluate` gains a route-minted default `provenance` object present on EVERY return path (same stricter shape R2-2 established) plus the engine's own `provenance` on success, reusing `ai_advisor.build_reasoning_context` verbatim; the route also gains an objective-only REASONED mode (both tickers omitted) alongside the byte-preserved EXPLICIT-PAIR mode — see the route section below; `DE-ADVISOR-R2-3-001`, closes the R2 program 3-of-3); prior: R2-2: `POST /ai-advisor/logic-changes/evaluate` gains a route-minted default `provenance` object present on EVERY return path (stricter than R2-1's SB route, which only carries it on success) plus the engine's own `provenance` on success, reusing `ai_advisor.build_reasoning_context` verbatim — see the route section below; `DE-ADVISOR-R2-2-001`); prior: 2026-07-13 R2-1: `POST /ai-advisor/strategy-builder/run` gains a run-level `provenance` object (4-key: `generation_model`/`mode`/`evidence_injected`/`run_id`) for symphony-scoped runs, plus a route-boundary `isinstance(dict)` serialization guard against bare-`MagicMock` `ProposalRun` stand-ins — see the route section below; `DE-ADVISOR-R2-1-001`); prior: advisor-remediation-r1, `DE-ADVISOR-R1-001`: the three operator Evaluate routes — asset-swaps/evaluate, logic-changes/evaluate, strategy-builder/run — and `GET /api/candidate-alert` gained honesty/transparency response-JSON fields; see the route sections below); prior: 2026-07-12 (candidate-alert cycle: new `GET /api/candidate-alert` + `POST /api/candidate-alert/mark-viewed` back the header candidate-alert indicator — see `feature-plans/candidate-alert.md`; prior: 2026-07-09 DE-PROD-ACCURACY-001: `/api/history/<days>` fallback now date-filters to the current ET trading day and never clobbers the windowed `trigger_count`; `/api/performance` scope=aggregate now serves the canonical value-weighted `shadow_history` series with corrected `live_returns`/`shadow_returns` field semantics; `/api/guard-alpha-summary` basis_label gains freshness stamping; prior: 2026-07-02 DE-PRISM-NUMERIC-VERIFY-001, `ai_advisor_tab()` additively fetches the MARKET_PRISM_VERIFICATION row and attaches per-check fact-check badges to the Overview render; 2026-07-02 DE-EOD-BASIS-001, EOD/frozen account-basis unification + per-field stale-cache hardening; 2026-06-30 DE-PRISM-SOURCES-PER-LENS-001, per-lens Market Prism sources carousels; 2026-06-26 fix/today-change-account-basis, DE-TODAY-BASIS-001)
...
- `_ACCOUNT_TOTALS_HTTP_TIMEOUT_S` (DE-EOD-BASIS-001) — named constant promoting the bare `timeout=10` literal previously inline in the Composer `requests.get` call inside `_refresh_account_totals()`. **Value raised `10 -> 30` (AC-6, `DE-AUDIT-BL4-001`, 2026-08-05):** [PM-ASSUMED] ruling under the project's full-autonomy directive -- the audit itself only recommended this "optionally... to reduce stale flicker" (~21 timeouts/day observed, 0 in the most recent 24h at audit time). Stays a named constant, never inlined; **reviewer-noted tradeoff (as reported to the team lead):** this timeout gates `_refresh_account_totals()`, which runs on the app's background scheduler thread (see `_refresh_account_totals()` below), not the 1-minute engine execution/dispatch path (`alpha_bot_execution.py` subprocess spawn is unaffected) -- but a slow Composer call can now block that scheduler thread up to 30s instead of 10s, a documented cost traded for fewer stale-flicker transitions.
...
Non-blocking daily off-hours wrapper (mirrors `_run_incubation_tick`'s own two-function split): `_run_retirement_recommender_tick()` spawns a daemon thread (`name="retirement-recommender-tick"`) so the scheduler thread returns immediately, never blocking the 1-minute execution path (Architecture Constraint 1); the thread runs `_retirement_recommender_tick_worker()`, which lazily imports `advisors.retirement_recommender.build_recommendations`/`persist_recommendations` (CC-2 -- never a module-level import), calls `build_recommendations()`, runs the AC-2 explainer step (below) over the result, then `persist_recommendations(<the same list>)`, and logs the count persisted at INFO. This is the sole production caller of either function -- without it, `GET /api/retirement-recommendations` and the Overview-tab panel would have nothing to read (the gap the original cycle shipped with; closed here). D-1: any exception is caught and logged as `type(exc).__name__` only, at WARNING+ -- a producer failure can never crash the scheduler thread.

**Explainer wiring (Phase 2 Cycle 2b, AC-2, `DE-RETIRE-APPROVAL-001`).** Between `build_recommendations()` and `persist_recommendations()`, the worker lazily imports `advisors.retirement_explainer.explain_recommendation` (CC-2) and calls it once per recommendation (subject to the Cycle-2c reuse check below), stamping the result onto `rec["explanation"]` in place before the batch is persisted.

**Display-name enrichment (Cycle 2c, AC-2, `DE-RETIRE-APPROVAL-001`/`DE-RETIRE-POLISH-001`, 2026-08-27).** Before the explain loop, the worker resolves each rec's `candidate_name`/`sibling_name` from ONE `database.load_state()` call, via the same `advisors.frontrunner_builder.resolve_incumbent_display_name` helper `ai_advisor_tab()` uses for the card (AC-1) -- never a second, independent hash-to-name lookup. Both keys are set ONLY when absent (`if "candidate_name" not in rec:`) -- never clobbering a value a caller already supplied. This enrichment does two things at once: it grounds `explain_recommendation`'s prompt in readable names instead of bare Composer hashes, AND it lands those two keys in the SAME dict that flows into `persist_recommendations` -- which the AC-4 reuse check immediately below depends on, to detect a genuine rename against a prior night's persisted row. A `database.load_state()` failure degrades both name fields to the raw id (the helper's own contract) but never skips the explainer call itself.

**Nightly-explain spend control (Cycle 2c, AC-4, `DE-RETIRE-APPROVAL-001`/`DE-RETIRE-POLISH-001`, 2026-08-27).** Before calling `explain_recommendation` for a given rec, the worker calls a new module-level helper:

```python
def _retirement_find_reusable_prior_explanation(rec: dict) -> str | None:
    ...  # 9-condition material-change check, fail-open on any exception
...
for rec in recs:
    ...
    try:
        ...
        _daemon_log.warning("Retirement explainer failed for %s: %s", rec.get("candidate_id"), type(exc).__name__)
        rec["explanation"] = None
```

`_retirement_find_reusable_prior_explanation` looks up the most recent PRIOR persisted `RETIREMENT_RECOMMENDATION` row for the exact same `(candidate_id, sibling_id)` pair via `database.get_advisor_observations_for_role("RETIREMENT_RECOMMENDATION", limit=_ADVISOR_OBSERVATIONS_PAGE_LIMIT)` (newest-first by `id DESC` -- the first matching row IS the most recent prior night's), and returns its `explanation` text VERBATIM (skipping the LLM call entirely for that rec) only when ALL 9 conditions hold: a truthy prior `explanation` exists; `correlation`/`candidate_composite`/`sibling_composite` match within 2-decimal rounding; `uncertainty_gate_passed`/`structural_redundancy_gate_passed`/`basis_label` match exactly; and `candidate_name`/`sibling_name` match exactly, with EITHER side missing the key counting as a mismatch (a legacy pre-AC-2 row can't prove "no rename happened," and a genuine rename must force a fresh explanation, never reuse prose naming the wrong symphony). **Fail-open, not fail-silent-drop:** the whole lookup-plus-compare is ONE `try`/`except Exception: return None` unit -- any failure anywhere inside it (a raising accessor, a malformed/missing prior field, a `TypeError` rounding a non-numeric value) resolves identically to "no prior history," i.e. falls through to a fresh `explain_recommendation` call, never to a silently-dropped explanation. A genuinely new or materially-changed pair is unaffected: any single one of the 9 conditions failing forces a fresh explanation exactly as it did before this cycle. This is bounded, non-recurring metered spend for a persistently-flagged pair -- the common case on a stable fleet, where the same high-correlation pair can recur night after night with unchanged evidence.

This is defense-in-depth, not a contract violation expectation: `explain_recommendation` is D-1/never-raises by its own contract (see [advisors/retirement_explainer](advisors_retirement_explainer.md)), so this `try`/`except` should rarely if ever fire in practice -- but a per-rec failure here degrades only that one recommendation's `explanation` to `None` and never blocks persistence of the rest of the batch (a batch of, say, 5 recommendations where the explainer times out on 1 still persists all 5, 4 with real explanations and 1 with `explanation: null`). **This keeps the LLM entirely out of `advisors/retirement_recommender.py`** (byte-unchanged across Cycle 2b AND Cycle 2c, pinned by a golden-hash test) **and out of the render/approve/checklist paths** (operator ruling, Gate-2b) -- the explainer only ever runs here, at 03:45, once per recommendation per night (or zero times, when the AC-4 reuse check finds a valid prior explanation).

**Four independent error-handling layers, by design (not redundant) -- three log, one degrades silently.** `build_recommendations()` itself gained its own internal `except`-clause WARNING log at `f9819274` (Revise round 2, PR-level `/code-review` Finding 5) -- since that function never raises (D-1), this worker's own outer `except` around the call chain will essentially never fire FOR a `build_recommendations`-internal failure specifically (it always returns `[]` cleanly). The per-rec explainer `try`/`except` covers a THIRD, narrower surface: an `explain_recommendation` contract violation for exactly one recommendation in the batch. The worker's own outer `except` remains the catch-all for failures OUTSIDE both of those boundaries -- e.g. an exception raised by `persist_recommendations()` itself, or by either lazy import. Those three all log. The Cycle-2c reuse check's own fail-open `except` (above) is a FOURTH, narrower still, and deliberately does NOT log -- it degrades silently to "generate fresh," since falling through to the normal explainer path is itself the correct, unremarkable outcome, not a failure worth a WARNING. The four layers cover disjoint failure surfaces.

See [advisors/retirement_recommender](advisors_retirement_recommender.md), [advisors/retirement_explainer](advisors_retirement_explainer.md), `DE-RETIRE-CORE-001`, and `DE-RETIRE-APPROVAL-001` (incl. its "Cycle 2c" section) in `DECISIONS.md`.
...
Fetches Composer account-level total-stats (`portfolio_value`, `simple_return`, `todays_percent_change`, `max_drawdown`) and populates `_account_totals_cache`. Must never raise (swallows all exceptions — D-1 contract). HTTP timeout is `_ACCOUNT_TOTALS_HTTP_TIMEOUT_S` (named constant, `30` as of `DE-AUDIT-BL4-001`, was `10` under `DE-EOD-BASIS-001`). Runs on the app's background scheduler thread (the per-minute tick, the `_notify_cycle_complete`-spawned thread, and the flush-resync thread — 3 real concurrent call sites), never on the 1-minute engine execution/dispatch path.
...
**Default-window `guard_alpha`/`window` block (`DE-CLOSED-BOUNCE-001`, 2026-08-25):** until this fix, the happy-path `_portfolio_strip` built above never set `guard_alpha` or `window` at all — unlike the live path (`_compute_portfolio_strip`, below), which has always populated a default-window pair via its own `compute_windowed_portfolio_strip` call (`app.py:1859-1881`). The absence was structural, not conditional: `guard_alpha` was missing on every single closed-market poll, which made `static/index.js`'s `renderGuardAlpha` fall back to `fetchWindowedStrip('30d')` every poll — and that fallback's wrapped windowed-VW payload, passed through to `updateComparisonRows` as an unintended side effect, raced the genuine account-basis values this branch computes for the Today/Cumulative/MDD comparison rows (see `docs/generated/static_index_js.md`). Fixed by a dedicated `try`/`except` block immediately after the `account_all_time_cr` assignment — deliberately its OWN try/except, not folded into the surrounding one, so a failure here can't discard the already-computed `today_change`/`cumulative_return`/`max_drawdown`. Mirrors the live path's block: calls `analytics.compute_windowed_portfolio_strip(_snap_symphonies_list, _snap_bot_state, window=_DEFAULT_HERO_WINDOW, conn=_frozen_shadow_conn)`, and on a dict result sets `guard_alpha` (float or `None`) and `window` (string, defaulting to `_DEFAULT_HERO_WINDOW` on a non-string result). `guard_alpha` can still legitimately resolve `None` (e.g. `weight_sum==0`, or this block's own exception, logged as `"closed_frozen default-window strip failed"`) — the client-side fallback remains necessary for that genuine edge case; it's just no longer the every-poll default. See `DE-CLOSED-BOUNCE-001` in `DECISIONS.md`.

**Revise round, `windowed_cumulative_return` dropped from this branch (PR #136 `/code-review` finding #2, 2026-08-25):** the original cycle's version of this block also set `windowed_cumulative_return` on `_portfolio_strip`, mirroring the live path's own field, for response-shape parity. Review finding #2 removed it: `static/index.js`'s `updateComparisonRows` has never read `windowed_cumulative_return` since F-014 (`DE-DISPLAY-TRUTH-001`), and finding #1's client-side fix means no comparison row is ever sourced from a windowed-strip payload at all any more (see `docs/generated/static_index_js.md`) — so the field would have been pure write-only weight on every closed-market poll response, with no shape-parity justification strong enough to keep it. **Asymmetric with the live path on purpose:** `_compute_portfolio_strip`'s own `windowed_cumulative_return` assignment (`app.py:1877-1879`) is UNCHANGED code-wise — it is genuinely dead there too (same F-014 history: it's a field on the `/api/state` `portfolio_strip`, distinct from `/api/strip/<window>`'s own response, which is `compute_windowed_portfolio_strip`'s raw 8-key dict and has never carried a `windowed_cumulative_return` key at all). It was pre-existing before this whole feature and finding #2 scoped its removal to the closed_frozen branch's NEW block only, not to this pre-existing live-path line — the commit message frames the live path's field as kept "for `/api/strip/<window>` response-shape parity"; only its comment is corrected here to state plainly that it has no client-side consumer. Full removal of the live-path field (if ever warranted) is out of this cycle's scope.

**Held-basis convergence, frozen-branch parity (Revise 1, PR #125 review F5):** three fixes bring this branch's held-basis handling in line with its live twin (`_compute_portfolio_strip()`, above). **F5(b):** `_snap_bot_state` — the frozen Tier-2 aggregate's hand-built 4-key dict (`current_value`/`current_return`/`name`/`account`) — gains an explicit `"current_return_is_reconstructed": _sym_r.get("current_return_is_reconstructed", False)` field; this is the ONE site the F4 `bot_state_entry`-primary pivot (see `docs/generated/analytics.md`'s `get_symphony_today_change` section) does not cover for free, since `_snap_bot_state` is not the real `bot_state` entry. **F5(a):** the frozen branch's `get_portfolio_today_change(...)` call (`_snap_vw_tc`) now passes `include_paired_guard_delta=True`, matching the live path's F6 opt-in requirement — and the frozen Tier-2 floor fallback (previously `_snap_tc_final = _snap_vw_tc`, a raw unwrapped passthrough with no account-basis conversion and no paired re-derivation) now re-derives `dry_run` from `if_held + guard_delta_vw` the SAME way `_compute_portfolio_strip`'s own Tier-2 floor does — `if_held` stays full-membership (AC-6). Before this fix, the frozen Tier-2 floor's mismatched-membership bug was WORSE than the live branch's pre-Revise-1 bug: on the reviewer's 2-of-3-coverage worked example (see `docs/generated/analytics.md`'s `_value_weighted_portfolio` section), it rendered a sign-flipped `-0.3333pp` rather than merely an overstated value. **[F6 follow-up, `DE-HELD-BASIS-001`, 2026-08-25]** the frozen Tier-2 floor's ELSE branch (zero paired coverage, `guard_delta_vw is None`) still assigned `_snap_tc_final = _snap_vw_tc` verbatim until this follow-up — the exact same internal-key leak the live path's else branch had, on this branch's own separate code path (`app.py:2412`). Fixed identically: `_snap_tc_final = {"if_held": _snap_vw_tc.get("if_held"), "dry_run": _snap_vw_tc.get("dry_run")}`. See `DECISIONS.md`'s "F6 follow-up" addendum for the paired live-path site and the reviewer's completeness proof that these two Tier-2-floor else branches are the full leak surface. **F5(c):** the frozen per-symphony card-render loop needed no code change — its loop variable already IS the real snapshot entry, so F4's `bot_state_entry`-primary marker resolution covers it "for free," the same way it covers the dashboard SSR card (F3) and `/api/strip` (F1). **F5(d)** — the "structurally immune" claim this branch's design was originally built on is corrected in `docs/generated/analytics.md`'s `get_symphony_today_change` section; this doc pass's own summary: a real (if narrow) EOD Composer-fetch-failure gap exists, closed at the render layer by F4/F5(a-c) honestly surfacing whatever marker value a snapshot actually carries, with zero diff to `alpha_bot_execution.py` (AC-7 held). See `DECISIONS.md`'s "Revise 1" section for the full account.
...
**BL-8 "never adopted" streak signal (`DE-AUDIT-BL5-12-001`, audit #118 Finding T3, 2026-08-05):** each row additively gains a `never_adopted_streak` field -- `analytics.compute_never_adopted_streak` (see `docs/generated/analytics.md`) computed by grouping the SAME already-fetched rows by `symphony_id` (zero extra DB round-trip), from the RAW pre-normalization rows (so the `"Adopted AI"` comparison always matches `autotuner.py`'s real persisted literal, independent of `_normalize_autotune_row`'s short-token remap -- a dedicated regression test proves this). Response stays a bare JSON ARRAY -- AC-3's existing consumer contract (`tests/reporting/test_dsr_surfacing.py::TestAutotuneRunsApiRoute` pins `isinstance(data, list)`), never wrapped in an envelope object. Rendered by `static/ai_advisor.js`'s `loadRecentRuns()` as a dim one-liner per card -- see `docs/generated/static_ai_advisor_js.md`.

#### `GET /api/advisor-observations`
Returns `advisor_observations` rows for a symphony. Accepts `?symphony_id=` query parameter.

**BL-7 stale-comment correction (`api_advisor_observations()`, `DE-AUDIT-BL5-12-001`, audit #118 Finding D6, 2026-08-05):** the comment above this route's `symphony_id`-branch AC-14 filter previously claimed DIVERGENCE_EXPLAINER "is permanently rejected but still writes one per autotune run" -- false since AC-14 (`advisors/divergence_explainer.py`'s `run_divergence_explainer` writes NOTHING, returns `None`, when `SECOND_WINDOW_CVAR_ENABLED` is off). Corrected to state the producer writes nothing today, and that the filter immediately below is legacy-row DEFENSE for the 22 pre-AC-14 `NOT_APPLICABLE` rows still in the DB, not ongoing-write suppression. Comment-only -- zero change to the filter logic itself (AC-8).
...
**`_fetch_retirement_recommendations(limit=None) -> list[dict]`** -- shared helper, called by BOTH this route AND the `ai_advisor_tab()` Overview panel prefetch below (one fetch+flatten implementation, never duplicated). Reads `database.get_advisor_observations_for_role("RETIREMENT_RECOMMENDATION", limit=...)` and returns each row's `raw_response` dict (the authoritative schema documented in [advisors/retirement_recommender](advisors_retirement_recommender.md) (its "`raw_response` shape" section) -- no renaming/translation layer for the persisted fields themselves). `limit` defaults to the existing `_ADVISOR_OBSERVATIONS_PAGE_LIMIT` module constant, resolved lazily inside the function body rather than as a parameter default (that constant is defined later in the file; a def-time default would raise `NameError` at import). This helper itself RAISES on a DB-read failure -- each caller degrades to its own honest empty result independently (route: `{"recommendations": []}`; panel: template's empty-state), the same per-route error-ownership convention `api_incubation` already uses.

**`approval_status` live-join, now INSIDE this helper (Cycle 2c, AC-6, `DE-RETIRE-APPROVAL-001`/`DE-RETIRE-POLISH-001`, 2026-08-27).** Before returning, `_fetch_retirement_recommendations` calls a new shared helper, `_join_retirement_approval_status(out)`, which stamps `approval_status` onto every returned dict IN PLACE, read FRESH from the mutable `retirement_decisions` table on every call (`database.get_retirement_decisions()`, never cached) -- an honest `"pending"` default when no decision row exists or the decisions read itself fails (`decisions_by_id = {}` on any exception, never raises out of this helper). **This is a relocation, not new logic:** Cycle 2b's live-join used to run ONLY inside `ai_advisor_tab()`'s render-time code, after calling this same shared helper -- meaning `GET /api/retirement-recommendations` (this route) never carried `approval_status` at all, a gap the FIRST `/code-review` pass's F2 finding flagged and the PM ruled a fast-follow (see `DE-RETIRE-APPROVAL-001`'s Revise-round section in `DECISIONS.md`). Moving the join inside the shared helper closes that gap for both consumers at once, from one call site.
...
**Response shape:** `{"recommendations": [<raw_response dict, flattened to top level, plus a live-joined "approval_status" key (Cycle 2c, AC-6)>, ...]}`.
...
| `correlation_matrix` | `correlation_diagnostic.compute_pairwise_correlations` over analytics history — **`DE-CORR-DATE-ALIGN-001` (2026-08-25):** per-symphony series are assembled as a date-keyed `dict[str, float]` (`dict(zip(_dates, _live_rets))`, `analytics.compute_per_symphony_returns`'s parallel date list), not a positional `list[float]`, so pairwise alignment is by shared calendar date, not raw list index — previously the sparse, event-driven trigger-day series for two symphonies (unrelated calendars) were index-aligned, correlating unrelated dates; `correlation_diagnostic.py` itself is unchanged, it already supported date-keyed dict input. **Known follow-up, not fixed here:** even date-aligned, the input is exit-trigger-day returns (only days a symphony recorded a trigger — a sparse, selection-biased sample), not a daily-NAV series; see `DECISIONS.md` `DE-CORR-DATE-ALIGN-001` for the Phase-2 basis caveat. | Correlations |
...
**Proposal-identity render prefetch (`DE-FR-PROPOSAL-IDENTITY-001`, 2026-08-20; Revise 2 same day).** Additive to the same `frontrunner_proposals` loop, per-row, after the existing `candidate_tree_preview` bounding:

- **`_bounded_json_preview(value, max_chars) -> str` helper (F9, Revise 2).** A new module-level function DRY-extracts the JSON-dump-and-truncate transform previously duplicated inline for both `candidate_tree_preview` and `overlay_tree_preview` — `None` → `""`, a serialization failure falls back to `str(value)`, and the result is truncated with a `"... truncated (N total chars)"` marker when it exceeds `max_chars`. Never raises. Both preview fields now call this one function with their own bound (`_FR_TREE_PREVIEW_MAX_CHARS=4000` / `_FR_OVERLAY_PREVIEW_MAX_CHARS=4000`, still two distinct named constants, not reused from each other).
- **Incumbent display-name resolution (F5/F8, Revise 2).** `resolve_incumbent_display_name`/`OVERLAY_NOT_RECORDED_TEXT` are lazily imported from `advisors.frontrunner_builder` (CC-2, inside their own dedicated `try`/`except` — an import failure sets a `None` sentinel and the per-row loop falls back to the raw-hash identity for every row, rather than the whole surrounding block being skipped). `database.load_state()` also moved into its own dedicated `try`/`except` (was previously unguarded inside the loop's own outer `try`) — a load failure now degrades to an empty `{}` `bot_state` (every row falls back to its raw hash) instead of raising past the loop-body code that pops `candidate_tree` off each row for template safety; the call itself is skipped entirely when `frontrunner_proposals` is empty (zero rows to resolve, no reason to spend the I/O). Per row, `resolve_incumbent_display_name(bot_state, symphony_id)` is now the SAME shared function `advisors.frontrunner_builder.approve_frontrunner_proposal` calls — closing an earlier honesty-drift risk where the two surfaces (Composer-upload identity vs. dashboard-card identity) each hand-rolled their own copy of the hash→name lookup.
- **`metrics_json` non-dict normalization (AC-6).** Before the template ever indexes `m.xxx`, a non-`dict` `metrics_json` (a malformed/legacy row, or a parsed-JSON list) is normalized to `{}`. This closes a real crash the original cycle found and fixed at the render layer (not merely a defensive guard) — see the "Mid-cycle regression" note in [advisors_frontrunner_builder.md](advisors_frontrunner_builder.md)'s Testing section: several stats-table cells used `is not none` on values that become Jinja `Undefined` (not literal `None`) once `metrics_json` degrades to `{}`, which then crashes Jinja's `format()` filter. The normalization here and the template's `is defined and` guards are two halves of the same AC-6 fix — the normalization alone was insufficient without the template-side guard.
- **`overlay_tree_preview`.** Now built via the shared `_bounded_json_preview` helper against `metrics_json.get("overlay_tree")` — but unlike `candidate_tree_preview`, `overlay_tree` is NOT popped off `metrics_json`, since the template still needs `m.replaced_node_id`/`m.overlay_summary` reachable from the same dict. A row predating this cycle (no `overlay_tree` key) renders an empty string; the template's own AC-6 fallback text is what the operator actually sees, not a blank preview block.
- **`overlay_not_recorded_text` template context variable (F10, Revise 2).** Threaded into `render_template(...)`'s kwargs, sourced from the same `advisors.frontrunner_builder.OVERLAY_NOT_RECORDED_TEXT` import above (module-level default `"overlay not recorded for this proposal"` if the import itself fails, so the name is never unbound at render time) — the template's overlay-summary fallback (`templates/ai_advisor.html`'s `fr-overlay-summary` block) now renders `{{ overlay_not_recorded_text | e }}` instead of a second, independently-hardcoded copy of the same literal string.

The whole per-row block stays inside the pre-existing `try/except` around the `frontrunner_proposals` loop — any failure degrades to `frontrunner_proposals = []` and the template's existing empty-state, never a 500. See [advisors_frontrunner_builder.md](advisors_frontrunner_builder.md)'s "Proposal Identity & Provenance" section (including its "Revise 2" subsection) for the pure helpers and persist-site wiring this render layer consumes, and `DE-FR-PROPOSAL-IDENTITY-001` in `DECISIONS.md` for the full cycle record.
...
**Panel placement (`templates/ai_advisor.html`).** A `<section class="matrix-card" data-testid="retirement-recommendations-panel">` sits at the **end of the Overview tab** (`tab-panel-overview`), after the existing correlations/Market-Prism blocks and immediately before that tab panel's closing `</div>`. Per recommendation card: **[Cycle 2c, AC-1]** the resolved candidate DISPLAY NAME → "kept: {resolved sibling display name}" (each with the raw Composer hash preserved as a `title=` tooltip, honest hash fallback when unresolvable — was bare candidate id → "kept: {sibling id}" through Cycle 2b), correlation (2dp or `—`), the composite gap (`sibling_composite - candidate_composite`, 2dp or `—` if either is `None`), both gate verdicts ("passed"/"not met"), the `basis_label` disclosure line in a small italic footer — **plus, since Cycle 2b (below), the explanation/status/actions/checklist block.** All fields render through Jinja `.get(...)` with `| e` escaping — no `| safe` anywhere in this block, matching this file's existing convention. Styling reuses existing dashboard CSS-token variables (`var(--studio-border)`, `var(--studio-ink)`, `var(--studio-ink-dim)`, plus new Cycle-2b tokens for the status chip/actions/checklist) — no raw hex, no inline colors, consistent with the Market-Prism/correlations panels' precedent.
...
**[CORRECTED, Phase 2 Cycle 2b, `DE-RETIRE-APPROVAL-001`, 2026-08-26] the sentence below is now STALE — preserved verbatim as historical context for what Cycle 2a actually shipped, not as current behavior.** *"Purely read-only, server-rendered, NO operator affordance of any kind (no approve/reject/dismiss control — that lifecycle is Cycle 2b)."* This was true of the Cycle-2a shipped panel. Cycle 2b (documented immediately below — the explainer, the live-join, the two new approve/reject routes, and the render-time checklist assembly) adds exactly that lifecycle. The panel is no longer a pure read-only surface: each card now carries a real operator affordance (Approve/Reject), gated by, and only by, a status-only DB write (`_dispatch_retirement_decision`) that structurally cannot reach any trade/exec primitive — see the "Retirement Approval Lifecycle" subsections below.

---

### Retirement Approval Lifecycle — explainer output, display names, live-join, and checklist assembly in `ai_advisor_tab()` (Phase 2 Cycle 2b AC-2/AC-4/AC-6/AC-7 + Cycle 2c AC-1/AC-6, `DE-RETIRE-APPROVAL-001`, 2026-08-27)

Pieces layered onto the Cycle-2a prefetch above, all inside the SAME `if retirement_recommendations:` guard so none of this work runs on an empty batch.

**1. Explanation (AC-2, read-only).** No extra code here — `rec["explanation"]` is already present on each `RETIREMENT_RECOMMENDATION` `raw_response` dict, stamped by the 03:45 producer (see `_retirement_recommender_tick_worker()` above and [advisors/retirement_explainer](advisors_retirement_explainer.md)). The route/panel prefetch reads it verbatim off the persisted row like any other `raw_response` key; a `None` value (explainer failure or a pre-2b historical row) renders the template's "explanation unavailable" fallback.

**2. `approval_status` (Cycle 2b's own AC-4 -- NOT Cycle 2c's differently-numbered AC-4, see the tick-worker section above for that one) — read here, not joined here (Cycle 2c's AC-6 relocation).** As of Cycle 2c, `approval_status` is already present on every `retirement_recommendations` entry by the time execution reaches this function — `_fetch_retirement_recommendations()` itself now performs the live-join (via `_join_retirement_approval_status`, see the `GET /api/retirement-recommendations` section above), so both this panel AND the JSON API route see identical, always-fresh decision state from ONE shared call site. Through Cycle 2b, this live-join ran HERE instead, reading `database.get_retirement_decisions()` directly inside `ai_advisor_tab()` — that code is gone; the section below shows what replaced it.

**3. Display-name resolution (Cycle 2c, AC-1).** Unconditional on `approval_status` (a pending card must also show the resolved name, not just an approved one — unlike the pre-2c `database.load_state()` fetch, which only ran for an approved batch):
...
for _rec in retirement_recommendations:
    if _ret_resolve_display_name is not None:
        _rec["_candidate_name"] = _ret_resolve_display_name(_ret_bot_state, _rec.get("candidate_id"))
        _rec["_sibling_name"] = _ret_resolve_display_name(_ret_bot_state, _rec.get("sibling_id"))
    else:
...
`_ret_resolve_display_name` is `advisors.frontrunner_builder.resolve_incumbent_display_name` — the SAME shared hash-to-name helper `DE-FR-PROPOSAL-IDENTITY-001` introduced for the Frontrunner Builder panel, reused verbatim here rather than a second independent lookup. Its own contract (see that module) already returns the id itself when unresolvable, so `_rec["_candidate_name"]`/`_rec["_sibling_name"]` degrade to the raw id honestly on a `bot_state` miss or an import failure. `_ret_bot_state` is loaded exactly ONCE per request here — the checklist-assembly step below (step 4) REUSES it, rather than calling `database.load_state()` a second time on an approved batch, closing a small pre-2c inefficiency (the old code fetched `bot_state` a second time inside the `if _ret_any_approved:` block).

**[CORRECTED, PR #139 review remediation, F6, `5655edfd`, 2026-08-26 — still true post-Cycle-2c]** neither this function nor the live-join it now reads from stamps a `_rec["_decision"]` key — a Cycle-2b GREEN-commit dead-weight field, removed the same cycle it shipped (see `DE-RETIRE-APPROVAL-001`'s Revise-round section in `DECISIONS.md`), and it did not reappear in the Cycle-2c relocation.

**4. Checklist assembly (AC-6/AC-7 glue, Cycle 2b; REUSES the Cycle-2c `_ret_bot_state` above).** Built ONLY for approved cards, and ONLY when at least one card in the batch is approved (`_ret_any_approved` short-circuit, computed in step 3 above):
...
**Deliberately NOT built inside the approve route.** AC-5 requires "Approve writes status ONLY" — `_dispatch_retirement_decision` never imports or calls `retirement_checklist.build_checklist`. The checklist is assembled here, at render time, on every subsequent page load of an approved card, so it always reflects LIVE current holdings (not a snapshot frozen at the moment of approval). A `build_checklist` import failure or call failure degrades `_rec["_checklist"]` to `None`; the template then renders "Checklist unavailable." (`.prism-empty` styling) rather than a 500 of the whole page. **As of Cycle 2c, AC-3**, the template also renders `build_checklist`'s own `candidate_name` field inside this checklist block (see [advisors/retirement_checklist](advisors_retirement_checklist.md)'s "Consumed by the template" note) — through Cycle 2b that field was computed and returned but never read by any template.

---

### `POST /ai-advisor/retirement/approve` / `POST /ai-advisor/retirement/reject` — `ai_advisor_retirement_approve()` / `ai_advisor_retirement_reject()` (Phase 2 Cycle 2b, AC-5, `DE-RETIRE-APPROVAL-001`, 2026-08-26)

Two thin one-line route handlers, each delegating to the shared `_dispatch_retirement_decision(approval_status)` helper:
...
**Deliberately NOT the frontrunner `/ai-advisor/proposal/approve` route** — that route reaches `composer_draft_client.save_symphony` (creates a real Composer symphony). This retirement pair is a SEPARATE route, status-write only, reaching no Composer/exec/`LIVE_EXECUTION`/trade primitive of any kind — the direction is the opposite of the frontrunner route (retiring a symphony, not creating one), so overloading the frontrunner route would have been actively wrong, not just inconsistent.

#### `_dispatch_retirement_decision(approval_status: str)`

The shared body both routes delegate to (never edit the two route handlers' logic directly — this is the one place the real behavior lives):

1. Parses `{"candidate_id": <str>}` from the JSON body (`request.get_json(silent=True) or {}`).
2. Validates `candidate_id` is a non-empty `str` under 300 characters (matching Composer-hash-scale ids) — on failure, returns `{"success": False, "approval_status": None, "error": "invalid candidate_id"}` with HTTP 200 (never a 4xx — this route's error contract is always-200-with-a-body, matching the rest of this codebase's advisor-route convention).
3. Calls `database.upsert_retirement_decision(candidate_id, approval_status=approval_status)`.
4. On success, returns `{"success": True, "approval_status": approval_status, "error": None}`.
5. On any `Exception` from the accessor call, logs the real exception at ERROR (`exc_info=True`, server-side only) but returns only `{"success": False, "approval_status": None, "error": type(exc).__name__}` to the client — the D-1 security contract: `str(exc)` is never echoed back over HTTP, since it could carry a DB file path or other internal detail.

**Deliberately NOT gated against the current latest recommendation batch.** `candidate_id` is validated for shape (non-empty, bounded length) but not cross-checked against `_fetch_retirement_recommendations()`'s current result — a team-lead-approved deviation from the feature plan's "where practical" wording. Rationale: the nightly 03:45 tick can rebuild the ledger between an operator's page load and their click, and this is a status write on an advisory record (not a money-moving action) — an orphaned decision row for a candidate that later drops out of the batch is harmless (see the live-join note above and the feature plan's Edge Cases).

**Never calls the checklist or the explainer.** `_dispatch_retirement_decision` never imports or calls `advisors.retirement_checklist.build_checklist` (assembled at render time only, see above) or `advisors.retirement_explainer.explain_recommendation` (producer-time only, see `_retirement_recommender_tick_worker()` above). This is structurally proven, not merely asserted by the docstring — `tests/security/test_retirement_action_no_trade_boundary.py`'s Group C performs a **transitive AST call-graph walk** starting from each route handler's FunctionDef, following bare-Name calls to other `app.py` module-level functions (bounded, visited-set-guarded against recursion), and asserts neither the LLM seam (`_build_client`) nor the explainer entrypoint (`explain_recommendation`) nor `composer_draft_client` nor `alpha_bot_execution` nor `LIVE_EXECUTION` appears anywhere in that walk. This matters concretely: both route handlers are ONE-LINE wrappers — the entire real body (candidate_id validation, the DB write) lives in `_dispatch_retirement_decision`, which a single-hop AST scan scoped to only the named route function would never see. A fix landed same-cycle (`04c483ca`, a `ret2-review` finding) replacing an original single-hop scan with the real transitive walk described above, closing exactly that gap — see `DE-RETIRE-APPROVAL-001`'s safety-boundary section in `DECISIONS.md` for the full account.

**Cycle 2c extends the same file's coverage to the 4 new call paths this cycle added (`01521618`, `ret3-review` finding (b), `DE-RETIRE-APPROVAL-001`/`DE-RETIRE-POLISH-001`, 2026-08-27) — Group E and Group F, distinct from Group C above.** Group C (above) proves only the two approve/reject routes are exec-seam-free; it never covered `_retirement_recommender_tick_worker`, `_retirement_find_reusable_prior_explanation`, `_join_retirement_approval_status`, or `ai_advisor_tab()`'s own retirement name-resolution block — all 4 new this cycle. **Group E** extends Group C's SAME transitive-AST-call-graph machinery (no new helpers) to the 3 new `app.py` helper functions, parametrized, checking `composer_draft_client`/`save_symphony`/`alpha_bot_execution`/`LIVE_EXECUTION` — deliberately WITHOUT the LLM-seam check Group C applies to the routes, since `_retirement_recommender_tick_worker` is SUPPOSED to reach `explain_recommendation` (AC-2/AC-4's entire job, a nightly producer call, not an operator action). **Group F** covers `ai_advisor_tab()`'s retirement block via a scoped source-window TEXT scan instead — an earlier attempt to fold it into Group E's transitive walk was tried first and empirically failed (a false positive via `_build_meta`, an unrelated shared page-header helper that legitimately reads `LIVE_EXECUTION` for a read-only "live mode" badge); the scoped window (bounded by the retirement panel's own opening comment marker and the Frontrunner Builder panel's, both loudly `pytest.fail`-guarded if either marker moves) checks the same 4 exec-seam strings. `ret3-review` mutation-tested both groups — injecting a literal `save_symphony(...)` call into one Group E target and, separately, the Group F window, one at a time, confirmed BOTH scans fail as expected, then reverted. 57/57 in the file post-extension (independently re-verified). See `DE-RETIRE-APPROVAL-001`'s "Safety-guard extension" subsection in `DECISIONS.md` for the full account, including this doc-writer's own missed-commit correction.

CSRF via the global `_csrf_before_request` hook, auth via the global `_auth_before_request` hook — neither route opts out. Neither route path (`/ai-advisor/retirement/approve`, `/ai-advisor/retirement/reject`) is in `_SETTINGS_WRITE_ALLOWLIST`.
...
See `DE-EOD-BASIS-001` in `DECISIONS.md` for the frozen-path mirror (`get_state()`, documented under `GET /api/state` above) and the 5 findings this hardening pass fixed. **Render consumers added (`DE-AUDIT-BL4-001`, 2026-08-05):** these three fields had zero frontend consumers prior to this cycle (confirmed by grep against `static/index.js`'s pre-fix `updateComparisonRows` body) -- `static/index.js`'s new `renderAccountBasisChip()`/`renderAccountBasisFreshness()` helpers now surface them as a stale/value-weighted-floor chip on the Today/Cumulative comparison rows plus a dedicated freshness stamp; this Python-side computation is pinned byte-unchanged (AC-5, `TestBackendComputationByteUnchanged`) -- see `docs/generated/static_index_js.md`.

**Held-basis convergence — [SUPERSEDED, Revise 1, PR #125 review F4] the 2-site marker-threading design below is REMOVED, not current.** The original cycle's `symphonies_list` sym_dict this function builds from `bot_state` (`app.py:~1508-1514`) carried an additive `"current_return_is_reconstructed"` field, threading BL-9's basket-reconstruction marker through to `analytics.get_symphony_today_change` per-caller. The `/review` skill gate on PR #125 found this left 4 of 6 real `sym_dict`-building call sites unthreaded (F1: `/api/strip`, F3: dashboard SSR card, F5(c): frozen per-symphony loop) and the accompanying claim that the frozen/closed-market snapshot branch was "structurally immune" to the defect was FALSE as stated (F5(d) — a real Composer-fetch-failure-at-EOD gap exists; see `docs/generated/analytics.md`'s `get_symphony_today_change` section for the full corrected account). **Current mechanism (F4):** `analytics.get_symphony_today_change` now reads the marker from its `bot_state_entry` parameter as PRIMARY (a key-presence check) — every real caller already passes the true `bot_state` sub-dict there, so this per-symphony threading in `_compute_portfolio_strip`'s `sym_dict` build is REDUNDANT and was removed; the same removal applies to `get_state()`'s live-poll card builder (documented under State Helpers below). The ONE site that still needs explicit threading is the frozen branch's hand-built `_snap_bot_state` (see the Frozen/EOD snapshot section below, F5(b)) — it is not the real `bot_state` entry F4's precedence check can read directly.

**F6 — `include_paired_guard_delta` opt-in, 3 sites in this function.** `analytics.get_portfolio_today_change`'s `include_paired_guard_delta` kwarg now defaults `False` (was hardcoded `True` inside that function) — the internal coverage-scaled `guard_delta_vw` intermediate no longer leaks into every consumer by default. All THREE of `_compute_portfolio_strip`'s `get_portfolio_today_change(...)` call sites (Tier-0 warm-cache, Tier-1 last-good, Tier-2 floor) now pass `include_paired_guard_delta=True` explicitly — a load-bearing requirement, not cleanup: the Tier-2 floor's paired re-derivation immediately below reads `vw_tc.get("guard_delta_vw")` and silently falls back to a raw (mismatched-membership) passthrough if any one of the three omits the opt-in. **Mutation-testing gap, closed:** held-imp's own GREEN report (commit `f35b3de7`) disclosed that Tier-0/Tier-1 had NO dedicated test coverage forcing this specific interaction — disabling either opt-in still produced a fully-green suite, since the pre-existing `test_account_basis_tc.py` regression guards mock `get_portfolio_today_change` wholesale. held-tw closed this in a dedicated follow-up (commit `50742343`, 2 new route-level tests forcing each tier via direct cache seeding, never mocking `analytics`) — all 4 real `include_paired_guard_delta=True` sites in the codebase (these 3 plus the frozen branch's, below) are now independently mutation-verified.

**Tier-2 floor re-derivation.** The Tier-2 fallback path (fires when `_account_totals_last_good` has nothing for TC either) previously rendered `today_change` directly from `analytics.get_portfolio_today_change()`'s raw `{"if_held":..., "dry_run":...}` -- whose `dry_run` used a dry-run-only-membership average that could mismatch `if_held`'s full-membership average on a coverage-gap day (FINDING-2). This path now re-derives `today_change["dry_run"]` as `_vw_tc_floor["if_held"] + _vw_tc_floor["guard_delta_vw"]` when the coverage-scaled `guard_delta_vw` is present-and-not-`None` -- `if_held` itself stays full-membership, preserving this floor's own AC-6 contract of never silently narrowing to exclude a live coverage-gap symphony. **When `guard_delta_vw` is `None` (zero paired coverage, e.g. an all-degraded portfolio), the else branch builds an explicit fresh `{"if_held": _vw_tc_floor.get("if_held"), "dry_run": _vw_tc_floor.get("dry_run")}` dict rather than passing `_vw_tc_floor` through verbatim** -- the raw 3-key dict still carries the internal `guard_delta_vw` key (`None` on this path, but still present as a key), and passing it straight through leaked that key into the public `/api/state` JSON (F6 follow-up, `DE-HELD-BASIS-001`, 2026-08-25 -- see `DECISIONS.md`). See `docs/generated/analytics.md`'s `_value_weighted_portfolio` section for the coverage-scaled formula (Revise 1 corrected this from a paired-mean extrapolation to a paired-sum-over-total-weight formula, F2/F7).
