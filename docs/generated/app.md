...
**Proposal-identity render prefetch (`DE-FR-PROPOSAL-IDENTITY-001`, 2026-08-20).** Additive to the same `frontrunner_proposals` loop, per-row, after the existing `candidate_tree_preview` bounding:

- **Incumbent display-name resolution.** `database.load_state()` is called ONCE per request (before the loop, not once per row — mirrors `advisors.frontrunner_builder.approve_frontrunner_proposal`'s own hash→name lookup pattern). Each row's `symphony_id` (the incumbent's Composer hash) is looked up against the resulting `bot_state`; a match stamps `_incumbent_display_name` with the live `name` field, an unmatched hash (e.g. a symphony removed since the proposal was created) falls back to the raw hash string itself — never a blank identity line.
- **`metrics_json` non-dict normalization (AC-6).** Before the template ever indexes `m.xxx`, a non-`dict` `metrics_json` (a malformed/legacy row, or a parsed-JSON list) is normalized to `{}`. This closes a real crash this cycle found and fixed at the render layer (not merely a defensive guard) — see the "Mid-cycle regression" note in [advisors_frontrunner_builder.md](advisors_frontrunner_builder.md)'s Testing section: several stats-table cells used `is not none` on values that become Jinja `Undefined` (not literal `None`) once `metrics_json` degrades to `{}`, which then crashes Jinja's `format()` filter. The normalization here and the template's `is defined and` guards are two halves of the same AC-6 fix — the normalization alone was insufficient without the template-side guard.
- **`overlay_tree_preview`.** Mirrors the existing `candidate_tree_preview` JSON-dump-and-truncate transform (`_FR_OVERLAY_PREVIEW_MAX_CHARS = 4000`, a dedicated constant, not reused from `_FR_TREE_PREVIEW_MAX_CHARS`) against `metrics_json.get("overlay_tree")` — but unlike `candidate_tree_preview`, `overlay_tree` is NOT popped off `metrics_json`, since the template still needs `m.replaced_node_id`/`m.overlay_summary` reachable from the same dict. A row predating this cycle (no `overlay_tree` key) renders an empty string; the template's own AC-6 fallback text ("overlay not recorded for this proposal") is what the operator actually sees, not a blank preview block.

The whole per-row block stays inside the pre-existing `try/except` around the `frontrunner_proposals` loop — any failure degrades to `frontrunner_proposals = []` and the template's existing empty-state, never a 500. See [advisors_frontrunner_builder.md](advisors_frontrunner_builder.md)'s "Proposal Identity & Provenance" section for the pure helpers and persist-site wiring this render layer consumes, and `DE-FR-PROPOSAL-IDENTITY-001` in `DECISIONS.md` for the full cycle record.
