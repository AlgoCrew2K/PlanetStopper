...
- **Cross-cutting contract, not SB-only — R2-2 confirms it:** `build_reasoning_context` and the 4-key `provenance` shape are the shared enabler `DE-ADVISOR-R2-1-001` established. **R2-2 (Logic Changes, `DE-ADVISOR-R2-2-001`) now reuses both verbatim** on `POST /ai-advisor/logic-changes/evaluate` — see the route section above — though its route wraps the same shape in a stricter never-absent contract than this route's. R2-3 (Asset Swaps) is the one remaining planned reuse point, not yet started.
