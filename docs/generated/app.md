...
| `date_range` | dict | `{earliest, latest}` ISO dates from filenames. EOD path: brackets ONLY the in-window files' own dates when `window` is present (not the cutoff bound, not the all-time earliest/latest) -- the full history's earliest/latest when `window` is omitted/`"all"`. `{null, null}` when using intraday path. |
...
See `DE-GAP-001`, `DE-LIVE-DASH-001`, `DE-POSTMORTEM-INTEGRITY-001`, `DE-EXIT-FRICTION-REALIZED-001`, and `DE-GAS-COHERENCE-001` in `DECISIONS.md`.

**Consumed by:** `fetchGuardAlphaSummary(windowToken)` in `static/index.js` -- as of `DE-GAS-COHERENCE-001`, called with the currently-active hero window token on page load, on every window-picker click, and on every `cycle-complete` SSE event, so the panel re-windows in lockstep with the rest of the hero instead of always showing all-time. Populates `#dollar-saved-headline`, `#dollar-saved-verb` (new, sign-conditional "saved"/"lost"), `#guard-event-count`, `#dollar-saved-basis-label` in `templates/index.html` (`data-testid="dollar-saved-panel"`) -- plus, as of exit-friction-realized-savings, the additive `#dollar-saved-realized-headline`/`#dollar-saved-realized-verb`/`#dollar-saved-realized-coverage` sibling elements (STATIC "marks basis" caption in the template, never JS-injected -- RULING A honesty requirement). Does not clobber `#guard-alpha-headline` (windowed % guard alpha from `/api/strip/<window>`). See `docs/generated/static_index_js.md`.
