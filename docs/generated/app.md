...
- **`provenance` is ABSENT (not `null`) on both error branches** (`run.error`, the route's own outer exception) — mirroring the `backtest_unavailable`/`mode_notice` honest-absence convention above. The route never invents a provenance object for a run that never produced candidates.
- **Route-boundary `isinstance(dict)` serialization guard (a named pattern for R2-2/R2-3 to reuse):**
  ```python
...
- **Cross-cutting contract, not SB-only:** `build_reasoning_context` and the 4-key `provenance` shape are the shared enabler R2-2 (Logic Changes) and R2-3 (Asset Swaps) reuse for their own routes — see `DE-ADVISOR-R2-1-001` in `DECISIONS.md`.
...
- `ai_advisor` — context assembly, Claude call, C2 safety gates, assessment builder; `build_reasoning_context` (R2-1, symphony-scoped Strategy Builder runs only)
- `database` — all state-DB reads and writes for advisor routes, including `get_latest_market_prism_verification_for_run` (DE-PRISM-NUMERIC-VERIFY-001 Overview overlay), `get_candidate_alert_new_valid_count`/`get_candidate_alert_last_run`/`mark_candidate_alert_viewed` (candidate-alert header indicator), and `load_state`/`normalize_name` (R2-1's NAME→hash resolution for `build_reasoning_context`)
