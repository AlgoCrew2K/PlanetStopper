...
**Explainer wiring (Phase 2 Cycle 2b, AC-2, `DE-RETIRE-APPROVAL-001`).** Between `build_recommendations()` and `persist_recommendations()`, the worker lazily imports `advisors.retirement_explainer.explain_recommendation` (CC-2) and calls it once per recommendation, stamping the result onto `rec["explanation"]` in place before the batch is persisted:
...
This is defense-in-depth, not a contract violation expectation: `explain_recommendation` is D-1/never-raises by its own contract (see [advisors/retirement_explainer](advisors_retirement_explainer.md)), so this `try`/`except` should rarely if ever fire in practice -- but a per-rec failure here degrades only that one recommendation's `explanation` to `None` and never blocks persistence of the rest of the batch (a batch of, say, 5 recommendations where the explainer times out on 1 still persists all 5, 4 with real explanations and 1 with `explanation: null`). **This keeps the LLM entirely out of `advisors/retirement_recommender.py`** (byte-unchanged this cycle, pinned by a golden-hash test) **and out of the render/approve/checklist paths** (operator ruling, Gate-2b) -- the explainer only ever runs here, at 03:45, once per recommendation per night.

**Three independent error-logging layers, by design (not redundant).** `build_recommendations()` itself gained its own internal `except`-clause WARNING log at `f9819274` (Revise round 2, PR-level `/code-review` Finding 5) -- since that function never raises (D-1), this worker's own outer `except` around the call chain will essentially never fire FOR a `build_recommendations`-internal failure specifically (it always returns `[]` cleanly). The new per-rec explainer `try`/`except` (above) covers a THIRD, narrower surface: an `explain_recommendation` contract violation for exactly one recommendation in the batch. The worker's own outer `except` remains the catch-all for failures OUTSIDE both of those boundaries -- e.g. an exception raised by `persist_recommendations()` itself, or by either lazy import. The three logs cover disjoint failure surfaces.

See [advisors/retirement_recommender](advisors_retirement_recommender.md), [advisors/retirement_explainer](advisors_retirement_explainer.md), `DE-RETIRE-CORE-001`, and `DE-RETIRE-APPROVAL-001` in `DECISIONS.md`.
...
### Retirement Approval Lifecycle — explainer output, live-join, and checklist assembly in `ai_advisor_tab()` (Phase 2 Cycle 2b, AC-2/AC-4/AC-6/AC-7, `DE-RETIRE-APPROVAL-001`, 2026-08-26)

Three additive pieces layered onto the Cycle-2a prefetch above, all inside the SAME `if retirement_recommendations:` guard so none of this work runs on an empty batch.
...
**2. Live-join (AC-4).** Status is read FRESH from the mutable `retirement_decisions` table on every request — never from the frozen `advisor_observations` row, which never carries an `approval_status` key at all. Mirrors the Strategy Incubation Gate's `_incubation_by_hash` live-join pattern (documented above):
...
A recommendation with no decision row shows `"pending"` (the `(_decision or {}).get(...) or "pending"` chain degrades honestly whether the row is entirely absent, or present with a falsy/unexpected `approval_status`). A decision row for a candidate no longer present in the latest batch is simply never looked up — the live-join only stamps cards that actually render this request; an orphaned row is harmless (feature plan Edge Cases). `_ret_decisions_by_id`/`_ret_any_approved` are read failures that degrade to `{}`/never-True respectively, never a 500.

**[CORRECTED, PR #139 review remediation, F6, `5655edfd`, 2026-08-26]** the loop above no longer stamps `_rec["_decision"] = _decision`. The original Cycle-2b GREEN commit did stamp it, but nothing ever consumed the key — neither `templates/ai_advisor.html` nor any other `app.py` code path reads `_decision`/`_rec["_decision"]` (verified: zero references anywhere in the shipped template or route code beyond the loop's own local `_decision` variable, which the `approval_status` line above already fully consumes). A same-cycle PR-level `/code-review` (PR #139) flagged this as dead weight and it was removed; the local `_decision` variable itself is unaffected (still used on the very next line).

**3. Checklist assembly (AC-6/AC-7 glue).** Built ONLY for approved cards, and ONLY when at least one card in the batch is approved (`_ret_any_approved` short-circuit — `database.load_state()` is never fetched on an all-pending/all-rejected batch, avoiding wasted I/O, the same rationale as the frontrunner identity-resolution prefetch elsewhere in this function):
