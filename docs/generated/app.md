...
**Held-basis convergence (`DE-HELD-BASIS-001`, 2026-08-13).** The `symphonies_list` sym_dict this function builds from `bot_state` (`app.py:~1508-1514`) now carries an additive `"current_return_is_reconstructed": s.get("current_return_is_reconstructed", False)` field -- threading BL-9's basket-reconstruction marker through to `analytics.get_symphony_today_change`, so a TRIGGERED symphony's Today-row `if_held` can prefer the raw `shadow_history` if-held trajectory over the engine's VWAP-based reconstruction (see `docs/generated/analytics.md`). The identical field is threaded at the live-poll `get_state()` card-builder site (`app.py:~2624-2630`, documented under State Helpers below) -- `dashboard()`'s own SSR sym_dict builder and the frozen/closed-market snapshot branch are deliberately NOT threaded (see `DE-HELD-BASIS-001` for why the frozen branch is structurally immune to this defect).

**Tier-2 floor re-derivation (same cycle).** The Tier-2 fallback path (fires when `_account_totals_last_good` has nothing for TC either) previously rendered `today_change` directly from `analytics.get_portfolio_today_change()`'s raw `{"if_held":..., "dry_run":...}` -- whose `dry_run` used a dry-run-only-membership average that could mismatch `if_held`'s full-membership average on a coverage-gap day (FINDING-2). This path now re-derives `today_change["dry_run"]` as `_vw_tc_floor["if_held"] + _vw_tc_floor["guard_delta_vw"]` when the new paired `guard_delta_vw` is present-and-not-`None` (falling back to the raw dict otherwise, e.g. an all-degraded portfolio) -- `if_held` itself stays full-membership, preserving this floor's own AC-6 contract of never silently narrowing to exclude a live coverage-gap symphony. See `docs/generated/analytics.md`'s `_value_weighted_portfolio` section for the paired-membership mechanism.
