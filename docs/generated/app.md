...
- `portfolio_strip["basis"] = "value_weighted"` fires when EITHER TC or CR has no basis at all (Tier 2 for that field) — each field's own cache+last-good state is checked independently, so a warm TC is never silently mislabelled just because CR is on Tier 2 (previously the marker only checked TC's state, letting a CR-only degradation ship as unlabelled raw VW).
