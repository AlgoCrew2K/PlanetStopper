...
**Reasoning-context assembler (2026-07-13 — R2-1, `DE-ADVISOR-R2-1-001`):** `build_reasoning_context(symphony_id, objective, *, composer_symphony_id=None) -> tuple[str, dict]` assembles a bounded, human-readable operator-context block for Strategy Builder generation — the operator's real symphony tree (rendered via `symphony_schema.render_rules_text`, capped at `_MAX_TREE_RENDER_CHARS=6000`), live Optuna stats, and the 5 market-lens blocks (reusing `assemble_advisor_context`'s existing nightly cache-serve path — never a fresh live fan-out) — paired with an honest per-source manifest (`tree`/`stats`: present|absent; the 5 lenses: available|stale|absent) a caller can surface as provenance. Falsy `symphony_id` (the from-scratch, non-symphony-scoped path) returns immediately with zero I/O — no Composer fetch, no DB read — matching the byte-preservation floor one layer up in `build_plan_generator._build_generation_prompt`. D-1: never raises, even when a collaborator (e.g. `symphony_logic.fetch_symphony_score`) itself raises. See [advisors/build_plan_generator](advisors_build_plan_generator.md) for how the returned text is threaded into the SB generation prompt, and `DE-ADVISOR-R2-1-001` in `DECISIONS.md` for the full provenance contract.

**R2-2 reuse confirmed (2026-07-14, `DE-ADVISOR-R2-2-001`):** `build_reasoning_context` is now also called by `app.py`'s `ai_advisor_logic_changes_evaluate()` route for symphony-scoped Logic Changes runs, threading its `(prompt_context, manifest)` return straight into `advisors.logic_change_engine.propose_operator_logic_change(reasoning_context=, reasoning_manifest=)`. **This file itself carries ZERO diff for R2-2** — `ai_advisor.py`, `advisors/symphony_schema.py`, and `advisors/backtest_gate_engine.py` are all byte-unchanged this cycle; R2-2 is pure reuse, exactly the cross-cutting design R2-1 built this assembler for. See [advisors/logic_change_engine](advisors_logic_change_engine.md) and `DE-ADVISOR-R2-2-001` in `DECISIONS.md`. R2-3 (Asset Swaps) is the one remaining planned reuse point, not yet started.

**Gate-3 direction fix + real holdout (2026-07-20 — F-013, `DE-ADVISOR-GATE3-DIRECTION-001`):** `revalidate_suggestion_oos` was comparing `run_simulation`'s raw, un-negated `-total_guard_alpha` return directly (`run_simulation` returns the objective NEGATED — smaller is better, autotuner.py:1949) — the inverted comparison greenlit the candidate with LESS guard-alpha and blocked genuine improvements. It also evaluated the FULL 125-day replay window on both calls, a false "OOS" claim (actually in-sample). Fixed: both `run_simulation` calls now run on an IDENTICAL held-out tail slice (`_slice_oos_holdout`, `_OOS_HOLDOUT_FRACTION=0.20`, `_OOS_HOLDOUT_PURGE_DAYS=10`) and both raw returns are re-negated to real guard-alpha before the strict-`>` comparison. Confirmed unfired in production before the fix (`llm_suggestions` table had zero rows — no operator Accept had ever reached Gate-3). See below and `DE-ADVISOR-GATE3-DIRECTION-001` in `DECISIONS.md`.
...
| `objective` | (unused today) | Reserved for a future objective-conditioned rendering; every source is gathered unconditionally regardless of objective. Accepts any of the callers' typed objective dataclasses (`Objective`, `LogicChangeObjective`, ...) — the function never inspects its shape. |
...
- Bound (AC-9): `_MAX_TREE_RENDER_CHARS = 6000` bounds INPUT-context growth/cost for the injected tree text specifically. Distinct from `build_plan_generator.MAX_OUTPUT_TOKENS` (a different, OUTPUT-side ceiling on the SDK's structured-tool-use response) AND distinct from `logic_change_engine._MAX_OUTPUT_TOKENS` (that module's own, separately-named output ceiling — see [advisors/logic_change_engine](advisors_logic_change_engine.md)). Conservative, uncalibrated value (~1,500 tokens at a rough 4-chars/token estimate) — no measured worst-case exists yet, unlike `MAX_OUTPUT_TOKENS`'s calibrated figure.
...
|----------|------|-------|---------|
| `_EMPTY_MANIFEST` | `dict` | 7-key, all `"absent"` | Returned (a fresh `dict()` copy) whenever nothing is injectable — every key defaults to `"absent"`, never omitted, never fabricated as present. `ai_advisor.py:67-78`. **R2-2:** also imported directly by `advisors/logic_change_engine.py` (module-level `import ai_advisor`) as the fallback `provenance["evidence_injected"]` value on any run whose caller supplies no `reasoning_manifest`. |
| `_MAX_TREE_RENDER_CHARS` | `int` | `6000` | INPUT-context bound: caps the rendered real-tree text injected into a generation prompt (AC-9). NOT related to `build_plan_generator.MAX_OUTPUT_TOKENS` or `logic_change_engine._MAX_OUTPUT_TOKENS` (see the wording-reconciliation note above). `ai_advisor.py:80-88`. |
...
- **R2-2 (2026-07-14):** `app.py`'s `ai_advisor_logic_changes_evaluate()` route — same symphony-scoped-only pattern. Threaded into `advisors.logic_change_engine.propose_operator_logic_change(reasoning_context=, reasoning_manifest=)` → `generate_reasoned_logic_candidates(reasoning_context=)` (see [advisors/logic_change_engine](advisors_logic_change_engine.md)).

**Cross-cutting contract:** this assembler and its manifest shape are the shared enabler `DE-ADVISOR-R2-1-001` establishes for the whole R2 program. **R2-2 (Logic Changes) now confirms this in practice** — it calls the SAME `build_reasoning_context` and extends the SAME 4-key provenance surface to its own route, with zero change to this file. R2-3 (Asset Swaps) is the one remaining planned reuse point, not yet started; no port has re-derived its own context-assembly or manifest shape from scratch.
...
Re-validates an accepted suggestion through the autotuner's OOS gate on a genuinely held-out tail slice of the 125-day replay window (`_slice_oos_holdout`, F-013 AC-4, `DE-ADVISOR-GATE3-DIRECTION-001`) — a purge-gap-bounded tail (`_OOS_HOLDOUT_FRACTION=0.20`, sourced from `autotuner.FROZEN_EVAL_RATIO`; `_OOS_HOLDOUT_PURGE_DAYS=10`, half of `autotuner.PURGE_DAYS` since the 125-day replay is exactly half the 250-day walk-forward window) — never the full training window. `run_simulation` is called TWICE on the IDENTICAL holdout slice (baseline + patched strategy); its raw return is `-total_guard_alpha` (autotuner.py:1949, smaller = better) — both raw returns are re-negated to real guard-alpha BEFORE comparison or storage. Pass rule is strict `>` on the re-negated values — a tie does not pass (autotuner strict-positive cascade rule).

Fails closed (`passed=False`, both guard-alpha fields `None`, `run_simulation` NEVER called) when: no `bot_state` account matches `symphony_id` (`acc_sym_ids` derivation double-normalizes both sides via `database.normalize_name`, matching the precedent at `app.py:4660/5011/5313/5784` — an un-normalized `symphony_id` arrives from the route, `app.py:5846`); the replay window is too short to carve out a real holdout tail after the purge gap; or either `run_simulation` call returns a non-finite (NaN/Infinity) objective. Never silently falls back to full-window in-sample evaluation.

The `autotuner` import is lazy (inside the function body) to avoid import-collision with the Anthropic SDK under pytest.

**Returns:** `{"passed": bool, "patched_guard_alpha": float | None, "baseline_guard_alpha": float | None, "detail": str}`. The old field names `oos_alpha`/`baseline_oos_alpha` — and their raw, un-negated, full-window semantics — no longer appear (clean rename, no dual-key back-compat needed: the sole production reader, `app.py:5874-5881`, reads only `passed`/`detail`; `oos_revalidation` is stored as an opaque JSON blob, `database.py:961`). The guard-alpha fields are `None` (never a NaN/Infinity literal) on every fail-closed path.
...
| `_OOS_HOLDOUT_FRACTION` | `float` | `0.20` | F-013: fraction of the OOS replay window Gate-3 (`revalidate_suggestion_oos`) carves off as a genuine held-out tail before comparing guard-alpha. Mirrors `autotuner.FROZEN_EVAL_RATIO` (autotuner.py:436). `DE-ADVISOR-GATE3-DIRECTION-001`. |
| `_OOS_HOLDOUT_PURGE_DAYS` | `int` | `10` | F-013: purge buffer immediately preceding the Gate-3 holdout tail, guarding against feature-lookback leakage. Half of `autotuner.PURGE_DAYS` (=20) — the 125-day OOS replay window is exactly half the autotuner's 250-day walk-forward window. `DE-ADVISOR-GATE3-DIRECTION-001`. |
| `_EMPTY_MANIFEST` | `dict` | 7-key, all `"absent"` | R2-1: default honest manifest returned by `build_reasoning_context` when nothing is injectable. `ai_advisor.py:67-78`. R2-2: also imported directly by `advisors/logic_change_engine.py` as its fallback `evidence_injected` default. |
| `_MAX_TREE_RENDER_CHARS` | `int` | `6000` | R2-1: INPUT-context bound — the rendered real-tree text `build_reasoning_context` injects into a generation prompt (AC-9). Independent of `build_plan_generator.MAX_OUTPUT_TOKENS` and `logic_change_engine._MAX_OUTPUT_TOKENS`. `ai_advisor.py:80-88`. |
