...
**Polarity precondition (Revise 5, F1 — evaluated FIRST, wraps BOTH paths above).** When `fire_is_else_branch=True` (the detected cascade's fire/signal content sits on the untrustworthy `is-else-condition?==True` side — `_graft_incumbent_core`'s core-preservation logic assumes normal polarity and would silently drop the real core for this shape), the WHOLE acceptance declines unconditionally — `accepted=False`, `tags=set()` — regardless of what either path's own gates would otherwise say. Revise 4 introduced this precondition scoped to SIMPLIFY only; Revise 5's F1 widened it to cover IMPROVE too, since a genuinely-better-Calmar inverted-polarity candidate was previously still reachable via IMPROVE and could be queued for a real Composer draft creation. No early return — every other reporting field (`candidate_sharpe`/`candidate_volatility`/both Calmars/`node_count_delta`) stays genuinely computed.
...
| `fire_is_else_branch` | `bool` | **New, Revise 4; scope widened, Revise 5 F1.** `True` when the cascade's fire (signal) content sits on the untrustworthy `is-else-condition?==True` side (inverted polarity) — `_graft_incumbent_core`'s core-preservation logic assumes normal polarity and would silently drop the real core for this shape. Declines the WHOLE acceptance unconditionally when `True` (Revise 4 had scoped this to SIMPLIFY only). Default `False` (never declining by default on an omitted value) |
