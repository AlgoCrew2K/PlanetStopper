...
**Prompt-builder seam.** Builds the full SDK prompt for the given objective. Extracted from the old inline f-string to make the sent instructions independently testable without mocking a full SDK round-trip. **Untouched by `DE-ADVISOR-CACHE-001` (2026-07-29)** — the prompt-caching split at the SDK-call boundary (see "Prompt Caching" below) calls this function exactly as before, twice when `reasoning_context` is supplied; this function's own signature, body, and return value carry ZERO diff.
...
Calls `_build_client()` → `_build_generation_prompt(objective, n_plans, membership_set, reasoning_context=None)` (embeds DSL grammar + `_EXAMPLE_PLAN` + per-objective signature) → wraps that reasoning_context-free base in a single `cache_control`-marked content block (`DE-ADVISOR-CACHE-001`, see "Prompt Caching" below), appending an UNCACHED trailing block carrying R2-1's operator context when `reasoning_context` was supplied → structured tool-use SDK call → parses the `"emit_build_plans"` tool-use block's `.input["plans"]` list → membership-validates every ticker → deduplicates structurally-identical plans → enforces objective structural signature → returns admitted plans tagged `provenance="built-new"`.
...
| `reasoning_context` | `str \| None` | R2-1: threaded straight through to `_build_generation_prompt` (see its Parameters above). Additive/keyword, default `None` — every pre-R2-1 caller's exact call shape is unaffected. `DE-ADVISOR-CACHE-001`: when truthy, its text reaches the SDK payload as a separate UNCACHED trailing content block, never inside the cached base. |
...
`generate_build_plans`'s SDK-call construction — not `_build_generation_prompt` itself — gains an ephemeral (5-minute TTL) `cache_control` breakpoint. Real `count_tokens` measurement: **3,126 tokens** for the reasoning_context-free base prompt, clearing the `claude-fable-5` (Fable) model's 2,048-token minimum-cacheable-prefix floor by ~1.5x.

**Mechanics — a stable-base/`reasoning_context` split, not a whole-string wrap:**

1. `_build_generation_prompt(objective, n_plans, membership, reasoning_context=None)` is called (exactly as any pre-R2-1 caller would) to obtain the reasoning_context-free `base_prompt` — byte-identical to the AC-8 golden fixture.
2. `base_prompt` is wrapped in one content block: `{"type": "text", "text": base_prompt, "cache_control": {"type": "ephemeral"}}`.
3. When `reasoning_context` is truthy, `_build_generation_prompt` is called a SECOND time with the real `reasoning_context` value to obtain `full_prompt`. The trailing `## OPERATOR CONTEXT` section is recovered via `full_prompt[len(base_prompt):]` — a plain byte slice, never a hand-duplicated `"\n\n## OPERATOR CONTEXT\n\n"` literal — and appended as a SEPARATE, UNCACHED content block. This guarantees the caching wrapper and `_build_generation_prompt`'s own append logic can never drift out of sync, since the slice is always derived from the function's own live output.
4. `_EMIT_BUILD_PLANS_TOOL` also carries `cache_control` on the tools array (Anthropic caches `tools`/`system`/`messages` as independent breakpoints, up to 4 per request; this site uses 2).

**Why the split, not a simpler whole-string wrap:** the initial approach considered wrapping the ENTIRE returned string (base + appended `reasoning_context`) as a single cached block. That would bury the per-symphony `reasoning_context` text inside the cached prefix — every symphony-scoped call would then present Anthropic with a DIFFERENT cache key, so cross-symphony/cross-run reuse (the real cost win, since the weekly scheduler and repeated operator clicks share the same objective-shaped base across many different symphonies) would never materialize; only the rare same-call retry would ever hit. The split instead caches only the objective-shaped, symphony-INdependent prefix.

**Retry-loop bonus:** the two-block content array is built once per `generate_build_plans` call and resent verbatim across the `MAX_GENERATION_ATTEMPTS=3` truncation-retry loop — a truncated first attempt's retry is therefore a genuine cache read too, not just cross-call reuse.

**Byte-preservation guarantee unaffected:** `_build_generation_prompt`'s signature, body, and return value carry ZERO diff from this cycle — the split happens entirely at the `generate_build_plans` SDK-call boundary. The AC-8 golden-fixture test (`tests/advisors/test_build_plan_generator_reasoning_context.py::test_from_scratch_prompt_byte_identical_to_golden_fixture_captured_pre_r2_1`) is unmodified and stays green.

**The real proof is a live probe, not tests-green.** Every test here mocks `_build_client`/`messages.create` and can only confirm the request SHAPE (a `cache_control`-marked block present, the right text inside/outside it). None can prove Anthropic actually cached anything — that requires a real, non-mocked API round-trip checking `response.usage.cache_creation_input_tokens`/`cache_read_input_tokens`, run once by the PM post-merge.

See `DE-ADVISOR-CACHE-001` in `DECISIONS.md` for the full measurement record across all 7 advisor SDK call sites (2 real breakpoints here + `frontrunner_builder.py`; 5 documented-excluded, including a sibling `ai_advisor.request_suggestions` `client.messages.parse` call missed by earlier `.messages.create`-only greps) and the deferred cross-symphony lens-data caching follow-up in `ai_advisor.py`.
...
- **`DE-ADVISOR-CACHE-001`'s split lives at the SDK-call boundary, not in the prompt-builder.** Consistent with the R2-1 note directly above: `generate_build_plans` (the SDK-call site) owns the caching concern; `_build_generation_prompt` (the prompt-text concern) stays byte-for-byte unchanged. This is the same separation-of-concerns principle applied a second time.
