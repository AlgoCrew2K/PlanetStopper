...
The prompt embeds four pieces of steering content:
...
2. **`if`/`if_compound` condition sub-grammar.** A dedicated section teaches the condition DICT shape for regime-gate nodes: `lhs_fn`, `lhs_ticker`, `window`, `comparator` (`gt`/`lt`/`gte`/`lte`), `rhs` (`{"fixed": N}` or `{"fn": ..., "ticker": ..., "window": ...}`). A WRONG-vs-CORRECT contrast is included: `WRONG: condition = "spy_above_200d_sma"` / `CORRECT: condition = {...dict...}`. All five fields (`lhs_fn`, `lhs_ticker`, `window`, `comparator`, `rhs`) are stated as required. This section appears in every objective prompt (not just `cut_drawdown`) so that any objective using an `if` node produces a compilable condition dict.
3. **`_EXAMPLE_PLAN` (diversify) + `_EXAMPLE_IF_PLAN` (cut_drawdown).** Both conforming examples are embedded verbatim. `_EXAMPLE_PLAN` shows the two-sleeve group structure. `_EXAMPLE_IF_PLAN` shows the `if` node with a compiler-verified condition dict (`rhs: {"fixed": 80}`) and two allocation branches. The `cut_drawdown` prompt uses `_EXAMPLE_IF_PLAN` as its primary worked example; all other objective prompts include it as a supplementary DSL reference.
4. **`_OBJECTIVE_SIGNATURES[obj_name]`.** The per-objective structural signature description is embedded for the requested objective, telling Opus which DSL construct is required (e.g. for `lift_risk_adjusted`: "A bare equal-weight basket does NOT satisfy this signature — the filter construct is required").

**Known limitation:** `if_compound` (compound/multi-condition regime gates) is taught in the prompt text but has no separate worked compiling example, and its compound-condition union (`type`/`operator`/`conditions`) is not independently schema-constrained — the `condition` property in `_EMIT_BUILD_PLANS_TOOL` covers the flat single-condition shape only. This is a documented residual; the PM's live re-exam probes `if_compound` plans to determine whether the flat-condition teaching is sufficient coverage.
...
- **Structured tool-use with tightened schema (defense-in-depth).** The Anthropic SDK call uses tool-use mode (`"emit_build_plans"` tool, `tool_choice={"type":"tool","name":"emit_build_plans"}`). The `_EMIT_BUILD_PLANS_TOOL` input schema has been extended in two phases:
  ...
  - **C2-fix Revise-1 (condition sub-grammar):** Added a `condition` property to the `if`/`if_compound` node entry: typed `object` with properties `lhs_fn` (string), `lhs_ticker` (string), `window` (integer), `comparator` (enum: `["gt","lt","gte","lte"]`), `rhs` (object); all five fields `required`. Structurally prevents Opus from emitting a bare string condition (the Revise-1 drift pattern). **Known residual:** the `condition` property covers the flat single-condition shape; `if_compound`'s compound-condition union (`type`/`operator`/`conditions`) is taught in the prompt text but is not separately schema-constrained — the JSON schema is not recursive enough to express it without combinatorial complexity.
  This schema tightening cannot prevent all deep-nesting drift (the JSON schema is not recursive), but it forces the correct top-level tokens and field vocabulary. The response is a structured `tool_use` block; free-text responses (`stop_reason="end_turn"`) are treated as failures and degrade to empty plans.
