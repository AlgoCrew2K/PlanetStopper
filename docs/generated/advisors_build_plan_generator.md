...
| `_EXAMPLE_IF_COMPOUND_PLAN` | `dict` | A concrete conforming if_compound compound-gate plan (cut_drawdown-shaped; condition is `{type:"compound", operator:"all", conditions:[binary_compound(RSI SPY gt 70 w14), binary_compound(max-drawdown QQQ lt 20 w30)]}`; then: equal-weight UVXY/TLT; else: inverse_vol SPY/IEF). Verified compiler-clean through `plan_tree_compiler.compile_plan` + `validate_tree==[]`. Embedded in every prompt as the third worked example to teach the compound-condition union (`type` discriminator, `operator`, `conditions[]`, `tickers[]` broadcast, `rhs:{const}`) and close the `if_compound` generation gap (Revise-2). |
...
4. **Three compiler-verified worked examples.** `_EXAMPLE_PLAN` (diversify-shaped; two weight sleeves), `_EXAMPLE_IF_PLAN` (flat if-node; `rhs: {"fixed": 80}`), and `_EXAMPLE_IF_COMPOUND_PLAN` (compound-gate if_compound; `{type:"compound", operator:"all", conditions:[...]}`) are all embedded verbatim in every prompt. All three have been verified compiler-clean through `plan_tree_compiler.compile_plan` + `validate_tree==[]`. The `cut_drawdown` prompt uses both if-node examples as its primary worked examples; all other objective prompts include all three as supplementary DSL references. The full Composer condition grammar — flat `if` and compound `if_compound` — is now generation-reachable with a concrete compiling example for each construct.
