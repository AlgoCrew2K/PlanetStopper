...
**Source:** `autotuner.py`
**Last updated:** 2026-08-05 (BL-11, `DE-AUDIT-BL5-12-001` -- a one-line comment documents `total_guard_alpha`'s multi-day-SUM convention at its accumulation start, cross-referencing `avg_oos_alpha` as the un-inflated per-day companion; see the new "oos_alpha Sum-Convention Comment" section below and `DE-AUDIT-BL5-12-001` in `DECISIONS.md`.) Prior: 2026-08-05 (BL-2, `DE-AUDIT-BL2-001` -- the per-symphony optimization loop (`for normalized_name in symphony_names:`) is now wrapped in try/except/else with `_batch_attempted`/`_batch_completed` counters and an additive `optimization_results["_batch_summary"]` return key; an uncaught exception in one symphony's processing is now caught, logged, and the batch continues -- see the new "Per-Symphony Loop Exception Isolation + Batch Visibility" section below. Prior: 2026-08-05 (BL-1, `DE-AUDIT-BL1-001` -- `run_autotuner`'s `database.save_autotune_run(...)` call now passes `pbo=_pbo_value` (was omitted entirely, leaving `autotune_runs.pbo` NULL in every live row despite the Phase-3 PBO gate consuming it for the veto decision -- audit #118 Finding T1); see the new "PBO Veto Persistence" section below. Prior: 2026-07-24 (exit-friction-realized-savings, `DE-EXIT-FRICTION-REALIZED-001` -- new `SIM_EXIT_FRICTION_PCT` constant + friction subtraction at all three replay exit-accounting sites (`_collect_sim_returns`, `_collect_sim_returns_dated`, `run_simulation`); see the new Exit-Friction Constant section below and the Constants table addition. Prior: 2026-07-24 (guard-alpha-preconditions, live-gate correction R2, `DE-GUARD-ALPHA-PRECONDITIONS-001` -- `build_if_held_replay_series`'s cache-hit-only description corrected to reflect the underlying bounded backward-scan probe (was exact-today-only); see the updated section below and `docs/generated/synthetic_history.md`). Prior: 2026-07-23 (guard-alpha-preconditions, `DE-GUARD-ALPHA-PRECONDITIONS-001` -- two new functions appended after `_collect_sim_returns_dated`: `collect_if_held_daily_returns` (pure extraction, distinct from `_collect_sim_returns` -- see recon finding 1) and `build_if_held_replay_series` (route-facing, CACHE-HIT-ONLY hard contract, never fetches live); see the new sections below and `docs/generated/guard_preconditions.md` / `docs/generated/app.md`). Prior: 2026-07-19 (`DE-AUTOTUNE-REPORTING-001`: `run_autotuner`'s three graceful-abort return sites now uniformly return the structured `{"aborted": True, "reason": ...}` marker -- see the note in the Returns section below.) Prior: 2026-07-18 (Math Remediation R3-c, `DE-MATH-R3C-001`, merged to `origin/main` via PR #102 @ `d92a6f4f` (code-complete `a5c011dd`; droplet-deploy status not reconciled by this doc-writer, outside this cycle's scope -- flagged to PM separately)) — `_replay_exit_tick`'s `compute_active_trailing_stop` call (`:1276`) now passes `squeeze_floor=p.get("MAX_SQUEEZE_FLOOR", _replay_squeeze_floor_default())`; new `_replay_squeeze_floor_default()` helper (`:76`) returns `alpha_bot_execution.MAX_SQUEEZE_FLOOR` live, matching production's own fallback source (never a replay-local mirror); see the Squeeze-Floor Replay Parity section below. Prior: 2026-07-18 (Math Remediation R3-b, `DE-MATH-R3B-001`, SHIPPED @ `origin/main` `f3c7e050`, droplet-deployed + verified) — `_replay_exit_tick`'s arm/disarm block is now delegated to the shared `math_engine.compute_arm_disarm_decision` seam, replacing the disarm condition that had been INVERTED (MA-4) and was independently duplicated from production; a new `disarm_confirm_count` key is added to `_fresh_replay_state`; see the corrected note below. Prior: 2026-07-18, Math Remediation R3-a, `DE-MATH-R3A-001` — pre-retune checklist item (a): walk-forward objective-variance coverage extended to all 6 tuned dims (`scripts/objective_variance_probe.py`), source-derived enumeration + suggest-call drift-guard, config-robust across `EXECUTION_START_TIME ∈ {09:30, 9:35}`; prior: 2026-07-17, Math Remediation R2, `DE-MATH-R2-001` — split-level CPCV scoring replacing the 5-path aggregation, train-only adoption holdout, real CRRA-EU frozen-eval metric, regime-conditional exit ticks wired into the undated search path; prior: 2026-07-17, Math Remediation R1 — replay fail-open arm (MA-10), regime-conditional exit ticks (F5), session-window action-phase gate (F6); prior: 2026-07-12, Workstream E)
...
9. Persists the autotune run row via `database.save_autotune_run(..., pbo=_pbo_value, s_count=...)` (returns int row id used by the OC producer). `pbo` is the SAME `_pbo_value` local already consumed by the acceptance-gate call (step 6) and veto decision — a raw passthrough, `NULL` when PBO was never computed (BL-1, `DE-AUDIT-BL1-001`, 2026-08-05 — see "PBO Veto Persistence" below). `s_count` feeds Indicator-3 (operator drift) on LATER runs via their own `prior_runs` query — see "s_count Wiring" below.
...
**Top-level batch shape:** the dict above describes the shape of `optimization_results[normalized_name]` for ONE symphony -- `run_autotuner` actually loops over every symphony in `symphony_names` in one call and returns a dict keyed by `normalized_name`, one entry per symphony, each shaped as above. **BL-2 addition (`DE-AUDIT-BL2-001`, 2026-08-05):** the top-level return also carries an additive `optimization_results["_batch_summary"]` structural key -- `{"attempted": N, "completed": M}` -- which is NOT a symphony entry. Consumers that iterate this dict must skip it explicitly; both live consumers were updated this cycle: `alpha_bot_execution.augment_optimization_results_with_selection_stats` (see `docs/generated/alpha_bot_execution.md`) and `reporting.send_eod_discord_post` (see `docs/generated/reporting.md`).
...
Runs the validation-fold simulation with `params` and returns the aggregate guard-alpha. Used for OOS re-validation in `ai_advisor.revalidate_suggestion_oos`. **Math Remediation R2 (AC-4, `DE-MATH-R2-001`):** this is one of the two UNDATED simulation entry points; it now resolves `exit_confirm_ticks` via `_replay_resolve_regime_exit_ticks` (R1's shared no-lookahead helper, reused not duplicated) before calling `_replay_exit_tick`, matching `_collect_sim_returns_dated`'s R1-era pattern. R1's `xfail(strict=False)` tripwire (`tests/autotuner/test_ac4_r2_residual_tripwire.py`) XPASSed and its marker is REMOVED — the test now passes for real. **Math Remediation R3-a:** also the scoring entry point `scripts/objective_variance_probe.py`'s `walkforward_dim_sweep` drives (never `run_autotuner`) — see [scripts/objective_variance_probe](scripts_objective_variance_probe.md). **exit-friction-realized-savings:** the triggered-exit accounting subtracts `SIM_EXIT_FRICTION_PCT` as a distinct term alongside `penalty` — feeds the OOS-alpha acceptance-gate cascade net-of-friction (AC-2v2 rationale, see the Exit-Friction Constant section above).

### Exit-Friction Constant (exit-friction-realized-savings, `DE-EXIT-FRICTION-REALIZED-001`, 2026-07-24)

#### `SIM_EXIT_FRICTION_PCT: float = 0.5`
Replay-only cost term (0.5 PERCENTAGE POINTS per simulated round-trip exit), subtracted as a distinct term at all THREE replay exit-accounting sites below (`_collect_sim_returns`, `_collect_sim_returns_dated`, `run_simulation`) so the walk-forward objective, the PBO gate, and the OOS-alpha acceptance gate all see net-of-cost guard alpha instead of gross. Closes the transaction-cost gap identified in `docs/research/methodology-validation-2026-07.md` (§3: the walk-forward replay was "100% gross-of-cost").

**Units derivation (Finding E):** the value is derived from `composer_backtest_client._DEFAULT_SLIPPAGE_PERCENT` (`0.005`, a DECIMAL FRACTION = 0.5%, `composer_backtest_client.py:71`) converted to this file's percent-point convention via `RETURN_PCT_TO_FRACTION` (`100.0`) — `0.005 * 100.0 = 0.5`. The bare `0.005` literal would have applied 100x too little friction at these percent-point-scaled subtraction sites (`tick['return']`, the sibling `-0.20` deviation penalty), silently defeating the feature. `[PM-ASSUMED]` — sourced for consistency with the Composer client's existing slippage assumption; operator may override with a measured value.

**Non-tunable by design:** absent from `OPTUNA_SEARCH_SPACE_KEYS`, never read from the per-trial `params` dict or `os.environ` — a tunable friction would let the optimizer game its own cost model. Deliberately kept OUT of `math_engine.py` (the live engine's decision-math module, imported by `alpha_bot_execution.py`) — this is a replay-environment parameter, not live-engine math; a blast-radius test asserts zero references outside `autotuner.py`/`reporting.py`/`database.py`/`app.py`/`static/index.js`/`templates/*.html` (AC-10).

**Distinct from the deviation-dict `penalty` term** (the sibling `-0.20` fallback literal at each site's `deviation_dict.get(reason_str, -0.20)` call) — different semantics: `penalty` is a modeling-uncertainty haircut (execution-deviation risk), `SIM_EXIT_FRICTION_PCT` is a trading-friction cost. Folding them together would destroy the audit trail of both; each retains its own named constant and its own subtraction term (`triggered_return = tick.get("return", 0.0) + penalty - SIM_EXIT_FRICTION_PCT`).

**Applied at all three sites, not just one (AC-2v2, ga2-opt pre-RED recon):** `_collect_sim_returns_dated` feeds `cscv_date_returns` into `math_engine.compute_pbo` (the Phase-3 PBO overfitting veto); `run_simulation` computes `oos_alpha`/`fallback_oos_alpha`/`default_oos_alpha` consumed by the acceptance-gate accept/reject cascade. Single-site application (only `_collect_sim_returns`, which `run_simulation_crra_eu` calls for the trial objective) would leave the PBO veto and the OOS-alpha acceptance gate on a gross-of-friction basis while the trial objective went net-of-friction — reintroducing the exact gross-vs-net inconsistency this feature exists to close, just relocated from objective to gate.

See `DE-EXIT-FRICTION-REALIZED-001` in `DECISIONS.md` for the full ruling trail (units correction, three-site scope amendment, and the realized-basis + turnover companion features shipped in the same cycle). A forward-looking source comment above the acceptance-gate call site (`~autotuner.py:3049`, `run_autotuner`) additionally flags a currently-inert risk: `_gate_verdict`'s `incumbent_stability_score`/`incumbent_prior_anchor_score` arguments are literal `1.0`/`1.0` placeholders today, so a constant-vs-constant comparison can't disagree on cost basis -- but if a future cycle wires real DB-sourced incumbent-performance data into those two fields, it must confirm the incumbent score was computed on a friction-consistent basis with the (now friction-aware) candidate score, or risk a silent gross-vs-net mismatch at the acceptance gate. Comment-only, no logic change; regression-pinned by `tests/autotuner/test_incumbent_score_friction_tripwire.py` (found during review by ga2-opt, PM-approved review-scope item).

#### `_collect_sim_returns(p, history_data, acc_sym_ids, current_date_str, deviation_dict) → list[float]`
Runs the guard-alpha simulation and returns per-triggered-day guard-alpha values. Shared tick loop with `run_simulation` via `_replay_exit_tick`. No recency decay (Decision D5). **Math Remediation R2 (AC-4, `DE-MATH-R2-001`):** the second UNDATED entry point — same regime-tick wiring as `run_simulation` above, landed in the same commit (`c66457dd`). **exit-friction-realized-savings:** the triggered-exit accounting subtracts `SIM_EXIT_FRICTION_PCT` as a distinct term alongside `penalty` — see the Exit-Friction Constant section above.
...
**`score_dates` keyword-only parameter (`set[str] | None`, default `None`) — Math Remediation R2, AC-1-adjacent (`DE-MATH-R2-001`), "the catch of the cycle":** when provided, restricts which dates undergo the expensive per-tick replay and appear in the returned list — dates present in `history_data` but absent from `score_dates` are skipped entirely. Regime-lookback chronology (`sorted_dates`/`date_to_idx`, and therefore `_replay_resolve_regime_exit_ticks`'s trailing window) is ALWAYS built from the FULL `history_data` regardless of `score_dates` — this is the decoupling that lets a per-split CPCV caller pass the full per-symphony history (gap-free regime chronology) while restricting each split's own scored-date set, instead of the old pattern of pre-filtering `history_data` itself (which would corrupt the trailing-window chronology into a gappy sample). `None` preserves the original behavior: every date in `history_data` is scored. Regime-conditional `exit_confirm_ticks` resolution (R1, extended below) is unchanged by this parameter — it was already wired here. **exit-friction-realized-savings:** the triggered-exit accounting subtracts `SIM_EXIT_FRICTION_PCT` as a distinct term alongside `penalty` — this is the function feeding `cscv_date_returns` into `math_engine.compute_pbo` (the PBO veto), so the veto itself now evaluates net-of-friction guard alpha (AC-2v2 rationale, see the Exit-Friction Constant section above).

#### `collect_if_held_daily_returns(history_data: dict, sym_id: str) -> list` (guard-alpha-preconditions, `DE-GUARD-ALPHA-PRECONDITIONS-001`, 2026-07-23)

Pure extraction: returns `ticks[-1]["return"]` for EVERY date present in `history_data[sym_id]`, unconditionally -- never filtered to triggered days. **Distinct from `_collect_sim_returns` above by design** (guard-alpha-preconditions recon finding 1): `_collect_sim_returns` returns the sparse per-TRIGGERED-day guard-alpha delta series, the wrong quantity for a lag-1-persistence/Sharpe measurement over the full if-held trajectory. Pinned by `tests/autotuner/test_collect_if_held_daily_returns.py::TestOutputIsNotTheGuardAlphaSeries`, which monkeypatches `_replay_exit_tick` to never trigger and confirms `_collect_sim_returns` legitimately returns `[]` on the same data while this function returns every day. No I/O, no re-simulation. Sole production caller: `build_if_held_replay_series` below.

#### `build_if_held_replay_series(symphony_id: str) -> list | None` (guard-alpha-preconditions, `DE-GUARD-ALPHA-PRECONDITIONS-001`, 2026-07-23)

Route-facing orchestration seam for `GET /api/guard-alpha-preconditions` (see `docs/generated/app.md`)'s "replay" sample. **CACHE-HIT-ONLY (PM-ruled hard contract, 2026-07-23) -- this function must NEVER trigger a live Alpaca fetch.** It reads `synthetic_history.get_cached_synthetic_history_only` (see `docs/generated/synthetic_history.md`) and returns `collect_if_held_daily_returns(...)` on whatever it finds; exhausting `synthetic_history.AUTOTUNE_CACHE_MAX_AGE_TRADING_DAYS`'s bounded backward window (10 NYSE trading days as of the R2 live-gate correction, 2026-07-24 -- see `docs/generated/synthetic_history.md`) without a hit returns `None` rather than falling through to a fetch, and the caller degrades that sample honestly rather than blocking a dashboard GET on a multi-minute history assembly.

**Why cache-hit-only:** Architecture Constraint 5 (dashboard/UI never reruns the engine), request latency (a cold fetch can take minutes), and the standing bill-protection directive (a dashboard refresh must never drive Alpaca fetch volume). This supersedes an earlier "synchronous on-demand, matches other advisory routes" framing floated during RED-writing, which the PM ruled incorrect for this specific route.

Resolves "today" via `synthetic_history.utc_to_eastern` (the same DST-aware conversion `generate_synthetic_history` itself uses) -- deliberately NOT `alpha_bot_execution.get_current_et`, which pulls a heavy production import graph and reads several credentials at import time, unnecessary weight for a date string on an advisory GET (ga-impl design judgment).

Pinned by `tests/autotuner/test_build_if_held_replay_series.py::TestCacheMissNeverFetches` (accessor-level: exhausted backward window -> `None`, `synthetic_history.fetch_bars` never called; the backward-scan mechanism itself is additionally pinned by `tests/synthetic_history/test_autotune_cache_backward_probe.py`) and `tests/app/test_guard_alpha_preconditions_route.py::TestColdCacheNeverFetchesEndToEnd` (route-level, through the real function, not mocked).
...
### Per-Symphony Loop Exception Isolation + Batch Visibility (BL-2, `DE-AUDIT-BL2-001`, 2026-08-05)

**The gap:** the per-symphony loop (`for normalized_name in symphony_names:`, `autotuner.py:2562`) had NO surrounding try/except -- Optuna `study.optimize`, CPCV/PBO computation, the OOS adoption cascade, and `database.save_autotune_run` were entirely unguarded (only the tail Overfitting Conscience/Divergence Explainer calls had their own local isolation). An uncaught exception anywhere in one symphony's processing propagated out of the whole loop, silently dropping every symphony not yet reached, with no `aborted` marker (the `DE-AUTOTUNE-REPORTING-001` graceful-abort contract only covers conditions detected BEFORE the loop starts). Live symptom: the 2026-07-24 autotune batch wrote only 7 of 11 symphonies.

**The fix:** the loop body (from `strat_data = database.get_symphony_strategy(...)` through the existing OC/DE tail calls) is wrapped in a top-level `try`/`except`/`else` (`autotuner.py:2564-3323`). Two counters, `_batch_attempted`/`_batch_completed`, increment at loop-body entry and on a clean iteration's `else:` clause respectively -- never on a caught exception. A caught exception is logged via `logging.error(..., exc_info=True)` naming the symphony, then `continue`s to the next symphony. The final counts are attached to the return value as `optimization_results["_batch_summary"] = {"attempted": N, "completed": M}` (`autotuner.py:3325-3329`) -- see the "Top-level batch shape" note in the Returns section above.

**Mid-window false-success gap (review-found, fixed same cycle):** an initial version of the fix left a narrow gap -- an exception raised AFTER `optimization_results[normalized_name] = {}` (`autotuner.py:2952`) but BEFORE the real delta keys / `_baseline_chosen` land later in the iteration left a stale, near-empty partial entry in the dict. `reporting.py`'s F-015 shape guard would then render that empty entry as "Optimal parameters retained." -- a false success for a symphony whose optimization actually failed. Fixed by adding `optimization_results.pop(normalized_name, None)` inside the except block before `continue` -- a failed symphony now has NO key in the return dict, matching the early-failure case.

**Honesty boundary -- this fix does NOT cover a process kill (`DE-AUDIT-BL2-001`, INV-1 correction):** `docs/audit/INV-FINDINGS-2026-08-05.md`'s same-day droplet investigation root-caused the actual 2026-07-24 incident to a deploy-restart SIGTERM at 22:34:39 UTC mid-batch -- NOT an uncaught exception. A try/except cannot catch a process kill. This fix ships as defense-in-depth against the uncaught-exception class regardless (per the feature plan's own framing), but neither the source comment at the except block (`autotuner.py:3302-3307`) nor `tests/autotuner/test_bl2_loop_isolation.py` claim SIGTERM coverage -- one test explicitly asserts `"sigterm"` never appears in the isolation log message.

**Consumers updated:** `alpha_bot_execution.augment_optimization_results_with_selection_stats` and `reporting.send_eod_discord_post` both skip the `_batch_summary` key explicitly; the latter renders a new "⚠️ Autotuner Partial Batch" Discord embed when `attempted != completed`, distinct from both the existing "Autotuner Aborted" embed and a normal full-batch embed -- see `docs/generated/alpha_bot_execution.md` and `docs/generated/reporting.md`.

**Tests:** 16 tests across `tests/autotuner/test_bl2_loop_isolation.py` (10), `tests/execution/test_bl2_batch_summary_augment_guard.py` (1), and `tests/reporting/test_bl2_partial_batch_eod_embed.py` (5) -- isolation, sibling-persistence, counter correctness, zero-diff-besides-`_batch_summary` shape regression, all-failed and late-stage-DB-failure edge cases, the SIGTERM-non-claim pin, and the partial-batch embed's distinctness from both sibling embeds. See `DE-AUDIT-BL2-001` in `DECISIONS.md`.

### PBO Veto Persistence (BL-1, `DE-AUDIT-BL1-001`, 2026-08-05)

**The gap:** the Phase-3 PBO veto gate (step 6 above) genuinely computes and consumes `_pbo_value` (`autotuner.py:2854/2870`, via `math_engine.compute_pbo`) — it is passed into `acceptance_gate.evaluate_acceptance_gate` (`autotuner.py:3075`) and can fire `_pbo_veto_fired` (`autotuner.py:3084-3100`). But the `database.save_autotune_run(...)` call (`autotuner.py:3218-3239`) omitted `pbo=` entirely, even though the accessor's signature has accepted a `pbo=None` keyword since migration 028 (`database.py:711`) and its INSERT already wires the column. Result: `autotune_runs.pbo` was NULL in 40/40 live rows (audit #118 Finding T1) — a persistence bug, not a computation bug; the live veto decision itself was never wrong.

**The fix:** a one-line `pbo=_pbo_value` addition at the `save_autotune_run(...)` call site — the SAME local already in scope from the gate call a few lines above, never re-derived. Zero change to `math_engine.compute_pbo`, the veto decision, the acceptance-gate call, or `database.py` (the accessor and INSERT were already correct). Forward-only: legacy pre-fix rows remain `NULL`, matching the same no-retroactive-backfill posture as "s_count Wiring" above — no historical-row repair was attempted or is planned.

**Tests:** `tests/autotuner/test_pbo_run_autotuner_persistence.py` — an integration test at the `run_autotuner()` level (not another accessor-level test; `test_pbo_migration_028.py`'s accessor round-trip was already correct and is precisely why this call-site gap went undetected). Asserts persisted `autotune_runs.pbo` equals the value `acceptance_gate.evaluate_acceptance_gate` itself consumed (identity check via a gate spy, `abs=1e-12` SQLite-REAL-round-trip tolerance only), plus a negative pin that `pbo=None` still persists as SQL `NULL` when PBO was never computed. See `DE-AUDIT-BL1-001` in `DECISIONS.md`.

### oos_alpha Sum-Convention Comment (BL-11, `DE-AUDIT-BL5-12-001`, audit #118 Finding T6, 2026-08-05)

**The gap:** `total_guard_alpha` (`run_simulation`, accumulation starts `:1958` at this doc pass) SUMS guard-alpha across every triggered OOS day; `avg_oos_alpha = oos_alpha / test_days_count` (`:3043`) exists specifically to un-inflate it for human reading. This convention is why raw `oos_alpha` values like -743%/-581% appear in the `autotune_runs` DB and logs -- a sum artifact, not corruption -- but it was easy to misread without a comment at the point of definition.

**The fix:** a one-line comment at the `total_guard_alpha = 0.0` accumulation start explains the SUM convention and cross-references `avg_oos_alpha` by name/line as the un-inflated per-day companion. The SAME annotation (a prose sentence, not just a parameter-list mention) is added to `database.py`'s `save_autotune_run` docstring so a reader inspecting the DB schema directly also sees the warning -- see `docs/generated/database.md`. Comment-only, zero behavioral change.

**AC-19 (surface disclosure, team-lead-ruled deviation from the plan's literal wording):** the plan's AC-19 called for surfacing `avg_oos_alpha` "alongside" any operator-facing surface that displays the raw `oos_alpha` sum. A grep before assuming confirmed exactly one such surface exists -- `static/ai_advisor.js`'s per-symphony assessment block (`'OOS alpha: <code>'`) -- but NO `avg_oos_alpha` companion is persisted anywhere to surface alongside it (it is a local print-statement variable in this module, never written to any table). Rather than inventing a new persistence path or display surface out of scope, the letter's INTENT (disclose the convention where the raw number is shown) is satisfied by relabeling the JS literal itself to `'OOS alpha (cumulative sum across triggered days): <code>'` -- see `docs/generated/static_ai_advisor_js.md`.

**Tests:** `tests/autotuner/test_bl11_oos_alpha_sum_convention.py` (comment presence at the accumulation site, cross-reference to `avg_oos_alpha` by name) + `tests/app/test_bl11_ai_advisor_oos_alpha_label_disclosure.py` (the JS relabel, "cumulative"/"sum" language present near the "OOS alpha" text). See `DE-AUDIT-BL5-12-001` in `DECISIONS.md`.
...
| `SIM_EXIT_FRICTION_PCT` | 0.5 | Replay-only trading-friction cost, PERCENTAGE POINTS per round-trip exit (`DE-EXIT-FRICTION-REALIZED-001`, 2026-07-24). Derived from `composer_backtest_client._DEFAULT_SLIPPAGE_PERCENT * RETURN_PCT_TO_FRACTION` (`0.005 * 100.0`). NOT in the Optuna search space; see the Exit-Friction Constant section above. |
...
- `database` — `get_spec_bundle_by_id`, `save_autotune_run` (now called with `pbo=_pbo_value` — BL-1, `DE-AUDIT-BL1-001`, 2026-08-05 — AND `s_count=`, Workstream E), `advisor_ro_query`. **`get_cached_regime_label` is explicitly NEVER called from this module** — forbidden by ruling (`DE-MATH-R1-001` AC-4): it is a live, latest-wins single-row cache and would inject lookahead into the replay if consulted for a historical date.
...
- `synthetic_history` — `generate_synthetic_history`, `_MC_REPLAY_SIMULATION_PATHS` (R1: the AC-6 parity battery shares this MC-path-count config rather than `math_engine`'s 5000-path default); `get_cached_synthetic_history_only`, `utc_to_eastern` (guard-alpha-preconditions: `build_if_held_replay_series`'s cache-hit-only reader and DST-aware date resolution, `DE-GUARD-ALPHA-PRECONDITIONS-001`)
