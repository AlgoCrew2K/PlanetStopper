...
Pinned by `tests/autotuner/test_build_if_held_replay_series.py::TestCacheMissNeverFetches` (accessor-level: cold cache -> `None`, `synthetic_history.fetch_bars` never called) and `tests/app/test_guard_alpha_preconditions_route.py::TestColdCacheNeverFetchesEndToEnd` (route-level, through the real function, not mocked).
