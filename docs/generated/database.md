...
**Source:** `database.py`
**Last updated:** 2026-08-28 (Retirement Approval Lifecycle, Cycle 2d — PR#140 2nd `/code-review` pass fast-follow, AC-3, `DE-RETIRE-POLISH-001`, HEAD `3c773c3b` -- no schema migration this cycle (still 38 migration files, still 35 active `_MIGRATION_FILES` entries; `database.py` diff is +7/-2 net, accessor-body-only). `upsert_retirement_decision`'s `decided_at` `CASE` gains a NEW leading branch, `WHEN excluded.approval_status = 'pending' THEN NULL`, checked BEFORE the Cycle-2c preserve-verbatim branch — a write that transitions an already-decided row BACK to `pending` now resets `decided_at` to `NULL`, so the invariant `pending ⇒ decided_at IS NULL` holds unconditionally (previously, a decided row later written back to `pending` — not a reachable transition via either shipped approve/reject route today, but the accessor itself always supported an arbitrary `approval_status` argument — would incorrectly retain its OLD `decided_at` timestamp instead of clearing it, since the preserve-verbatim branch fired first and unconditionally for any already-non-NULL value regardless of the NEW status being written). Approved/rejected writes are unaffected — still preserve-first, per Cycle 2c AC-5, unchanged. See the updated "Retirement Decisions" section below and `DE-RETIRE-POLISH-001`'s "Cycle 2d" section in `DECISIONS.md`.) Prior: 2026-08-27 (Retirement Approval Lifecycle, Cycle 2c epic-completion, `DE-RETIRE-APPROVAL-001` / implementer-side `DE-RETIRE-POLISH-001` -- no schema migration this cycle (still 38 migration files, still 35 active `_MIGRATION_FILES` entries). Two accessor-level fixes to the Cycle-2b `retirement_decisions` table: AC-5, `upsert_retirement_decision`'s `ON CONFLICT` `decided_at` assignment gains a leading `CASE`-branch that preserves an already-set `decided_at` verbatim on every subsequent write (was: re-stamped on every write landing on `approved`/`rejected`, including an idempotent re-decision); AC-8, the unused singular `get_retirement_decision` accessor is deleted (zero production callers, `get_retirement_decisions` remains the sole read path, now consumed via `app.py`'s relocated `_join_retirement_approval_status()`). See the updated "Retirement Decisions" section below and `DE-RETIRE-APPROVAL-001`'s "Cycle 2c" section in `DECISIONS.md`.) Prior: 2026-08-26 (Retirement Approval Lifecycle, Phase 2 Cycle 2b, `DE-RETIRE-APPROVAL-001` -- new migration 038 `retirement_decisions` (mutable, `candidate_id`-keyed approve/reject status, deliberately the opposite shape from the append-only `advisor_observations` Cycle-2a recommendation rows) + 3 new accessors (`upsert_retirement_decision`/`get_retirement_decisions`/`get_retirement_decision`); see the new "Retirement Decisions" section below and `DE-RETIRE-APPROVAL-001` in `DECISIONS.md`.) Prior: 2026-08-05 (BL-10/BL-11 hygiene documentation, `DE-AUDIT-BL5-12-001` -- the 6 never-wired `autotune_runs` columns gain schema-comment + docstring notes distinguishing 3 provenance tiers; `oos_alpha`'s multi-day-SUM convention is documented at the same docstring site, cross-referencing `autotuner.py`'s `avg_oos_alpha` companion; documentation-only, no migration -- see the updated "Autotune Run Persistence" section below and `DE-AUDIT-BL5-12-001` in `DECISIONS.md`.) Prior: 2026-07-25 (strategy-incubation-gate, `DE-INCUBATION-GATE-001` -- new migration 037 `strategy_incubation`/`incubation_daily` tables + 7 new accessors, see the "Strategy Incubation Ledger" section below. Prior: 2026-07-24 (exit-friction-realized-savings, `DE-EXIT-FRICTION-REALIZED-001` -- new `get_exit_turnover_stats`/`compute_est_annual_friction_drag_pct` accessors in the Exit Trigger Telemetry section below; no schema migration, `exit_triggers` already existed. Prior: 2026-07-14 (branch-integration merge — Frontrunner Builder migration renumbered 033→**034** `frontrunner_proposals` on top of main's migration 033 `candidate_alert_state`; combines: Frontrunner Builder `frontrunner_proposals` table + accessors + `_VALID_DOF_EVIDENCE_SOURCES` addition; candidate-alert cycle migration 033 `candidate_alert_state` — five new accessors, `get_candidate_alert_viewed_marker`/`set_candidate_alert_viewed_marker`/`mark_candidate_alert_viewed`/`get_candidate_alert_new_valid_count`/`get_candidate_alert_last_run`, back the header candidate-alert indicator; prior: Workstream E, advisor-rewire cycle: `save_autotune_run` gains `s_count`; prior: 2026-07-09 DE-PROD-ACCURACY-001: `save_state` sanitizes numpy/non-finite values via `_sanitize_state_for_json` before every write, new `load_latest_shadow_row`/`load_earliest_shadow_row` Stage-1 accessors, `record_exit_trigger` now returns the inserted row id; prior: 2026-07-02 DE-PRISM-NUMERIC-VERIFY-001 `get_latest_market_prism_verification_for_run` accessor; prior: DE-ADVISOR-LATENCY `get_latest_market_lens_cache()`; DE-PRISM-SOURCES-001 `get_latest_market_prism_sources_for_run`)

## Overview

`database.py` is the single write layer for `alphabot_state.db`. It owns schema initialization, 38 numbered migration SQL files (001-038), and every public accessor function. `_MIGRATION_FILES` wires 35 active entries (004-038); migrations 001-003 use a separate bootstrap path. The dashboard uses `get_ro_connection()` for all reads; the engine uses `get_connection()` for writes. The two-DB pattern (state DB here; Optuna studies in a separate DB) is an architecture hard rule — no cross-DB joins in application code.
...
Migrations are listed in `_MIGRATION_FILES` and applied by `run_migrations()`. They are idempotent (tracked in `schema_migrations`). Current highest: **038** (`038_retirement_decisions.sql`), following **037** (`037_strategy_incubation.sql`) and **036** (`036_sleeve_rule_fires.sql`); see `docs/generated/sleeves.md` for the sleeves schema and the "Strategy Incubation Ledger" / "Retirement Decisions" sections below for 037/038.
...
- `038_retirement_decisions.sql` — `retirement_decisions` table (mutable operator approve/reject status for Retirement Recommender candidates, keyed by `candidate_id` UNIQUE). Additive-only, no existing table modified. See the "Retirement Decisions" section below, `docs/generated/advisors_retirement_explainer.md`, `docs/generated/advisors_retirement_checklist.md`, and `DE-RETIRE-APPROVAL-001`.
...
**Dead/unwired columns documented, BL-10 (`DE-AUDIT-BL5-12-001`, audit #118 Finding T5, 2026-08-05):** a 3-tier distinction, correcting the feature plan's own framing (which had lumped the non-`ce_metric`-tier columns together as "actively-used on the port_state table" -- verified false by grepping the whole tree):
1. `ce_metric`/`cvar_feasible`/`lambda_budget` -- truly never wired: no writer or reader anywhere beyond the bare schema column. Schema-comment added at the `CREATE TABLE ... autotune_runs` block.
2. `account_id`/`sortino_sentinel_pct` -- have an ORPHANED writer: `database.record_autotune_run` inserts both, but that function has ZERO callers repo-wide. The production `save_autotune_run` path (this function) never touches them.
...
Wording deliberately avoids "always NULL" anywhere near these columns -- a legacy row could theoretically be non-NULL from a historical accessor no longer in use; the guarantee is "never populated by the CURRENT `save_autotune_run` path," not a blanket claim about every row that has ever existed.

**`oos_alpha` sum-convention documented, BL-11 (`DE-AUDIT-BL5-12-001`, audit #118 Finding T6, 2026-08-05):** the docstring now carries a dedicated prose sentence (distinct from the pre-existing `s_count` paragraph's own unrelated "SUM" mention) stating `oos_alpha` is a multi-day CUMULATIVE SUM of guard-alpha across every triggered OOS day (`autotuner.py`'s `total_guard_alpha` accumulation), not a per-day or annualized figure -- explaining why raw values like -743%/-581% appear in the DB. Cross-references `avg_oos_alpha` (`autotuner.py:3043`, a local print-statement variable, never persisted to this column) as the un-inflated per-day companion. See `docs/generated/autotuner.md`'s matching comment-site note.
...
### Retirement Decisions (migration 038)

Mutable operator approve/reject status for Retirement Recommender candidates — see [advisors/retirement_explainer](advisors_retirement_explainer.md), [advisors/retirement_checklist](advisors_retirement_checklist.md), and `DE-RETIRE-APPROVAL-001` in `DECISIONS.md`. **Deliberately the opposite shape from `advisor_observations`:** Cycle-2a's `RETIREMENT_RECOMMENDATION` rows are append-only history (one row per nightly 03:45 tick, immutable by design); this is ONE mutable row per `candidate_id`, live-joined onto the (possibly re-run, possibly re-flagged) recommendation at render time — never a frozen field on the append-only side. One additive table, no existing schema modified.

**Table `retirement_decisions`:**

| Column | Type | Constraints | Description |
|--------|------|-------------|-------------|
| `id` | INTEGER | PRIMARY KEY AUTOINCREMENT | |
| `candidate_id` | TEXT | NOT NULL UNIQUE | Composer symphony hash — the stable natural join key (matches `retirement_recommender`'s `raw_response.candidate_id`). SQLite auto-creates the backing index for this UNIQUE column; no separate `CREATE INDEX` was added. |
| `sibling_id` | TEXT | NULLable | **Informational only, NOT part of the key** — a candidate's sibling can rotate night to night (a persistently-flagged candidate re-paired with a different symphony), so the decision is about the CANDIDATE's retirement, not the pair. **No production writer this cycle** — both approve/reject routes call `upsert_retirement_decision(candidate_id, approval_status=...)` with `sibling_id` omitted (defaults to `None`, which the UPSERT's `COALESCE` then treats as "preserve whatever is already stored" — see the accessor below). The column/param exist per the schema's reserved-for-future-use shape; nothing in this cycle populates it. |
| `approval_status` | TEXT | NOT NULL DEFAULT `'pending'` | `pending` \| `approved` \| `rejected` — validated in **Python** (`_VALID_RETIREMENT_APPROVAL_STATUSES`), not a SQL `CHECK` constraint, same convention as `strategy_incubation.status`. |
| `decided_at` | TEXT | NULLable | **Invariant (Cycle 2d, AC-3): `pending ⇒ decided_at IS NULL`, unconditionally.** `NULL` while `pending`. Stamped `datetime('now')` on a candidate's FIRST real decision (existing `decided_at` is still `NULL`) when the written `approval_status` is `approved` or `rejected`. **As of Cycle 2c (AC-5, `DE-RETIRE-APPROVAL-001`/`DE-RETIRE-POLISH-001`, 2026-08-27), once non-`NULL` it is PRESERVED VERBATIM on every subsequent write that stays `approved`/`rejected`** — an idempotent re-approve/re-reject, or a status transition (e.g. `approved -> rejected -> approved`), never re-stamps it. **As of Cycle 2d (AC-3, `DE-RETIRE-POLISH-001`, 2026-08-28), a write that transitions the row BACK to `pending` resets `decided_at` to `NULL`** — this takes priority over the Cycle-2c preserve-verbatim rule (checked FIRST in the `CASE`), so the invariant above holds even for a previously-decided row. A subsequent re-approve/re-reject after such a reset stamps a genuinely fresh `decided_at` (nothing is left to preserve). `decided_at` otherwise records the ORIGINAL decision time, not the time of the last write. |
| `updated_at` | TEXT | NOT NULL DEFAULT `(datetime('now'))` | Stamped `datetime('now')` on every write, including a re-decision that leaves `approval_status` unchanged. |

No `PRAGMA foreign_keys=ON` anywhere in this codebase (verified against `database.py` and every prior migration) — `sibling_id` is a soft reference only, same convention as migrations 035/036/037. State DB only — zero optimization-DB placement (architecture constraint 3).

#### `upsert_retirement_decision(candidate_id: str, *, approval_status: str, sibling_id: str | None = None) -> bool`

UPSERT keyed by `candidate_id`'s UNIQUE constraint (`INSERT ... ON CONFLICT(candidate_id) DO UPDATE`) — a repeat call for the same `candidate_id` updates the existing row in place, never inserts a second row. This is the ONLY write path for this table; both `POST /ai-advisor/retirement/approve` and `POST /ai-advisor/retirement/reject` call it via the shared `app.py` helper `_dispatch_retirement_decision` (see [app](app.md)).

- **`approval_status`** must be one of `{pending, approved, rejected}` — raises `ValueError` otherwise (a caller bug, mirrors `update_frontrunner_proposal_status`'s `_VALID_PROPOSAL_APPROVAL_STATUSES` pattern) and writes nothing on an invalid value.
- **`sibling_id=None`** (the default) on an update PRESERVES the existing stored value via `COALESCE(excluded.sibling_id, retirement_decisions.sibling_id)` (same pattern as `update_frontrunner_proposal_status`'s `created_symphony_id`); a non-`None` `sibling_id` always overwrites. No production caller passes a non-default `sibling_id` this cycle.
- **`decided_at`** is written by TWO separate expressions depending on whether the row is a fresh `INSERT` or an `ON CONFLICT DO UPDATE` — they are NOT the same `CASE` statement. On the initial `INSERT ... VALUES` clause, a simple 2-branch `CASE WHEN ? IN ('approved', 'rejected') THEN datetime('now') ELSE NULL END` (parameterized on `approval_status`) sets `decided_at` on row creation — unaffected by this or any prior cycle. On the `ON CONFLICT(candidate_id) DO UPDATE` clause (the path every repeat call for an existing `candidate_id` takes), `decided_at` is computed via a **4-branch `CASE`** (as of Cycle 2d, AC-3) on `excluded.approval_status`, in this exact order: **(1, Cycle 2d, new)** `WHEN excluded.approval_status = 'pending' THEN NULL` (a write transitioning the row back to `pending` resets `decided_at` to `NULL` — checked FIRST, unconditionally, so it wins even over a currently-non-`NULL` `decided_at`); **(2, Cycle 2c)** `WHEN retirement_decisions.decided_at IS NOT NULL THEN retirement_decisions.decided_at` (preserve the existing row's value verbatim, for a write that stays `approved`/`rejected`); **(3)** `WHEN excluded.approval_status IN ('approved', 'rejected') THEN datetime('now')` (first real decision on this row); **(4)** `ELSE NULL` — a defensive default only, structurally UNREACHABLE on the `UPDATE` path given `approval_status` is validated to `{pending, approved, rejected}` before this SQL ever executes (a `ValueError` is raised otherwise, see below): branch (1) always catches `pending` and branch (3) always catches `approved`/`rejected` whenever branch (2)'s preserve condition doesn't already fire first, so one of (1)/(2)/(3) always resolves the expression for every value this function can actually be called with. **[CORRECTED, Cycle 2d, AC-3, PR#140 2nd `/code-review` finding 4, `DE-RETIRE-POLISH-001`, 2026-08-28]** through Cycle 2c, branch (1) above did not exist — the `UPDATE`-path expression was the 3-branch `CASE` starting at (2), meaning a row transitioned back to `pending` would incorrectly PRESERVE its old `decided_at` timestamp (branch (2) fired unconditionally for any already-non-`NULL` value, regardless of the newly-written status) instead of clearing it — violating the `pending ⇒ decided_at IS NULL` invariant for exactly that one transition. Neither the shipped `POST /ai-advisor/retirement/approve` nor `/reject` route sends `approval_status="pending"` today (see [app](app.md)'s `_dispatch_retirement_decision`), so this was a latent defect, not one observed in production — but the accessor itself has always accepted an arbitrary validated `approval_status` argument (no route-level restriction to only `approved`/`rejected`), so a future caller (or a direct DB/test call) writing `pending` on an existing row needed the correct invariant. **[CORRECTED, Cycle 2c, AC-5, `DE-RETIRE-APPROVAL-001`/`DE-RETIRE-POLISH-001`, 2026-08-27]** through Cycle 2b, branches (1)+(2) above did not exist at all — the `UPDATE`-path expression was a simpler 2-branch `CASE WHEN excluded.approval_status IN ('approved', 'rejected') THEN datetime('now') ELSE NULL END`, meaning EVERY write that landed on `approved`/`rejected` re-stamped `decided_at` to the current time, including an idempotent re-approve or a reject-then-approve reversal.
- **`updated_at`** is stamped `datetime('now')` unconditionally on every write — unaffected by the AC-5 fix above.

Returns `True` on a successful write. Write path (`get_connection()`), fully parameterized SQL.

#### `get_retirement_decisions() -> list[dict]`

All rows, unfiltered — the sole read path for `_join_retirement_approval_status()` (Cycle 2c, AC-6 — relocated inside `app.py`'s `_fetch_retirement_recommendations()`, shared by both the render-time panel and `GET /api/retirement-recommendations`; see [app](app.md)). Through Cycle 2b this was read directly inside `ai_advisor_tab()`'s own render code instead — same accessor, different call site. Read path (`get_ro_connection()`, architecture constraint 5). Never raises: an empty table, a missing table (a DB that predates migration 038), or any other read failure degrades to `[]`.

**`get_retirement_decision(candidate_id: str) -> dict | None` — REMOVED, Cycle 2c, AC-8, `DE-RETIRE-APPROVAL-001`/`DE-RETIRE-POLISH-001`, 2026-08-27.** This singular single-row-lookup accessor (through Cycle 2b: read path via `get_ro_connection()`, never raised, returned `None` on any read failure or unknown `candidate_id`) had zero production callers since it was first added in Cycle 2b — the live-join has always read via `get_retirement_decisions()`'s bulk fetch instead (see above). Verified via a repo-wide grep for `get_retirement_decision(` (excluding the plural) before deletion. `tests/database/test_038_retirement_decisions.py`'s coverage of it was repointed onto a local test-only `_find_decision()` helper ahead of the deletion, in the same cycle's RED commit — no test coverage was silently dropped, the production function is simply gone.
