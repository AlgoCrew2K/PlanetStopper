...
| `decided_at` | TEXT | NULLable | `NULL` while `pending`; stamped `datetime('now')` whenever the WRITTEN `approval_status` is `approved` or `rejected` (both on initial insert and on a later re-decision). |
...
- **`decided_at`** is computed in-SQL via a `CASE WHEN ... IN ('approved', 'rejected') THEN datetime('now') ELSE NULL END` expression on both the INSERT values and the UPDATE's `excluded.approval_status` — so a decision reverted to `pending` (not a reachable transition via either shipped route today, but the accessor itself supports it) would correctly clear `decided_at` back to `NULL` rather than leaving a stale timestamp.
- **`updated_at`** is stamped `datetime('now')` unconditionally on every write.

Returns `True` on a successful write. Write path (`get_connection()`), fully parameterized SQL.

#### `get_retirement_decisions() -> list[dict]`

All rows, unfiltered — the sole read path for the `ai_advisor_tab()` AC-4 render-time live-join (see [app](app.md)). Read path (`get_ro_connection()`, architecture constraint 5). Never raises: an empty table, a missing table (a DB that predates migration 038), or any other read failure degrades to `[]`.

#### `get_retirement_decision(candidate_id: str) -> dict | None`

Single-row lookup by `candidate_id`. Read path (`get_ro_connection()`). Never raises: an unknown `candidate_id`, a missing table, or any other read failure returns `None`. No production caller this cycle (the live-join reads via `get_retirement_decisions()`'s bulk fetch instead) — provided for interface completeness/future use, mirroring several other single-row accessors in this file.
