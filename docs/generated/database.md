...
| `decided_at` | TEXT | NULLable | `NULL` while `pending`. Stamped `datetime('now')` on a candidate's FIRST real decision (existing `decided_at` is still `NULL`) when the written `approval_status` is `approved` or `rejected`. **As of Cycle 2c (AC-5, `DE-RETIRE-APPROVAL-001`/`DE-RETIRE-POLISH-001`, 2026-08-27), once non-`NULL` it is PRESERVED VERBATIM on every subsequent write** — an idempotent re-approve/re-reject, or a status transition (e.g. `approved -> rejected -> approved`), never re-stamps it. `decided_at` records the ORIGINAL decision time, not the time of the last write (was: re-stamped on every write where the written status was `approved`/`rejected`, through Cycle 2b). |
...
- **`decided_at`** (Cycle 2c, AC-5) is computed in-SQL via a 3-branch `CASE` on the UPDATE's `excluded.approval_status`: `WHEN retirement_decisions.decided_at IS NOT NULL THEN retirement_decisions.decided_at` (preserve the existing row's value verbatim — checked FIRST, so it wins over the other two branches whenever it applies) `WHEN excluded.approval_status IN ('approved', 'rejected') THEN datetime('now')` (first real decision) `ELSE NULL` (a decision reverted to `pending` — not a reachable transition via either shipped route today, but the accessor itself supports it — correctly clears `decided_at` back to `NULL` rather than leaving a stale timestamp, same as before). **[CORRECTED, Cycle 2c, AC-5, `DE-RETIRE-APPROVAL-001`/`DE-RETIRE-POLISH-001`, 2026-08-27]** through Cycle 2b, this expression was a simpler 2-branch `CASE WHEN excluded.approval_status IN ('approved', 'rejected') THEN datetime('now') ELSE NULL END`, with no preserve-existing-value branch at all — meaning EVERY write that landed on `approved`/`rejected` re-stamped `decided_at` to the current time, including an idempotent re-approve or a reject-then-approve reversal. The new leading `CASE` branch is the fix; the reached-`pending` `ELSE NULL` behavior is unchanged.
