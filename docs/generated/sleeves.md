...
**Last updated:** 2026-07-08 (P1 GREEN tip `0c5c4df`, review-approved; P2 GREEN tip `ef34848`, review-approved; P3 GREEN tip `9d8e46c` — 477 passed / 0 failed across `tests/sleeves/` (P1+P2+P3) + `test_p3_sleeves_engine_wiring.py` + all 5 P1 containment canaries + `test_main_pipeline.py`, ruff clean; includes s3-review's BLOCK 1+2 fix, see [Notes from the P3 review cycle](#notes-from-the-p3-review-cycle) below. **Two further points are open as of this doc update, neither yet fixed:** (1) a THIRD BLOCK finding (task #31, pending, not yet RED/GREEN) — `reconcile_sleeve_or_pause` compares one sleeve's own cash against the WHOLE-ACCOUNT broker cash, which would false-pause every sleeve once more than one sleeve shares the account; PM ruling recorded (aggregate one-sided cash invariant, per-symbol position aggregation across sleeves) but not yet implemented; (2) a non-blocking open point — `arm_sleeve_live` has no prior-PAPER/track-record check, tracked in `DECISIONS.md` `DE-SLEEVES-P3-001` for the operator/PM to rule on)

## Overview

A managed sleeve is a bounded slice of the operator's own Alpaca account (paper today; live only once live keys are provisioned) governed by operator-authored rules. P1 shipped the infrastructure a rule engine acts through: the single Alpaca order client, envelope clamping, risk-based sizing, cash/position ledger accounting, and broker-truth reconciliation. P2 builds the rule engine itself (`sleeves/rules/`): JSON rule-doc validation, a closed sense registry, fail-safe condition-tree evaluation, a DB-backed pacing/episode-latch state machine, action dispatch, and the tick orchestrator. Every rule is born in SHADOW (record-only fires, executes nothing); the runner/actions layer also structurally supports a non-SHADOW (armed) path that reaches the P1 order client only through `envelope.clamp`. **Resolved in P3:** the `/api/sleeves*` route surface, both arming ceremonies (SHADOW→PAPER's shadow-fire gate and PAPER→LIVE's panic-flow ceremony), the dashboard panel, and the `alpha_bot_execution.main()` engine hook are now built — a rule really can reach PAPER mode and place a real paper-account order. See [Managed Sleeves P2: the rule engine](#managed-sleeves-p2-the-rule-engine) and [Managed Sleeves P3: engine wiring and surfaces](#managed-sleeves-p3-engine-wiring-and-surfaces) below.
...
2. **`reconcile_sleeve_or_pause`** — also runs for every sleeve not already `PAUSED_RECONCILIATION` coming into this tick (an operator clears a standing pause via a dedicated route, out of this module's concern; auto-retrying every tick would otherwise hammer the broker on a standing breach). Reconstructs the sleeve's ledger from its full order+fill history (`sleeves.ledger.reconstruct_from_history`), fetches broker-truth positions/account, and calls `sleeves.reconciliation.reconcile_sleeve`. A broker API failure on either the positions or account fetch is ITSELF treated as a `PAUSED_RECONCILIATION` breach (fail-closed) — "the call returned nothing" is never read as "the broker confirms zero positions/cash," which could silently mask a real drift behind a transient outage. On breach, persists `PAUSED_RECONCILIATION` onto the sleeve row and best-effort posts a Discord alert; skips rule evaluation entirely for that sleeve this tick. A SHADOW sleeve that breaches transitions to `PAUSED_RECONCILIATION` exactly like any other status — SHADOW is not a drift-detection exemption.

   **Open point (BLOCK, tracked, task #31, NOT yet fixed as of this doc update):** as currently committed, this function passes `sleeve_cash_usd=ledger_state.cash_usd` (one sleeve's own ledger cash) against `broker_cash_usd` read straight off the WHOLE Alpaca account (`alpaca_orders.get_account`'s `"cash"` field is account-level, not per-sleeve). Under a single-sleeve reality this is correct by coincidence; the moment a second sleeve shares the same Alpaca account, every sleeve's own cash is a fraction of total account cash, so this comparison would false-pause all of them every tick. PM ruling on record: replace with a one-sided aggregate cash invariant (`Σ sleeve cash ≤ account cash + tolerance`; an aggregate breach pauses ALL sleeves) and aggregate per-symbol position comparison across sleeves (an unattributed broker position — held by neither ledger — is ignored rather than flagged). RED is assigned to `s3-test-writer`, GREEN to `s3-engine`; this section will be corrected in place once that lands.
...
**Known limitation (tracked, non-blocking):** `closes_by_symbol`/`fred_cache` are passed as empty dicts to `evaluate_rules` — no historical daily-bar or FRED-cache accessor is wired into the engine tick yet. Safe by construction, not a silent gap: `sleeves.rules.senses`/`conditions` fail closed on missing sense data (not fireable, no fire recorded), so a rule referencing a daily-bar or FRED sense simply never fires until a real data source lands in a follow-up cycle. A DEFENSIVE/ENTRY rule that only senses `sleeve_status`/`sleeve_equity_usd`/`position_qty`/time-of-day is unaffected.
...
All nine routes are CSRF-protected via the existing global `before_request` hook and session-auth via the existing global auth gate — no per-route code needed for either, matching the rest of `app.py`'s route conventions.
...
| `POST /api/sleeves/<id>/rules/<rule_id>/arm` | `arm_sleeve_rule` | SHADOW→PAPER (AC-13) — rejected without ≥1 recorded `mode_at_fire == "SHADOW"` fire for that rule (no arm-on-faith); never touches `SLEEVE_LIVE_EXECUTION`/`ALPACA_LIVE_*`. |
...
- **`_build_sleeves_panel_context() -> list[dict]`** — per-sleeve `{id, name, status, capital_usd, rules: [{id, name, mode, today_fires, lifetime_fires}]}`. `today_fires`/`lifetime_fires` are read straight from `sleeve_rule_fires` (via `database.get_sleeve_rule_fires`) — **real per-rule P&L attribution is not yet wired** (no fill-attribution/churn-brake machinery exists anywhere in the codebase yet), so the panel shows fire counts only; a fabricated P&L number would be worse than omitting it.
...
`templates/index.html` renders a standalone `data-testid="sleeves-panel"` section (existing light-theme card styling, `var(--studio-*)` tokens only — no new CSS framework, no dark-theme classes) with one `data-testid="sleeve-card"` per sleeve, a `data-testid="sleeve-status-badge"` that renders the raw status string verbatim (works for every documented status plus an arbitrary/seeded value like `"stale"`, with no hardcoded enum branching), the `data-testid="atlas-cache-health-badge"`, a per-rule `data-testid="sleeve-arm-control"` ("Arm Paper" button, shown only when `rule.mode == 'SHADOW'`), and a per-sleeve `data-testid="sleeve-disarm-control"` (disabled when the sleeve is already `SHADOW`). Sleeve/rule names render through plain Jinja autoescaping (no `|safe`) — pinned by an XSS RED test. **`arm-live` and envelope-widen are deliberately NOT wired to a one-click control** — those ceremonies need a dedicated confirm-phrase modal (a follow-up UI pass); only SHADOW→PAPER arm and disarm get real buttons in P3. No sleeve-creation UI exists yet either (no committed RED requires it). The panel is server-rendered on each full page load like the rest of the dashboard — no new client-side polling interval.

`static/index.js` gains `armSleeveRuleToPaper(sleeveId, ruleId, btn)` and `disarmSleeve(sleeveId, btn)`, appended inside the existing IIFE so both reuse the module's already-fetched `_csrfToken` rather than duplicating the CSRF-token fetch — each confirms via `window.confirm`, POSTs to the corresponding route, and does a full `location.reload()` on success (no partial DOM patch).
...
`s3-review`'s post-GREEN pass over the engine wiring (`d7c8bb5`) found two BLOCK-level gaps and one non-blocking open point; a third BLOCK-level gap (shared-account reconciliation semantics) surfaced later from `s3-ux`'s live visual-gate testing and is still open:
...
- **BLOCK 3 (open, task #31, not yet RED/GREEN as of this doc update) — shared-account reconciliation semantics.** `s3-ux`'s live visual-gate testing (task #25) found `reconcile_sleeve_or_pause` comparing one sleeve's own cash against the whole Alpaca account's cash — correct only by coincidence when exactly one sleeve exists; under real multi-sleeve use every sleeve would false-pause every tick. PM ruling on record (see above): a one-sided aggregate cash invariant (`Σ sleeve cash ≤ account cash + tolerance`, aggregate breach pauses ALL sleeves) plus per-symbol position aggregation across sleeves (an unattributed broker position is ignored, not flagged). RED assigned to `s3-test-writer` (multi-sleeve/float/external-position scenarios + fixture rewrite), GREEN to `s3-engine`. This section (and the `reconcile_sleeve_or_pause` description above) will be corrected in place once it lands.
