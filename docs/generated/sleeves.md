...
**Last updated:** 2026-07-08 (P1 GREEN tip `0c5c4df`, review-approved; P2 GREEN tip `ef34848`, review-approved; P3 GREEN tip `f8ca848` — includes s3-review's BLOCK 1+2 fix (`9d8e46c`), the shared-account reconciliation redesign (BLOCK 3, `c178ee2`), the AC-16 panel-rendering fixes (status-badge distinctness, ledger-cash/per-rule-P&L, delete control — `821b385`, cursor-specificity residual `cd9a970`), and the delete route's non-terminal-order-refusal fix (s3-review's last BLOCK, `f8ca848`), ruff clean throughout. See [Notes from the P3 review cycle](#notes-from-the-p3-review-cycle) below. **One non-blocking open point remains:** `arm_sleeve_live` has no prior-PAPER/track-record check, tracked in `DECISIONS.md` `DE-SLEEVES-P3-001`'s "Operator questions for PR review" list for the operator/PM to rule on)
...
- **`_build_sleeves_panel_context() -> list[dict]`** — per-sleeve `{id, name, status, status_badge_class, capital_usd, ledger_cash_usd, rules: [{id, name, mode, today_fires, lifetime_fires, realized_pnl_usd}]}`.
  ...
  - `ledger_cash_usd` and each rule's `realized_pnl_usd` (task #32 findings #2/#3) are now real figures, not placeholders — both derived with ZERO new schema by folding `sleeve_orders`/`sleeve_fills` through `sleeves.ledger.reconstruct_from_history`: sleeve-wide for the ledger-cash figure, filtered to `sleeve_orders.rule_id` (already present) for one rule's own attributed realized P&L. **The "vs broker truth" signal is still the existing `sleeve.status` column** (`PAUSED_RECONCILIATION` already means "known mismatch") — persisting a raw broker-truth cash figure for a direct side-by-side comparison would need a schema migration and stays a separate, explicit open point (operator-questions item #3 in `DECISIONS.md` `DE-SLEEVES-P3-001`), not silently assumed in scope here.
