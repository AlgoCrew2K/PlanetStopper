...
**Last updated:** 2026-08-26 (new module, Phase 2 Cycle 2b, `DE-RETIRE-APPROVAL-001`; corrected same-day for the PR #139 review's F8 finding -- the "exactly" mirror claim vs. `advisor_chat.explain_artifact` was inaccurate, see the "Pattern" note below)
...
**Pattern: shares `advisors/advisor_chat.py`'s `explain_artifact` client factory seam (`ai_advisor._build_client()`), plain-text (not tool-use) `client.messages.create(...)` call shape, response text-block extraction, and D-1 graceful-degradation contract** -- a deliberate reuse of an already-established pattern in this codebase, not a new one.

**[CORRECTED, PR #139 review F8, 2026-08-26] NOT an exact mirror.** `explain_artifact` additionally calls `validate_artifact(artifact)` as a Layer-0 defense-in-depth re-validation, truncating any string field exceeding `advisor_chat.CHAT_ARTIFACT_MAX_FIELD_VALUE_CHARS=500` before the artifact reaches the LLM prompt (the fix for that module's own "a 100k-char `rules_text` would otherwise reach the LLM prompt verbatim" finding). `explain_recommendation`/`_build_explain_messages` has NO equivalent cap -- `json.dumps(recommendation, default=str, indent=2)` is called on the whole dict with no length bound. PM ruling: ACCEPT, no code change to add a cap -- the retirement `raw_response` schema has no open-ended, operator-controlled string field analogous to `rules_text` (every field is a short id, a float, a boolean, the fixed `basis_label`, or `explanation` itself, bounded to `_EXPLAINER_MAX_TOKENS=512` tokens by this module's own prior LLM call) -- the BUG-1 attack shape (an arbitrarily long operator-authored string reaching the prompt unbounded) has no analog here. The module's own docstring is softened to match (no longer claims "exactly"). See `DE-RETIRE-APPROVAL-001`'s ACCEPT-rulings section in `DECISIONS.md`.
