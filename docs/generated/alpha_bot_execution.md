...
**Known architectural gap found by PR #125 review (F5(d), 2026-08-13) — read-scope mismatch between the EOD marker-reset loop and the snapshot builder, both in this module.** The reset loop at `:1037` (`for account, symphonies in symphony_data_cache.items(): for sym in symphonies: ...`) only reaches symphonies present in THIS CYCLE's freshly-fetched `symphony_data_cache` -- its `if s_id in bot_state:` guard (`:1040`) never protects against the REVERSE case, a symphony absent from `symphony_data_cache` entirely (a Composer fetch failure at the 16:00-16:05 ET EOD run). The snapshot builder's symphony list, `symphony_keys_eod` (`:1066`, reused at `:1120`), is computed independently as `[k for k in bot_state if isinstance(bot_state[k], dict)]` -- ALL of `bot_state`, with no `symphony_data_cache` membership check at all. **Consequence:** a symphony whose Composer fetch fails at the EOD run is silently excluded from the marker-reset loop (never gets `current_return_is_reconstructed=False` re-stamped) but is NOT excluded from the snapshot builder -- its stale `bot_state` entry, potentially still carrying `current_return_is_reconstructed=True` and a reconstructed value from an earlier intraday cycle, is spread verbatim into `_eod_accounts_map` (`:1127`) and written into `last_market_close_snapshot.accounts_map` (`:1153`). **This module carries ZERO diff for this finding (AC-7 held) -- it is documented here for architectural completeness, not as a defect fixed in this file.** The team-lead's scope ruling classified this as a RENDER-LAYER fix: `app.py`'s frozen-branch rendering (previously hardcoded to ignore the marker regardless of snapshot content) now honestly reads whatever marker value the snapshot actually carries -- closing the SILENT SWALLOW of a reconstructed value, not the underlying `symphony_data_cache`-vs-`bot_state` scope asymmetry documented here, which would require an engine-layer fix out of this cycle's scope. See `DE-HELD-BASIS-001`'s "Revise 1" section in `DECISIONS.md` and `docs/generated/app.md`.
