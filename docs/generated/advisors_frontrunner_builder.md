...
### `splice_candidate_into_symphony(incumbent_symphony: dict, incumbent_cascade, candidate: dict, *, compiled_tree: dict | None = None) -> dict | None`
...
**`compiled_tree` (new, F7, `DE-FR-SIMPLIFY-001` Revise 3, keyword-only, default `None`).** The already-compiled Composer tree for `candidate` (`GenerationResult.compiled_tree`), when available — the PREFERRED input, unwrapped via `_unwrap_single_compiled_child` with no compile call at all. Same invariant as `_count_overlay_node_count`'s own `compiled_tree` reuse (folded in earlier this cycle at `f2611ee1`): `generate_candidate_overlay` already compiled `candidate` once, so reusing that result here avoids what had been a THIRD redundant compile-plus-validate pass on the successful-generation happy path. Falls back to compiling `candidate` fresh (raw DSL, `"kind"` key) or accepting an already-compiled node (`"step"` key) as-is only when `compiled_tree` is absent — `_run_build_for_symphony`'s real call site always supplies it (`compiled_tree=result.compiled_tree`), so the fallback paths are second-layer defense on the current call graph, not the normal path.

**Returns:** the full spliced symphony dict, or `None` on any structural failure (node not found, compile failure, validation errors, or — F7 — `compiled_tree` present but failing to unwrap to a single node). Never raises.

---

## SIMPLIFY-Path Node-Count Threading (`DE-FR-SIMPLIFY-001`)

`frontrunner_acceptance.evaluate_calmar_acceptance`'s SIMPLIFY path used to compare whole-symphony node counts (`incumbent_node_count`/`candidate_node_count`) — structurally unreachable, since a spliced candidate is always ~98-100% of the incumbent's size (found by PR #126's `/code-review`; original fix 2026-08-24). This module supplies the real delta-scoped operands the corrected clause consumes.

**Revise 3 (2026-08-24, PR #128's `/code-review`, CRITICAL F1).** The original fix's operands were STILL not honest — `replaced_cascade_node_count` counted the whole detector-compacted cascade INCLUDING its synthetic stub padding, which could inflate the denominator arbitrarily and invert SIMPLIFY a second time (unreachable → admitting oversized overlays). Every helper below now counts SIGNAL-LOGIC-ONLY — condition + real fire/then branch, excluding the placeholder-else/stub-padded continuation on each side. See [advisors_frontrunner_acceptance.md](advisors_frontrunner_acceptance.md)'s "SIMPLIFY-Path Semantics" section for the full defect/fix account and the acceptance-side gate logic (RULING 1/2/3).

### `STUBBED_CORE_CONTINUATION_TICKER` (constant, promoted public)

The synthetic placeholder-leaf ticker `frontrunner_detector._build_cascade_overlay` stamps onto the stub-padded continuation/core branch it replaces (never real tree content) — value `"_STUBBED_CORE_CONTINUATION"`. Lives in `advisors/frontrunner_detector.py`, PUBLIC there (not underscore-prefixed) specifically so this module can import it rather than duplicate the literal — a detector-side rename can never silently break either this module's `watched_tickers` exclusion (pre-existing) or the new `_contains_stub_marker` stub-branch identification (Revise 3) both of which depend on matching this exact string. This module's OWN prior local duplicate literal, `_STUBBED_CONTINUATION_TICKER`, is deleted in favor of the import.

### `_PLAN_ID_GENERATION` / `_PLAN_ID_OVERLAY_NODE_COUNT` / `_PLAN_ID_SPLICE_CANDIDATE` (constants, F13)

The three distinct `plan_id` markers this module stamps onto its own `plan_tree_compiler.compile_plan` calls — one per consumer (`generate_candidate_overlay`'s real generation compile, `_count_overlay_node_count`'s fallback node-counting compile, `splice_candidate_into_symphony`'s own fallback compile). Previously three inline string literals; named here so a test asserting against the literal markers keeps working unchanged while production code has a single source of truth per marker.

### `_unwrap_single_compiled_child(compiled_root) -> dict | None`

Private. Returns the sole top-level child of a compiled `plan_tree_compiler`-shaped root (the raw compiled `if`/`if_compound` node), or `None` if the root isn't shaped as exactly one child. The same unwrap `splice_candidate_into_symphony` performs on its own `compile_plan` result (and, since F7, on a directly-supplied `compiled_tree` too) — factored out so both it and `_count_overlay_node_count` apply it identically to either an already-compiled tree they were handed or one they compiled themselves.

**F8 hardening (Revise 3).** A strict dict-or-None contract for ANY input shape, never raises — `children` must genuinely be a `list` of length 1, and the sole entry must itself be a `dict`, or this returns `None`. Never coerces a non-list `children` via truthy fallback (the pre-Revise-3 body used `compiled_root.get("children") or []`, which silently treated e.g. a falsy-but-present non-list value the same as absent) — a caller passing a malformed shape now gets an honest `None`, never an accidental non-dict return value.

### `_contains_stub_marker(node) -> bool`

**New, Revise 3, RULING 1.** Private, recursive. `True` if `node` (or anything nested under it) is one of `frontrunner_detector._build_cascade_overlay`'s synthetic `STUBBED_CORE_CONTINUATION_TICKER` placeholder leaves. The ONLY reliable way to identify the stub-padded continuation branch from OUTSIDE the detector — position and `is-else-condition?` are both unreliable there (the detector's `_compact_if_node` picks fire/continuation by SIZE, not direction; either original branch can end up on either side). Never raises on a well-formed dict tree (mirrors `_collect_tickers`'s own shape assumptions).

### `_count_signal_logic_nodes(if_node: dict) -> int | None`

**New, Revise 3, RULING 1 — the CRITICAL fix.** `1 +` the node count of whichever if-child is the real fire/then branch, EXCLUDING the other if-child (continuation/else) entirely — never the whole stub-padded/placeholder-else branch. This is the fix for PR #128's CRITICAL finding: counting the whole compacted cascade subtree counted core-sized stub padding as "replaced logic," inverting SIMPLIFY from structurally unreachable to admitting overlays LARGER than the signal logic they actually replace.

**Identifies which if-child to exclude, in priority order:**
1. A `_contains_stub_marker` search — reliable on detector-compacted cascade output (`cascade.overlay_tree`), where fire/continuation is picked by SIZE not direction.
2. Falling back to `is-else-condition? is True` — reliable on a freshly compiled overlay CANDIDATE (never detector-compacted, no stub marker ever exists there), mirroring the existing production `_find_terminal_else_child`'s identical "find the placeholder else slot" logic.

**Fail-closed contract (team-lead ruling, Revise 3).** Returns `None` — NEVER falls back to a whole-tree count — when `if_node` isn't shaped as exactly two dict if-children. An unidentifiable shape means the fire/continuation split cannot be honestly performed, and a whole-tree fallback would be ANTI-conservative: it would INFLATE the cascade-side denominator, making SIMPLIFY easier to fire on exactly the inputs this function understands least — symmetric with every other fail-closed guard in this module and in `frontrunner_acceptance.evaluate_calmar_acceptance`.

**Consumed by two call sites** (both threading operands into the acceptance call): `_count_overlay_node_count` (below, applied to the overlay side at all three tiers) and `_run_build_for_symphony`'s threading site (applied directly to the cascade side).

### `_count_overlay_node_count(candidate: dict | None, compiled_tree: dict | None = None) -> int | None`

Returns the SIGNAL-LOGIC-ONLY node count of a build-plan-DSL overlay candidate — the AC-2 `overlay_node_count` operand — or `None` if it cannot be honestly determined. Never raises (D-1).

**Three-tier resolution, in order — RULING 1 (Revise 3): every tier now applies `_count_signal_logic_nodes` to its unwrapped node, never the raw `_count_tree_nodes` whole-subtree count the original fix used:**
1. **`compiled_tree` (preferred).** When the caller already has `GenerationResult.compiled_tree` (from `generate_candidate_overlay`'s own earlier `plan_tree_compiler.compile_plan` call), unwrap it via `_unwrap_single_compiled_child`, then apply `_count_signal_logic_nodes` — no redundant second compile. This tier's `compiled_tree` reuse exists because of an earlier fps-reviewer INFO finding folded in mid-cycle before Revise 3 (team-lead preference: "fold the fix in now if small"); a dedicated regression test (`test_overlay_node_count_reuses_the_already_compiled_tree_no_redundant_compile`) pins it.
2. **Already-step-shaped candidate (rare — F11, "unreachable under current call graph, second-layer defense," mirrors `_resolve_admission_mdd_baseline`'s established project convention).** If `compiled_tree` is absent but `candidate` itself already carries a `"step"` key (the same shape `splice_candidate_into_symphony` accepts as pre-compiled), apply `_count_signal_logic_nodes` directly. The sole production caller, `_run_build_for_symphony`, always has `compiled_tree` populated alongside a non-`None` `candidate`, so this tier cannot fire via the real call path today.
3. **Fresh pure-compile (fallback).** Wraps `candidate` in a minimal plan envelope (`plan_id=_PLAN_ID_OVERLAY_NODE_COUNT`) and calls `plan_tree_compiler.compile_plan` **without `backtest_fn`** — a pure compile, counting only, never network. Passing `backtest_fn` here would let the compiler's tradeability-repair loop fire live Composer calls and nondeterministically prune tickers, corrupting the count with network I/O and bill spend this counting-only helper must never incur.

**Parameters:**

| Name | Type | Description |
|------|------|--------------|
| `candidate` | `dict \| None` | The build-plan-DSL overlay node (`GenerationResult.candidate`) — used only as the fresh-compile fallback INPUT when `compiled_tree` is unavailable |
| `compiled_tree` | `dict \| None` | The already-compiled Composer tree for `candidate` (`GenerationResult.compiled_tree`), when available — the preferred input |

**Returns:** `int | None`. Never raises — any compile failure, unwrap failure, or unidentifiable if-child shape (per `_count_signal_logic_nodes`'s fail-closed contract) degrades to `None`. Never a fabricated `0`: a real compiled overlay's signal logic always has ≥1 node, so a `0` (were it ever produced) could only mean an upstream failure — treating it as real would let it sail through the acceptance gate's ratio check and fabricate a material-simplification accept.

**A1 logging (Revise 3 addendum).** Every EXPECTED `None`-degradation path (each unwrap failure, each unidentifiable-shape result from `_count_signal_logic_nodes`, the fresh-compile-failure branch) now logs a WARNING with the specific reason — mirroring `_run_build_for_symphony`'s own per-skip logging style. Previously silent (only the outer catch-all `except Exception` logged, and only at DEBUG). A future counting failure must never silently recreate "SIMPLIFY unreachable, zero operator-visible signal" — the ORIGINAL CRITICAL defect's exact survival mode.

### `_run_build_for_symphony`'s threading site

Right before the `splice_candidate_into_symphony`/`_gate_and_accept_candidate` calls, two SIGNAL-LOGIC-ONLY counts are computed from values already in scope in the same loop iteration:

```python
replaced_cascade_node_count = _count_signal_logic_nodes(cascade.overlay_tree)   # RULING 1 — was _count_tree_nodes pre-Revise-3
overlay_node_count = _count_overlay_node_count(result.candidate, result.compiled_tree)
```

`splice_candidate_into_symphony` is also now called with `compiled_tree=result.compiled_tree` (F7). Both counts are threaded through `_gate_and_accept_candidate`'s additive `overlay_node_count`/`replaced_cascade_node_count` keyword-only params (default `None`, so any other caller of `_gate_and_accept_candidate` is unaffected) straight into `evaluate_calmar_acceptance`. Zero diff to `frontrunner_detector.py`'s cascade-detection logic, `backtest_gate_engine.py`, or Calmar math itself (AC-7) — `frontrunner_detector.py` gained only the `STUBBED_CORE_CONTINUATION_TICKER` public-constant promotion (a rename of an existing private literal, not a behavior change).

### A2 addendum — `_gate_and_accept_candidate` persists the operands on EVERY outcome

**New, Revise 3 addendum.** All three of `_gate_and_accept_candidate`'s metrics-dict-construction sites — gate-reject, calmar-reject, and accept — now additively include `overlay_node_count`/`replaced_cascade_node_count` verbatim (both already in scope as this function's own keyword params for the whole function body). Previously only implicitly reconstructable from the acceptance call's inputs; now the admission/rejection BASIS is explicitly part of the persisted audit record for every outcome, not just the accepted path — an accepted or rejected proposal is never indistinguishable from one decided on stale/wrong operands.
...
**RULING 3 completeness fix (`DE-FR-SIMPLIFY-001` Revise 3, F3+F10).** `_count_tree_nodes` previously only walked a node's `children` list — a compound condition's clause list (`make_compound_condition`'s `conditions` field, a DATA field sitting directly on an if-child, not under `children`) was invisible to the walk, so a 2-clause and a 12-clause compound condition with identical then/else content counted as the SAME total (empirically verified: both counted 7 nodes). Fixed by additionally pushing an if-child's `condition` dict field (when present) and each dict entry of a `conditions` list clause (when present) onto the same traversal stack — making clause count genuinely load-bearing on every count this function produces, including `node_count_delta` and (via `_count_signal_logic_nodes`) both SIMPLIFY-path operands. Verified the pre-existing `node_count_delta` display consumers tolerate the resulting (strictly more accurate, never behaviorally different in kind) counts.
...
- `tests/advisors/test_frontrunner_simplify_path_wiring.py` (`DE-FR-SIMPLIFY-001`; independently re-collected and re-run at HEAD `70f832b3`, 2026-08-24 — see [advisors_frontrunner_acceptance.md](advisors_frontrunner_acceptance.md)'s Testing section for the exact combined pass count) — builder-side integration: `_run_build_for_symphony` threads real SIGNAL-LOGIC-ONLY overlay/replaced-cascade counts into the acceptance call; a SIMPLIFY reachability proof against REAL detector output (`_build_real_cascade_via_detector`, rebuilt this Revise-3 cycle to run the real `frontrunner_detector.detect_frontrunner_cascades` rather than a hand-built `Cascade` — the earlier hand-built fixture is exactly what let the CRITICAL F1 defect ship undetected, since it bypassed the detector's real stub-padding behavior entirely); `_count_overlay_node_count`'s three tiers (compiled-tree-reuse-no-redundant-compile, fresh-compile fallback, fallback-compile-failure-returns-`None`); the A4 cross-module stub-padding-insensitivity pin (`test_replaced_cascade_signal_logic_count_is_insensitive_to_stub_padding_size`); the redundant-compile regression guard extended to cover `splice_candidate_into_symphony` too (`test_no_redundant_compile_anywhere_in_the_flow_including_splice`, F7); the A1 warning-log regression pin; the A2 metrics-persistence pins across all three `_gate_and_accept_candidate` outcomes. The SIMPLIFY-path acceptance-math truth table itself (the three-gate logic, ratio boundary, fail-closed/finiteness guards) lives in `test_frontrunner_acceptance.py` — see the acceptance module's doc for that count.
