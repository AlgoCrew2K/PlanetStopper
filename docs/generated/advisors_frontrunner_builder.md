...
## SIMPLIFY-Path Node-Count Threading (`DE-FR-SIMPLIFY-001`, 2026-08-24)

`frontrunner_acceptance.evaluate_calmar_acceptance`'s SIMPLIFY path used to compare whole-symphony node counts (`incumbent_node_count`/`candidate_node_count`) — structurally unreachable, since a spliced candidate is always ~98-100% of the incumbent's size (found by PR #126's `/code-review`, see [advisors_frontrunner_acceptance.md](advisors_frontrunner_acceptance.md)'s "SIMPLIFY-Path Delta-Scoping" section for the full defect account). This module supplies the two real delta-scoped operands the corrected clause now consumes.

### `_unwrap_single_compiled_child(compiled_root) -> dict | None`

**New.** Private. Returns the sole top-level child of a compiled `plan_tree_compiler`-shaped root (the raw compiled `if`/`if_compound` node), or `None` if the root isn't shaped as exactly one child. The same unwrap `splice_candidate_into_symphony` performs on its own `compile_plan` result — factored out so `_count_overlay_node_count` can apply it identically to either an already-compiled tree it was handed or one it compiled itself.

### `_count_overlay_node_count(candidate: dict | None, compiled_tree: dict | None = None) -> int | None`

**New.** Private. Returns the compiled node count of a build-plan-DSL overlay candidate — the AC-2 `overlay_node_count` operand — or `None` if it cannot be honestly determined. Never raises (D-1) and never returns a fabricated `0`: a real compiled overlay always has ≥1 node, so a `0` count can only mean the compile itself silently failed — treating it as real would let it sail through the acceptance gate's `overlay <= cascade * ratio` check and fabricate a material-simplification accept.

**Three-tier resolution, in order:**
1. **`compiled_tree` (preferred).** When the caller already has `GenerationResult.compiled_tree` (from `generate_candidate_overlay`'s own earlier `plan_tree_compiler.compile_plan` call), unwrap it via `_unwrap_single_compiled_child` and count directly — no redundant second compile. **This tier exists because of a reviewer finding, folded in mid-cycle:** the first GREEN draft always re-compiled `candidate` from scratch even when a compiled tree was already available, redundantly re-running compile-plus-validate on the successful-generation happy path; `fps-reviewer`'s non-blocking INFO finding was folded in the same cycle (team-lead preference — "fold the fix in now if small"), landing as this three-tier signature (`compiled_tree: dict | None = None`) plus a dedicated regression test (`test_overlay_node_count_reuses_the_already_compiled_tree_no_redundant_compile`) confirming only 2 distinct `plan_id`s are ever compiled per call, not 3.
2. **Already-step-shaped candidate (rare).** If `compiled_tree` is absent but `candidate` itself already carries a `"step"` key (the same shape `splice_candidate_into_symphony` accepts as pre-compiled), count it directly.
3. **Fresh pure-compile (fallback).** Wraps `candidate` in a minimal plan envelope and calls `plan_tree_compiler.compile_plan` **without `backtest_fn`** — a pure compile, counting only, never network. Passing `backtest_fn` here would let the compiler's tradeability-repair loop fire live Composer calls and nondeterministically prune tickers, corrupting the count with network I/O and bill spend this counting-only helper must never incur.
...
**Returns:** `int | None`. Never raises — any compile failure, unexpected shape, or genuine zero-node result degrades to `None`.

### `_run_build_for_symphony`'s threading site

Right before the existing `_gate_and_accept_candidate` call, two counts are computed from values already in scope in the same loop iteration:

```python
replaced_cascade_node_count = _count_tree_nodes(cascade.overlay_tree)   # direct — already raw_value-shaped
overlay_node_count = _count_overlay_node_count(result.candidate, result.compiled_tree)
```

Both are threaded through `_gate_and_accept_candidate`'s new additive `overlay_node_count`/`replaced_cascade_node_count` keyword-only params (default `None`, so any other caller of `_gate_and_accept_candidate` is unaffected) straight into `evaluate_calmar_acceptance`. Zero diff to `frontrunner_detector.py`, splice mechanics, `backtest_gate_engine.py`, or Calmar math itself (AC-7).
...
- `tests/advisors/test_frontrunner_simplify_path_wiring.py` (new, `DE-FR-SIMPLIFY-001`, 2026-08-24, 5 tests — independently re-collected and re-run, see [advisors_frontrunner_acceptance.md](advisors_frontrunner_acceptance.md)'s Testing section) — builder-side integration: `_run_build_for_symphony` threads real overlay/replaced-cascade counts into the acceptance call; a SIMPLIFY reachability proof (small overlay vs large cascade, preserved Calmar — ACCEPTED, impossible pre-fix); `_count_overlay_node_count`'s three tiers (compiled-tree-reuse-no-redundant-compile, fresh-compile fallback, fallback-compile-failure-returns-None). The SIMPLIFY-path acceptance-math truth table itself (ratio boundary, fail-closed guards) lives in `test_frontrunner_acceptance.py` — see the acceptance module's doc for that count.
