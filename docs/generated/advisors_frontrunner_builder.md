...
> Orchestrates the Frontrunner Builder pipeline: detect the incumbent frontrunner overlay, generate a candidate via Fable, splice it into the symphony, independently re-backtest and gate both sides, apply Calmar acceptance, and queue survivors for operator approval. AC-5 signal-gating functions (frontrunner-signals cycle) are built and unit-tested but **NOT YET WIRED** into the generation path — see the caveat in "AC-5: Signal-Gated Generation (functions built, wiring pending)" below.

**Source:** `advisors/frontrunner_builder.py`
**Last updated:** 2026-07-16 (AC-5 signal-gating functions landed GREEN at unit-test level at `bf6f026b` — see "AC-5: Signal-Gated Generation" below and `DE-FR-SIGNALS-001` in `DECISIONS.md`; **wiring into `_run_build_for_symphony` is a separate, currently-open item, "Cluster D," being driven through its own RED→GREEN loop as of this writing — do not read the AC-5 functions' existence as "candidate generation is signal-gated in production" until that lands and is re-reviewed**; prior: Wave-2 UI shipped in-branch at `eb1b612` -- see DE-FRONTRUNNER-002 and "Wave-2 UI (built, 2026-07-11)" below; Wave-1 backend, frreview-APPROVED, P2-1 iterative-traversal hardening landed at `26c1364`)
...
## AC-5: Signal-Gated Generation (functions built, wiring pending — frontrunner-signals cycle, 2026-07-16)

**Status caveat (read first):** the six functions below are implemented, unit-tested (`tests/advisors/test_frontrunner_builder_signal_gating.py`, 55/55 GREEN at the function level), and reviewed as individually correct by fr-review. **None of them are yet called from `_run_build_for_symphony` or any other production code path** (fr-review's Cluster-D finding, `bf6f026b`) — as of this doc revision, candidate generation does NOT actually consume live Atlas signal edge data in production; it still generates structurally-only, exactly as wave-1 did. Wiring is in progress as a separate RED→GREEN cycle. Do not cite this section as "AC-5 ships end-to-end" until the wiring commit lands and is re-reviewed — check `DE-FR-SIGNALS-001` for the current state before relying on this.

Once wired, the intended design is: candidate frontrunner generation consumes live Atlas signal edge data (`docs/generated/advisors_frontrunner_signals.md`) rather than generating structurally-only. Six functions, all in this module per the Architecture doc:
...
- `tests/advisors/test_frontrunner_builder_signal_gating.py` — AC-5 (frontrunner-signals cycle) FUNCTION-LEVEL tests only: positive-edge filtering, Tier-1 remove-key veto, provenance building, per-symphony `signals_unavailable` resolution, the crossover/vs display-format functions' non-collision invariant. **These test the functions in isolation, not their integration into `_run_build_for_symphony`** — see the AC-5 section's status caveat above; there is currently no integration test proving the build path actually calls them, because it doesn't yet.

**PM-authoritative gate run** (quiet window, `-n0`, unique `DB_PATH`, process table checked clear before running) on `26c1364`: **207 passed, 2 skipped, 44.31s** across the 9 frontrunner files + `test_no_live_mongo_guard.py` + `test_community_strats.py` + `test_atlas_cache.py` (+5 vs the prior 202 = the new deep-tree hardening file). The 2 skips are pre-existing/stale (`test_community_strats.py` claiming the module doesn't exist — it does; unrelated to this cycle, not fixed here). Warnings are benign quantstats divide-by-zero on zero-max-drawdown edges. **This cycle's AC-5 addition:** 55/55 own RED tests GREEN at `bf6f026b` at the FUNCTION level (shared commit with the AC-3/AC-4/AC-6 work in `frontrunner_detector.py`/`frontrunner_signals.py`); 110/110 across the full RED batch on the frontrunner-signals branch as of that commit. Cluster-D wiring (integration into the build path) is a separate, currently-open item — see `DE-FR-SIGNALS-001` for its resolution status once it lands, and do not treat the function-level GREEN count as evidence of end-to-end functioning.
...
- `advisors.frontrunner_signals` — CC-2 lazy import inside `resolve_signals_unavailable_marker` (AC-5, this cycle) — but `resolve_signals_unavailable_marker` itself is not yet called from anywhere in the build path (see status caveat above)
