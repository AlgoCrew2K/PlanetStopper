...
> 125-day Alpaca historical fetcher with parallel bar download, file cache, and eligibility guards — feeds the autotuner walk-forward replay.
...
The fetch window is computed to guarantee at least `_REQUIRED_FETCH_TRADING_DAYS = 125 + 39 + 10 = 174` trading days (replay window + MC warmup + buffer). A widen-and-refetch loop (max 3 attempts, 60-day calendar step) handles upstream data gaps. A persistent shortfall raises `HistoryShortfallError`.
...
|----------|-------|-------------|
| `_WALK_FORWARD_TRADING_DAYS` | 125 | Autotuner replay window length |
...
| `_REQUIRED_FETCH_TRADING_DAYS` | 174 | Total trading days the fetch must guarantee |
