...
| `advisors/lens_gdelt` | [advisors_lens_gdelt.md](advisors_lens_gdelt.md) | GDELT 2.0 tone/sentiment producer: timelinetone→normalized AvgTone [-1,1], artlist→citations; bounded 429 retry (BASE=20s, MAX=4, CAP=60s); inter-request 6s sleep; D-1 named labels; honest-availability invariant; never raises; off-execution-path | 2026-06-15 |
...
| `advisors/community_strats` | [advisors_community_strats.md](advisors_community_strats.md) | Weekly-cached captplanet Atlas loader: routes the Atlas read through atlas_cache.cached_pull (bill-protection), validates via symphony_schema, deduplicates by tree-structural SHA-256 hash, sharpe-filters (keeps no-sharpe docs), returns {available,candidates,stats,source}; never-raising D-1 contract; NOT wired into a production route — app.py:3437 passes no community_candidates= kwarg; route-wiring cycle pending operator decision | 2026-06-14 |
...
- **GDELT sentiment lens (gdelt-tone cycle, 2026-06-15):** `advisors/lens_gdelt.py` adds `_fetch_gdelt_sentiment(universe) -> dict`. Bounded 429 retry: `BASE=20.0s`, `MAX=4`, `CAP=60.0s`. D-1: named labels for HTTP failures; `type(exc).__name__` for caught exceptions. `tone is None → available=False`. See `DE-GDELT-001..004` in `DECISIONS.md`.
