...
**Last regenerated:** 2026-06-14 (propose-wiring — advisors/strategy_builder_engine.py community_candidate_infos adapter + MAX_COMMUNITY_CANDIDATES_PER_RUN + community_candidates kwarg on propose_strategies; 39 AC-1..AC-7 tests)
...
- **GDELT tone/sentiment producer (gdelt-tone cycle):** `advisors/lens_gdelt.py` adds `_fetch_gdelt_sentiment(universe) -> dict`. Makes two GETs to GDELT 2.0 DOC API: `timelinetone` for normalized `AvgTone ÷ 100` → `[-1, 1]`, `artlist` for source citations (best-effort). Bounded 429 retry: `BASE=20.0s`, `MAX=4`, `CAP=60.0s`, backoff `min(BASE × 2ⁱ, CAP)`. Inter-request sleep of `6.0s` between the two GETs prevents reusing the per-IP rate-limit window. D-1 contract: named labels (`rate_limited`, `gdelt_fetch_failed`, `no_tone_data`) for HTTP-status failures; `type(exc).__name__` for caught exceptions. Honest-availability invariant: `tone is None ⟹ available is False`. Off-execution-path; lazy-imported by `ai_advisor._build_sentiment_section`. Fixes the prior bug where `entry.get("value")` was read from the series-wrapper `{series, data}` (no `"value"` key at that level) — correct path is `timeline[0]["data"][k]["value"]`. See `DE-GDELT-001..004` in `DECISIONS.md`.
