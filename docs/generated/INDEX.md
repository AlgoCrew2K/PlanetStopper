...
**Last regenerated:** 2026-06-16 (fundamentals-portfolio-fanout — ai_advisor._build_fundamentals_section portfolio fan-out + _fetch_fundamentals_for_ticker helper + _FUNDAMENTALS_PROXY_UNIVERSE company floor; fundamentals lens is now wired, no longer a stub — DE-FUND-001)
...
| `ai_advisor` | [ai_advisor.md](ai_advisor.md) | Claude-backed config advisor: context assembly, per-symphony assessment, structured-output Claude call, safety gates (7-item allowlist, risk-direction check, OOS re-validation), and multi-lens pipeline (technicals + sentiment + derivatives wired with freshness guard + fundamentals wired with portfolio fan-out via _FUNDAMENTALS_PROXY_UNIVERSE; macro stub only) | 2026-06-16 |
