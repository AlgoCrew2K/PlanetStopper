...
**Last regenerated:** 2026-06-14 (community-strats — advisors/community_strats.py weekly-cached captplanet Atlas loader; validate/dedup/sharpe-filter; 38 AC-1..AC-9 tests)
...
|--------|------|-------------|--------------|
| `advisors/community_strats` | [advisors_community_strats.md](advisors_community_strats.md) | Weekly-cached captplanet Atlas loader: routes the Atlas read through atlas_cache.cached_pull (bill-protection), validates via symphony_schema, deduplicates by tree-structural SHA-256 hash, sharpe-filters (keeps no-sharpe docs), returns {available,candidates,stats,source}; never-raising D-1 contract; no production caller yet | 2026-06-14 |
...
- **Community-strats loader (community-strats cycle):** `advisors/community_strats.py` is the first caller of `atlas_cache.cached_pull`. Routes the `captplanet.strategies` Atlas read through the weekly cache (operator bill-protection directive). Validates via `symphony_schema.validate_tree`, deduplicates by tree-structural SHA-256 hash (strip ids → json.dumps sort_keys → sha256; NOT `database.compute_composition_hash`, which hashes `list[str]` symphony IDs for portfolio-set identity), sharpe-filters keeping no-sharpe docs, returns `{available, candidates, stats, source}`. Off-execution-path; advisory-only; no production caller wired yet. See `DE-CS-001` in `DECISIONS.md`.
