...
**Last regenerated:** 2026-06-16 (derivatives-vix-freshness — advisors/lens_options_proxy.py recent-window fetch + freshness guard + stale->available=False contract; also lens-technicals — advisors/lens_technicals.py producer + ai_advisor._build_technicals_section wiring; MA posture/breadth/momentum from Alpaca daily bars via synthetic_history cache)
...
| `ai_advisor` | [ai_advisor.md](ai_advisor.md) | Claude-backed config advisor: context assembly, per-symphony assessment, structured-output Claude call, safety gates (7-item allowlist, risk-direction check, OOS re-validation), and multi-lens pipeline (technicals + sentiment + derivatives wired with freshness guard; macro, fundamentals stubs) | 2026-06-15 |
