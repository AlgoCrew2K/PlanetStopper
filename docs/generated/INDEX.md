...
| `advisors/community_strats` | [advisors_community_strats.md](advisors_community_strats.md) | Weekly-cached captplanet Atlas loader: routes the Atlas read through atlas_cache.cached_pull (bill-protection), validates via symphony_schema, deduplicates by tree-structural SHA-256 hash, sharpe-filters (keeps no-sharpe docs), returns {available,candidates,stats,source}; never-raising D-1 contract; first production caller: propose_strategies via community_candidate_infos | 2026-06-14 |
...
- **Community-strats loader (community-strats cycle):** `advisors/community_strats.py` routes the `captplanet.strategies` Atlas read through the weekly cache (operator bill-protection directive). Validates via `symphony_schema.validate_tree`, deduplicates by tree-structural SHA-256 hash (strip ids → json.dumps sort_keys → sha256; NOT `database.compute_composition_hash`), sharpe-filters keeping no-sharpe docs. First production caller: `propose_strategies` via `community_candidate_infos` adapter (propose-wiring cycle). See `DE-CS-001` in `DECISIONS.md`.
