...
- **Retirement Recommendations panel** (`retDispatchDecision`, Overview tab, Phase 2 Cycle 2b) — operator-initiated approve/reject dispatch for one retirement recommendation card, structurally mirroring `frDispatchProposalAction`'s CSRF/disable-on-submit/dim-card pattern; on success, updates BOTH the confirmation message (colored by the actual action taken) and the card's own status chip in place (`textContent`/`className`), without a page reload.
...
### `window.retDispatchDecision(action, candidateId)` (Overview tab, Retirement Recommendations panel, Phase 2 Cycle 2b, `DE-RETIRE-APPROVAL-001`, 2026-08-26)

Shared approve/reject dispatch for one retirement recommendation card. `action` is `'approve'` or `'reject'`; routes to `POST /ai-advisor/retirement/approve` or `POST /ai-advisor/retirement/reject` respectively with `{ candidate_id: candidateId }`. Structurally mirrors `frDispatchProposalAction` above (same CSRF-token-cache-or-fetch pattern, same disable-on-submit, same dim-the-card-while-in-flight, same restore-on-failure), but is a top-level `window`-exposed function rather than an internal-plus-two-wrappers split, since there is only ever one caller shape (`onclick="retDispatchDecision('approve', this.dataset.candidateId)"` / `'reject'`, both reading `candidateId` off the clicked button's own `data-candidate-id` attribute at the Jinja call site — never a raw string literal embedded in inline JS).

**Card lookup by data attribute, never a built CSS-selector string.** `retDispatchDecision` scans every `[data-testid="retirement-recommendation-card"]` for the one whose approve/reject button carries a matching `data-candidate-id` — `candidateId` (a live value, e.g. a Composer hash) is never interpolated into a selector string.
