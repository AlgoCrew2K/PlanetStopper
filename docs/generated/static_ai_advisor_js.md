...
- **Retirement Recommendations panel** (`retDispatchDecision`, Overview tab, Phase 2 Cycle 2b, clicked-element rework Cycle 2c AC-7) — operator-initiated approve/reject dispatch for one retirement recommendation card, structurally mirroring `frDispatchProposalAction`'s CSRF/disable-on-submit/dim-card pattern; on success, updates BOTH the confirmation message (colored by the actual action taken) and the card's own status chip in place (`textContent`/`className`), without a page reload. Locates its card via the clicked button element's own `.closest(...)` ancestor (Cycle 2c), not a page-wide scan.
...
### `window.retDispatchDecision(action, btnEl)` (Overview tab, Retirement Recommendations panel, Phase 2 Cycle 2b; clicked-element rework Cycle 2c, AC-7, `DE-RETIRE-APPROVAL-001`/`DE-RETIRE-POLISH-001`, 2026-08-27)

Shared approve/reject dispatch for one retirement recommendation card. `action` is `'approve'` or `'reject'`; routes to `POST /ai-advisor/retirement/approve` or `POST /ai-advisor/retirement/reject` respectively with `{ candidate_id: candidateId }`. Structurally mirrors `frDispatchProposalAction` above (same CSRF-token-cache-or-fetch pattern, same disable-on-submit, same dim-the-card-while-in-flight, same restore-on-failure), and is a top-level `window`-exposed function rather than an internal-plus-two-wrappers split, since there is only ever one caller shape: `onclick="retDispatchDecision('approve', this)"` / `'reject'` — the Jinja call site now passes the button element itself (`this`), not a pre-extracted dataset value.

**Card lookup via the clicked element's own ancestor, not a page-wide scan (Cycle 2c, AC-7).** `btnEl` is the button that fired the click. `candidateId = btnEl ? btnEl.dataset.candidateId : null` and `card = btnEl ? btnEl.closest('[data-testid="retirement-recommendation-card"]') : null` are both derived directly from `btnEl` — the direct-element analog of `frDispatchProposalAction`'s unique-id lookup: the clicked button's own ancestor card is always correct regardless of any duplicate/mismatched `data-candidate-id` edge case, unlike the pre-2c implementation. **[SUPERSEDED, Cycle 2c]** through Cycle 2b, `retDispatchDecision(action, candidateId)` instead scanned every `[data-testid="retirement-recommendation-card"]` on the page for the one whose approve/reject button carried a matching `data-candidate-id` string — a page-wide `querySelectorAll` compare-and-match, replaced by the direct `.closest()` lookup above. Neither the old nor the new approach ever interpolates `candidateId` into a built CSS-selector string.

**On success (`{success: true}`):**
- Restores the card's opacity to `1`.
- **F3 (`5655edfd`, PR #139 review remediation):** colors the confirmation message by the ACTUAL action taken — `cssVar('--studio-neg')` on reject, `cssVar('--studio-pos')` on approve — replacing the `.retirement-rec-actions` row's markup with a `<p>` confirmation (`"Approved — reload this page to view the wind-down checklist."` / `"Rejected."`). **Original defect (fixed same-cycle, never shipped to a live release):** the confirmation color was hardcoded to `--studio-pos` (success-green) regardless of `action` — a reject confirmation rendered in the same green as an approve confirmation, visually implying the opposite of what happened.
- **F4 (`5655edfd`, PR #139 review remediation):** updates the card's own status chip (`[data-testid="retirement-rec-status"]`) in place — both `textContent` (capitalized `"Approved"`/`"Rejected"`) and `className` (`retirement-status-chip retirement-status-chip--{approved|rejected}`) — so the chip reflects the new decision immediately. **Original defect (fixed same-cycle):** only `.retirement-rec-actions` was updated on success; the status chip was left showing its pre-decision `"Pending"` value until the next full page reload, sitting inconsistently next to a confirmation message that said otherwise.

**On failure** (`{success: false}`, a non-2xx response, or a thrown error): restores full opacity, re-enables both buttons, and `alert()`s the error (`data.error` when present, else `err.message`, else a generic fallback) — the operator can retry. Both property assignments on the success path (`textContent`/`className`) are plain property assignments, never `innerHTML` — the confirmation `<p>` markup IS built via `innerHTML` (same pattern `frDispatchProposalAction` already uses for its own confirmation message), but every interpolated value in it (`confirmColor`, `msg`) is either a CSS-variable lookup or one of two fixed literal strings, never operator- or server-supplied text.

**Deliberately does NOT re-render the wind-down checklist client-side.** The checklist is assembled server-side at render time only (`advisors.retirement_checklist.build_checklist`, called from `ai_advisor_tab()` — see [app](app.md)) — a JS-side re-implementation would be a second, drift-prone copy of the same deterministic template. The approve confirmation message tells the operator to reload the page to see it.
