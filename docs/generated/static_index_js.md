...
- **Comparison rows** — `updateComparisonRows()` re-renders the Today/Cumulative/MDD rows on every poll; the Cumulative row sources the lifetime `cumulative_return` only (F-014), preserves a genuine `null` through to the honest empty-state instead of coercing it to a false zero (F-016 locus 1), and (as of `DE-AUDIT-BL4-001`) additionally renders the account-basis honesty chip/freshness markers described above.
