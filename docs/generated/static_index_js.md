...
**Account-basis honesty markers (`DE-AUDIT-BL4-001`, 2026-08-05):** at the top of the function, immediately after destructuring `var ps = (data && data.portfolio_strip) || {}`, three calls run before the rest of the row-rendering logic: `renderAccountBasisChip('comp-today-basis-chip', ps)`, `renderAccountBasisChip('comp-cumulative-basis-chip', ps)`, `renderAccountBasisFreshness(ps)`. See the dedicated section above for the two helpers' full contract. Zero change to the existing Today/Cumulative/MDD value-rendering logic below this point.
