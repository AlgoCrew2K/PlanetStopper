...
**Last updated:** 2026-07-02 (DE-PRISM-NUMERIC-VERIFY-001, new module)
...
The `<TICKER>.<CONCEPT>` shape is matched via `_TICKER_CONCEPT_RE` (`^[A-Za-z0-9]+\.[A-Za-z0-9_]+$`) in `_lookup_registry_entry` — any indicator string matching this pattern that is not itself a literal registry key falls back to the fundamentals wildcard entry.
...
- `DECISIONS.md` §`DE-PRISM-NUMERIC-VERIFY-001` — full design rationale, the Option (b) vs prose-regex fork, and the AC-6 magnitude-only deviation.
