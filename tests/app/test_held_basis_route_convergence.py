...
class TestF6NoInternalKeyLeakage:
    ...
    def test_frozen_tier2_floor_zero_paired_coverage_strips_guard_delta_vw_key(self, frozen_client):
        ...
        Co-located here (not in TestF5FrozenBranchConvergence) so this class
        holds the COMPLETE live/frozen x IF/else internal-key-leak guard
        matrix in one discoverable place -- the original leak guard missed
        this branch precisely because the guards weren't comprehensive/
        co-located.
        """
        app_module._account_totals_cache.clear()
        app_module._account_totals_last_good.clear()
...
# ---------------------------------------------------------------------------
# GDVW-PROJECTION-001 -- app_module._project_today_change_floor (extraction of
# app.py's 2 Tier-2 floor sites' copy-pasted guard_delta_vw re-derivation into a
# shared module-level helper, so a leak of the internal key structurally cannot
# happen at a future 5th call site). Pure-dict unit tests -- no DB/fixture
# needed, the helper takes a plain vw_tc dict.
#
# Lives here (tests/app/), not tests/analytics/: the helper is a module-level
# app.py function, not an analytics.py attribute -- deliberately, so tests
# that `patch.object(app_module, "analytics", MagicMock())` (e.g.
# tests/dashboard/test_card_consistency_liveness.py) never touch it. Putting
# it in analytics.py would make it a mock surface under exactly that pattern.
# ---------------------------------------------------------------------------


def _old_inline_floor_logic(vw_tc: dict) -> dict:
    """Verbatim copy of the pre-extraction inline if/else (both app.py sites
    ...
    if _floor_guard_delta is not None and vw_tc.get("if_held") is not None:
        return {
            "if_held": vw_tc["if_held"],
            "dry_run": vw_tc["if_held"] + _floor_guard_delta,
        }
    else:
        return {
            "if_held": vw_tc.get("if_held"),
            "dry_run": vw_tc.get("dry_run"),
        }


class TestFloorBasisHelper:
    def test_if_branch_computes_dry_run_from_if_held_plus_guard_delta(self):
        """guard_delta_vw + if_held both present -> dry_run is re-derived as
        if_held + guard_delta_vw. The input's own "dry_run" is a deliberate
        decoy value to prove it's ignored on this branch (the whole point of
        the re-derivation is that the raw dry_run is mismatched-membership).
        """
        vw_tc = {"if_held": 2.0, "dry_run": 999.0, "guard_delta_vw": 0.5}
        result = app_module._project_today_change_floor(vw_tc)
        assert result == {"if_held": 2.0, "dry_run": 2.5}

    def test_else_branch_passthrough_when_guard_delta_is_none(self):
        """Zero paired coverage -> guard_delta_vw resolves None -> honest
        passthrough, dry_run stays None (mirrors the route-level F6 test's
        zero-paired-coverage fixture: if_held=1.5, dry_run=None).
        """
        vw_tc = {"if_held": 1.5, "dry_run": None, "guard_delta_vw": None}
        result = app_module._project_today_change_floor(vw_tc)
        assert result == {"if_held": 1.5, "dry_run": None}

    def test_else_branch_passthrough_when_if_held_is_none(self):
        """guard_delta_vw present but if_held missing -- the `and` condition's
        other failure mode -- must also fall through to passthrough.
        """
        vw_tc = {"if_held": None, "dry_run": 3.0, "guard_delta_vw": 0.2}
        result = app_module._project_today_change_floor(vw_tc)
        assert result == {"if_held": None, "dry_run": 3.0}

    def test_else_branch_passthrough_when_guard_delta_key_absent(self):
        """A plain {"if_held", "dry_run"} dict with no guard_delta_vw key at
        all (e.g. a hand-built caller) -- .get() returns None, same as the
        explicit-None case above.
        """
        vw_tc = {"if_held": 4.0, "dry_run": 4.4}
        result = app_module._project_today_change_floor(vw_tc)
        assert result == {"if_held": 4.0, "dry_run": 4.4}

    def test_guard_delta_zero_exactly_still_triggers_if_branch(self):
        """`is not None`, not a truthiness check -- a genuine 0.0 guard delta
        must still re-derive dry_run (== if_held + 0.0), not fall through to
        the raw passthrough.
        """
        vw_tc = {"if_held": 3.0, "dry_run": -1.0, "guard_delta_vw": 0.0}
        result = app_module._project_today_change_floor(vw_tc)
        assert result == {"if_held": 3.0, "dry_run": 3.0}

    def test_if_held_zero_exactly_still_triggers_if_branch(self):
        """Same `is not None` edge, other operand: if_held == 0.0 must still
        be treated as present.
        """
        vw_tc = {"if_held": 0.0, "dry_run": -1.0, "guard_delta_vw": 0.5}
        result = app_module._project_today_change_floor(vw_tc)
        assert result == {"if_held": 0.0, "dry_run": 0.5}

    def test_structural_guarantee_never_leaks_guard_delta_vw_key_if_branch(self):
        """AC-4: even with guard_delta_vw AND an unrelated junk key present in
        the input, the IF-branch result must be exactly {if_held, dry_run}.
        """
        vw_tc = {
            "if_held": 1.0,
            "dry_run": 1.0,
            "guard_delta_vw": 0.3,
            "some_future_internal_key": "junk",
        }
        result = app_module._project_today_change_floor(vw_tc)
        assert set(result) == {"if_held", "dry_run"}
        assert "guard_delta_vw" not in result
        assert "some_future_internal_key" not in result

    def test_structural_guarantee_never_leaks_guard_delta_vw_key_else_branch(self):
        """AC-4, zero-coverage input (the ELSE/passthrough branch): must
        still construct a fresh 2-key dict, never leak guard_delta_vw even
        though it's present (as None) in the input.
        """
        vw_tc = {
            "if_held": 1.5,
            "dry_run": None,
    ...
    @pytest.mark.parametrize(
        "vw_tc",
        [
            {"if_held": 2.0, "dry_run": 999.0, "guard_delta_vw": 0.5},  # full coverage
            {"if_held": 1.5, "dry_run": None, "guard_delta_vw": None},  # zero coverage
            {"if_held": 3.0, "dry_run": -1.0, "guard_delta_vw": 0.0},  # guard_delta == 0.0
            {"if_held": 0.0, "dry_run": -1.0, "guard_delta_vw": 0.5},  # if_held == 0.0
            {"if_held": 5.0, "dry_run": 1.0, "guard_delta_vw": -2.5},  # negative guard_delta
            {"if_held": 4.0, "dry_run": 4.4},  # guard_delta_vw key absent entirely
            {"if_held": None, "dry_run": 3.0, "guard_delta_vw": 0.2},  # if_held missing
            {"if_held": None, "dry_run": None},  # both missing
        ],
        ids=[
            "full_coverage",
            "zero_coverage",
            "guard_delta_zero",
            "if_held_zero",
            "negative_guard_delta",
            "guard_delta_key_absent",
            "if_held_missing",
            "both_missing",
        ],
    )
    def test_matches_old_inline_logic_across_representative_inputs(self, vw_tc):
        """Differential invariance pin: the extracted helper must reproduce
        the pre-extraction inline if/else EXACTLY across every representative
        input shape, not just the cases the route-level tests happen to
        exercise. This is the strongest guarantee that the extraction is
        behavior-neutral.
        """
        expected = _old_inline_floor_logic(dict(vw_tc))
        actual = app_module._project_today_change_floor(dict(vw_tc))
        assert actual == expected, (
            f"GDVW-PROJECTION-001 FAIL: helper output {actual} diverges from the old inline "
            f"logic's output {expected} for input {vw_tc} -- the extraction is not "
            f"behavior-neutral."
        )
