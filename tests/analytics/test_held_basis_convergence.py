...
# ---------------------------------------------------------------------------
# GDVW-PROJECTION-001 -- get_portfolio_today_change_floor_basis (extraction of
# app.py's 2 Tier-2 floor sites' copy-pasted guard_delta_vw re-derivation into a
# shared helper, so a leak of the internal key structurally cannot happen at a
# future 5th call site). Pure-dict unit tests -- no DB/fixture needed, the
# helper takes a plain vw_tc dict.
# ---------------------------------------------------------------------------
...
class TestFloorBasisHelper:
    def test_if_branch_computes_dry_run_from_if_held_plus_guard_delta(self):
        ...
        result = analytics.get_portfolio_today_change_floor_basis(vw_tc)
    ...
    def test_matches_old_inline_logic_across_representative_inputs(self, vw_tc):
        ...
        actual = analytics.get_portfolio_today_change_floor_basis(dict(vw_tc))
