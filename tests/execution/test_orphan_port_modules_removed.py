...
Auto-resolution of S3-AUDIT-009:
  Two xfails at tests/execution/test_port_dispatch_removal.py:661-683 declared
  port_aggregator + port_selector "will not be importable after SITE-A2
  removal" (strict=False). Today they are XPASS because the modules still
  exist. After this cycle deletes the modules, the imports inside those xfail
  tests raise ImportError → the xfail catches → the tests record as PASS
  without source edits. This file asserts that auto-flip explicitly so a
  later regression that reintroduces the modules is caught here even before
  the xfail test inverts back to XPASS.
...
# ---------------------------------------------------------------------------
# xfail auto-flip — S3-AUDIT-009 resolves when the modules are gone
# ---------------------------------------------------------------------------
...
class TestXfailTripwiresAutoFlipToXfail:
    ...
    def test_two_xfail_tripwires_record_as_xfail_after_orphan_removal(self):
        ...
        if not target_file.exists():
            pytest.skip(
                "tests/execution/test_port_dispatch_removal.py not present "
                "in this worktree — skipping auto-flip cross-check"
            )

        target_tests = [
            f"{target_file}::TestPortmodeTestFilesToBeDeleted::"
            "test_port_aggregator_import_fails_after_removal",
            f"{target_file}::TestPortmodeTestFilesToBeDeleted::"
            "test_port_selector_import_fails_after_removal",
        ]
