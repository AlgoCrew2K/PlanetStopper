...
# Keys that MUST be redacted regardless of exact pattern implementation.
# The parametrize list is the authoritative spec — the test MUST be able to
# fail on an implementation that ignores any of these variants.
#
# Group A — covered by the initial regex (api[_-]?key|token|secret|password|webhook):
#   api_key, apikey, api-key, token, secret, password, webhook, WEBHOOK_URL
# Group B — HTTP auth / credential conventions NOT in the initial regex (F-1 RED):
#   Authorization, Bearer, authorization, credential, cookie, auth
_SECRET_KEY_CASES = [
    # Group A — initial spec
    "api_key",
    "apikey",
    "api-key",
    "token",
    ...
    "webhook",
    "WEBHOOK_URL",
    # Group B — HTTP auth / credential conventions (F-1 RED — must fail before fix)
    "Authorization",
    "Bearer",
    "authorization",
    "credential",
...
@pytest.mark.parametrize("secret_key", _SECRET_KEY_CASES)
def test_secret_key_is_redacted_at_top_level(warehouse_db, secret_key):
    """AC-8 (non-vacuous): each parametrized secret key must be PRESENT in storage
    AND have its value replaced with '<redacted>'.

    The previous vacuous version filtered the stored JSON through the same regex
    as the implementation, producing an empty list for keys like 'Authorization'
    so all([]) passed trivially.  This version asserts the key exists by its
    original name (case-preserved) and that its stored value == '<redacted>'.
    """
    _wh_path, lw = warehouse_db
    sentinel_secret = "supersecret_value_12345"
    payload = {
        "data": "visible",
    ...
    # Fetch raw_json directly from SQLite — bypass get_lens_snapshots so
    # a bug in the reader can't mask a redaction failure.
    conn = sqlite3.connect(_wh_path)
    try:
        stored_raw_json_str = conn.execute(
            "SELECT raw_json FROM lens_snapshots WHERE id=?", (row_id,)
        ).fetchone()[0]
    finally:
        conn.close()

    stored = json.loads(stored_raw_json_str)

    # Non-secret key must be preserved
    assert stored.get("data") == "visible", "'data' key should be preserved (not a secret)"

    # The secret key MUST exist in the stored dict (key name is preserved, value replaced)
    assert secret_key in stored, (
        f"Secret key '{secret_key}' is absent from stored JSON — "
        "implementation must preserve the key name and replace the value with '<redacted>'"
    )

    # The stored value MUST be the redaction sentinel, NOT the original secret
    stored_value = stored[secret_key]
    assert stored_value == "<redacted>", (
        f"Key '{secret_key}': expected '<redacted>', got {stored_value!r}. "
        "Secret value was persisted verbatim."
    )

    # Belt-and-suspenders: original secret must not appear anywhere in stored JSON
    assert sentinel_secret not in stored_raw_json_str, (
        f"Original secret value for key '{secret_key}' found verbatim in raw_json"
    )
...
# ---------------------------------------------------------------------------
# F-2 RED: Secret-in-value detection — benign keys containing secret strings
# ---------------------------------------------------------------------------
#
# A secret can appear as part of a VALUE under a non-secret key name, e.g.:
#   {"url": "https://discord.com/api/webhooks/12345/abcdef"}
#   {"config": "Bearer sk-ant-abc123"}
#
# These MUST NOT be persisted verbatim.  These tests define the contract;
# they are expected to FAIL until the implementer extends _strip_secrets to
# scan string values in addition to key names.


@pytest.mark.parametrize("benign_key,secret_value", [
    # Discord / Slack webhook URL in a URL-typed field
    (
        "url",
        "https://discord.com/api/webhooks/1234567890/ABCDEFGHIJ_secret_token_value",
    ),
    # Bearer token in a config string
    (
        "config",
        "Bearer sk-ant-api03-abcdefghijklmnopqrstuvwxyz1234567890ABCDE",
    ),
    # API key pattern in a string value under an innocuous key
    (
        "endpoint_auth",
        "api_key=supersecret_12345",
    ),
])
def test_secret_in_value_under_benign_key_is_not_persisted(warehouse_db, benign_key, secret_value):
    """F-2 RED: A secret embedded in a STRING VALUE under a non-secret key name
    must not be stored verbatim.

    The key name 'url', 'config', or 'endpoint_auth' does not match the
    secret-key regex, so naive key-only redaction passes the value through
    unchanged.  The stored raw_json must NOT contain the raw secret string.

    Acceptable outcomes (implementation chooses one):
      - The entire value is replaced with '<redacted>', OR
      - The secret sub-string within the value is masked/truncated.
    Either way the original secret_value string must NOT appear verbatim.
    """
    _wh_path, lw = warehouse_db
    payload = {
        "safe_field": "not_a_secret",
        benign_key: secret_value,
    }
    ...
    assert secret_value not in stored_raw_json_str, (
        f"Secret value under benign key '{benign_key}' was persisted verbatim in raw_json. "
        "Value-level secret detection is required."
    )
