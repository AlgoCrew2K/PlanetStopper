"""
AC-10 (migration 016) — additive composite index on shadow_history.
...
Fix (plan AC-10): add migration 016 — a composite index on
``shadow_history (symphony_id, ts_utc)`` so the LIMIT-1 sort is index-served.
Non-destructive, idempotent (`IF NOT EXISTS`), additive-only.

Pin:
  1. migrations/016_shadow_history_sym_ts_index.sql exists.
  2. Its content creates the named composite index with IF NOT EXISTS
     against shadow_history(symphony_id, ts_utc).
  3. After running migrations (via database.migrate or the manual apply
     path), SQLite's ``PRAGMA index_list('shadow_history')`` shows the
     new index name, and ``PRAGMA index_info(<name>)`` shows the
     (symphony_id, ts_utc) columns in that order.
...
MIGRATION_FILE = MIGRATIONS_DIR / "016_shadow_history_sym_ts_index.sql"
EXPECTED_INDEX_NAME = "idx_shadow_history_sym_ts"


class TestMigration016FileExists:
    """The plan dictates the exact filename."""

    def test_migration_016_file_exists(self):
        assert MIGRATION_FILE.exists(), (
            f"AC-10 / migration 016: expected file {MIGRATION_FILE!s} not "
            "found. The plan's filename is "
            "'016_shadow_history_sym_ts_index.sql'."
        )

    def test_migration_016_contents_create_composite_index(self):
        """The file must contain a CREATE INDEX statement (idempotent via
        IF NOT EXISTS) on shadow_history(symphony_id, ts_utc)."""
        if not MIGRATION_FILE.exists():
            pytest.fail("016 migration not present; see preceding test.")
        sql = MIGRATION_FILE.read_text(encoding="utf-8").lower()

        assert "create index" in sql, (
            "Migration 016 must use CREATE INDEX, not a different DDL "
            "verb."
        )
        assert "if not exists" in sql, (
            "Migration 016 must be idempotent — use 'IF NOT EXISTS'."
        )
        assert EXPECTED_INDEX_NAME.lower() in sql, (
            f"Migration 016 must name the index {EXPECTED_INDEX_NAME!r}."
        )
        assert "shadow_history" in sql, (
            "Migration 016 must target the shadow_history table."
        )
...
class TestMigration016AppliedCreatesIndex:
    ...
    def test_applying_016_creates_named_composite_index(self):
        if not MIGRATION_FILE.exists():
            pytest.fail("016 migration not present.")
        ...
        assert cur.fetchone() is not None, (
            f"AC-10 / migration 016 VIOLATED: index "
        ...
        assert column_order == ["symphony_id", "ts_utc"], (
            f"AC-10 / migration 016 VIOLATED: composite index columns "
