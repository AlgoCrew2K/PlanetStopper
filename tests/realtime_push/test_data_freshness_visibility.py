...
#
# Scope decision:
#   IN-SCOPE: app.py:2117  `data_as_of = datetime.now().strftime("%H:%M ET")`
#     This top-level field is returned at app.py:2230 and consumed by the JS
#     as the FALLBACK when `meta.portfolio.data_as_of` is absent
#     (index.js: `portfolio.data_as_of || data.data_as_of`).
#     When bot_state carries a `last_successful_cycle_at`, the top-level
#     data_as_of should derive from that timestamp, not the render clock.
#     The existing tests only cover portfolio_strip.data_as_of (GREEN); this
#     top-level field is still naive datetime.now() — no _ET timezone, render-clock.
#
#   OUT-OF-SCOPE (defensible render-clock):
#     app.py:1362  exception-fallback in _compute_portfolio_strip — the entire
#     strip computation failed, no bot_state data is reachable; the render clock
#     is the only available value.  No data timestamp is available to substitute.
#     Rationale filed to rtf-doc: "exception fallback has no bot_state to read;
#     datetime.now(_ET) is the correct behavior for a full-strip failure path."
# ---------------------------------------------------------------------------


class TestTopLevelDataAsOfDerivesFromCycleTimestamp:
    """RED tests: top-level data_as_of at app.py:2117 must derive from
    last_successful_cycle_at, not datetime.now() (no timezone, render clock).

    The JS reads `portfolio.data_as_of || data.data_as_of` — when portfolio.data_as_of
    is present (GREEN, from _compute_portfolio_strip), the top-level field is a fallback.
    But the top-level field is also passed to render_template (HTML table partial) and
    must not mislead an operator reading it as "current" when data is stale.

    These tests FAIL until rt-impl changes app.py:2117 to read
    last_successful_cycle_at from state_data (same pattern as _compute_portfolio_strip).
    """
    ...
    def test_get_state_top_level_data_as_of_not_always_render_clock(self, client):
        ...
        2. GET /api/state.
        3. Assert top-level data_as_of does NOT contain the current ET clock HH:MM
           when that differs from the known cycle timestamp.

        Uses a past fixed timestamp (10:09 ET) so the ET render-clock HH:MM is distinct
        unless the test happens to run at exactly 10:09:XX ET (skipped in that case).

        Note: app.py:2117 uses datetime.now() (no timezone) which may produce a UTC
        or local-system HH:MM; we compare against both ET-now AND the raw now() to
        detect either form of render-clock exposure.

        Fails until rt-impl changes app.py:2117 from `datetime.now().strftime(...)` to
        derive data_as_of from state_data's last_successful_cycle_at.
        """
        from datetime import datetime
        ...
        if not top_level_dao:
            # Absent is not wrong — just cannot assert render-clock behaviour.
            return

        # Skip if we happen to be running at the same minute as the fake cycle.
        # Capture multiple now() forms (ET-aware and naive) to detect either flavor.
        ...
        # Both ET and naive render-clock values differ from the cycle label.
        # If the top-level field encodes EITHER clock form and NOT the cycle label,
        # the implementation is using a render clock.
        ...
        if (is_et_clock or is_naive_clock) and not has_cycle_label:
            ...
            pytest.fail(
                ...
                "rt-impl: change app.py:2117 to derive data_as_of from "
                "state_data last_successful_cycle_at, matching the pattern in "
                "_compute_portfolio_strip (app.py:1281-1303)."
            )
    ...
    def test_get_state_top_level_data_as_of_encodes_cycle_timestamp(self, client):
        ...
        Fails until rt-impl reads last_successful_cycle_at from state_data at
        app.py:2117 (currently `datetime.now().strftime('%H:%M ET')`).
        """
        import database as db
        ...
        assert _FAKE_CYCLE_LABEL in top_level_dao, (
            ...
            "rt-impl: change app.py:2117 to derive data_as_of from the cycle "
            "timestamp, not datetime.now() (which is always the render clock)."
        )
