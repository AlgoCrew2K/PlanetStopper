...
Design invariants:
  ...
  - Secret-stripping (two layers):
      1. KEY-side: any key matching api[_-]?key / token / secret / password
         / webhook / authorization / bearer / credential / cookie / auth
         (case-insensitive, any nesting depth) has its value replaced with
         "<redacted>" before JSON serialisation.
      2. VALUE-side: any string value that contains a high-signal secret
         pattern (Bearer token, sk- API key, Discord/Slack webhook URL) is
         replaced with "<redacted>" regardless of the key name.
  - D-1 error contract: errors surface type(exc).__name__ only (never raw
    str(exc) containing potential credentials or stack frames to callers).
...
# Covers API key conventions, auth header names, and credential identifiers.
_SECRET_KEY_PATTERN = re.compile(
    r"api[_-]?key|token|secret|password|webhook"
    r"|authorization|bearer|credential|cookie|auth",
    re.IGNORECASE,
)

# Regex for secret-value detection — applied to string values regardless of key name.
# Conservative: matches only high-signal patterns unlikely to appear in normal text.
#   - Bearer <token>  (HTTP Authorization header value)
#   - sk-<alphanum>   (Anthropic / OpenAI style API keys)
#   - Discord/Slack webhook URLs  (https://discord.com/api/webhooks/... or
...
def _strip_secrets(obj: Any) -> Any:
    """Recursively replace secret-key values and secret string values with '<redacted>'.

    Two layers of protection are applied at every nesting level:

    1. KEY-side: if a dict key matches _SECRET_KEY_PATTERN (api_key, token,
       authorization, bearer, auth, cookie, credential, …) the corresponding
       value is replaced with '<redacted>' unconditionally.

    2. VALUE-side: if a string value (under any key) matches
       _SECRET_VALUE_PATTERN (Bearer token, sk- API key, webhook URL), the
       entire string value is replaced with '<redacted>'.

    Traverses dicts and lists at any nesting depth.  The replacement is done
    on a structural copy so the caller's original object is never mutated.
    """
    ...
    if isinstance(obj, str) and _SECRET_VALUE_PATTERN.search(obj):
        return _REDACTED
    return obj
...
def init_warehouse() -> None:
    ...
    D-1: raises RuntimeError(type(exc).__name__) on DB failure — never exposes
    raw exception messages, file paths, or stack frames to callers.
    """
...
def persist_lens_snapshot(
    ...
    try:
        ...
        # D-1: surface only the exception class name — never raw message, path, or traceback.
        raise RuntimeError(type(exc).__name__) from None
    finally:
        conn.close()
